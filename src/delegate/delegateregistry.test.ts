...
testSuite({
  ...
  testBasic_registerMultiple() {
    ...
    assertEquals('Bar(42) Foo(23) Foo(42)', registry.delegates((ctor) => new ctor(42)).join(' '));
  ...
  testPrioritized_registerClass() {
    ...
    assertEquals('Bar(2) Foo(2)', registry.delegates((ctor) => new ctor(2)).join(' '));
  },
  ...
  testCacheInstantiation_withLateRegistration() {
    /** @type {!DelegateRegistry<!Foo>} */
    const registry = new DelegateRegistry().cacheInstantiation().allowLateRegistration();
    registry.registerClass(Foo);
    registry.registerClass(Foo);
    assertEquals('Foo(42)', registry.delegate((ctor) => new ctor(42)).toString());
    assertEquals('Foo(42) Foo(3)', registry.delegates((ctor) => new ctor(3)).join(' '));
    registry.registerClass(Foo);
    assertEquals('Foo(42) Foo(3) Foo(99)', registry.delegates((ctor) => new ctor(99)).join(' '));
  },
