 ...
 */
goog.date.UtcDateTime.fromTimestamp = (timestamp) => {
 ...
 */
goog.date.UtcDateTime.fromIsoString = (formatted) => {
...
goog.date.UtcDateTime.prototype.add = function (interval) {
  ...
  var daysAndTimeMillis =
    1000 *
    (interval.seconds + 60 * (interval.minutes + 60 * (interval.hours + 24 * interval.days)));
  this.date = new Date(this.date.getTime() + daysAndTimeMillis);
};

/** @override */
goog.date.UtcDateTime.prototype.getTimezoneOffset = () => 0;
