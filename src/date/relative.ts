...
goog.scope(() => {
   ...
   */
  goog.date.relative.setCasingMode = (capitalizeMode) => {
   ...
   */
  goog.date.relative.upcase = (text) => {
    ...
    if (
      codePoint0 >= goog.date.relative.SURROGATE_LOW_ &&
   ...
   * @private
   */
  goog.date.relative.relativeCasedString_ = (dayOffset) => {
   ...
   * @private
   */
  goog.date.relative.formatMonth_ = (date) => {
   ...
   * @private
   */
  goog.date.relative.formatShortTime_ = (date) => {
   ...
   * @private
   */
  goog.date.relative.formatFullDate_ = (date) => {
   ...
   * @private
   */
  goog.date.relative.rdtformat_ = (absQuantity, futureFlag, relUnit) => {
   ...
   */
  goog.date.relative.format = (dateMs) => {
    var now = goog.now();
   ...
   * @return {string} The formatted date.
   */
  goog.date.relative.formatPast = (dateMs) => {
    var now = goog.now();
    if (now < dateMs) {
      dateMs = now;
    }
   ...
   * @return {string} The formatted date.
   */
  goog.date.relative.formatDay = (dateMs, opt_formatter) => {
    ...
    const timezoneOffsetCorrection =
      (today.getTimezoneOffset() - originalTimezoneOffset) * goog.date.relative.MINUTE_MS_;

    let dayOffset =
      (dateMs - today.getTime() + timezoneOffsetCorrection) / goog.date.relative.DAY_MS_;
   ...
   */
  goog.date.relative.getDateString = (date, opt_shortTimeMsg, opt_fullDateMsg) =>
    goog.date.relative.getDateString_(
      date,
   ...
   * @return {string} The date string in the above form.
   */
  goog.date.relative.getPastDateString = (date, opt_shortTimeMsg, opt_fullDateMsg) =>
    goog.date.relative.getDateString_(
      date,
