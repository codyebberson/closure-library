...
goog.html.SafeUrl.unwrap = (safeUrl) => {
  ...
  if (safeUrl instanceof goog.html.SafeUrl && safeUrl.constructor === goog.html.SafeUrl) {
    ...
    goog.asserts.fail(
      "expected object of type SafeUrl, got '" + safeUrl + "' of type " + goog.typeOf(safeUrl)
    );
 ...
 * @package
 */
goog.html.SafeUrl.isSafeMimeType = (mimeType) => goog.html.SAFE_MIME_TYPE_PATTERN_.test(mimeType);
 ...
 *   as a SafeUrl.
 */
goog.html.SafeUrl.fromBlob = (blob) => {
 ...
 */
goog.html.SafeUrl.revokeObjectUrl = (safeUrl) => {
 ...
 */
goog.html.SafeUrl.fromMediaSource = (mediaSource) => {
 ...
 */
goog.html.SafeUrl.fromDataUrl = (dataUrl) =>
 ...
 */
goog.html.SafeUrl.fromTelUrl = (telUrl) => {
 ...
 *     wrapped as a SafeUrl if it does not pass.
 */
goog.html.SafeUrl.fromSipUrl = (sipUrl) => {
 ...
 *     wrapped as a SafeUrl if it does not pass.
 */
goog.html.SafeUrl.fromFacebookMessengerUrl = (facebookMessengerUrl) => {
  if (
    !goog.string.internal.caseInsensitiveStartsWith(facebookMessengerUrl, 'fb-messenger://share')
  ) {
 ...
 *     wrapped as a SafeUrl if it does not pass.
 */
goog.html.SafeUrl.fromWhatsAppUrl = (whatsAppUrl) => {
 ...
 *     wrapped as a SafeUrl if it does not pass.
 */
goog.html.SafeUrl.fromSmsUrl = (smsUrl) => {
  if (
 ...
 *     wrapped as a SafeUrl if it does not pass.
 */
goog.html.SafeUrl.fromSshUrl = (sshUrl) => {
 ...
 */
goog.html.SafeUrl.sanitizeChromeExtensionUrl = (url, extensionId) =>
  goog.html.SafeUrl.sanitizeExtensionUrl_(/^chrome-extension:\/\/([^\/]+)\//, url, extensionId);
 ...
 *     `INNOCUOUS_STRING` if it's not.
 */
goog.html.SafeUrl.sanitizeFirefoxExtensionUrl = (url, extensionId) =>
  goog.html.SafeUrl.sanitizeExtensionUrl_(/^moz-extension:\/\/([^\/]+)\//, url, extensionId);
 ...
 *     `INNOCUOUS_STRING` if it's not.
 */
goog.html.SafeUrl.sanitizeEdgeExtensionUrl = (url, extensionId) =>
  goog.html.SafeUrl.sanitizeExtensionUrl_(/^ms-browser-extension:\/\/([^\/]+)\//, url, extensionId);
 ...
 *     `INNOCUOUS_STRING` if it's not.
 */
goog.html.SafeUrl.sanitizeExtensionUrl_ = (scheme, url, extensionId) => {
 ...
 */
goog.html.SafeUrl.fromTrustedResourceUrl = (trustedResourceUrl) =>
 ...
 *     if validation fails.
 */
goog.html.SafeUrl.trySanitize = (url) => {
...
goog.html.SafeUrl.sanitize = (url) =>
  goog.html.SafeUrl.trySanitize(url) || goog.html.SafeUrl.INNOCUOUS_URL;
 ...
 * @return {!goog.html.SafeUrl} The validated URL, wrapped as a SafeUrl.
 */
goog.html.SafeUrl.sanitizeAssertUnchanged = (url, opt_allowDataUrl) => {
  if (url instanceof goog.html.SafeUrl) {
    return url;
 ...
 */
goog.html.SafeUrl.extractScheme = (url) => {
  let parsedUrl;
  try {
...
goog.html.SafeUrl.createSafeUrlSecurityPrivateDoNotAccessOrElse = (url) =>
  new goog.html.SafeUrl(url, goog.html.SafeUrl.CONSTRUCTOR_TOKEN_PRIVATE_);
