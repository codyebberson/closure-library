...
testSuite({
  testSafeHtmlFromStringKnownToSatisfyTypeContract_ok() {
    ...
    const safeHtml = uncheckedconversions.safeHtmlFromStringKnownToSatisfyTypeContract(
      Const.from('Test'),
      html
    );
  ...
  testSafeScriptFromStringKnownToSatisfyTypeContract_ok() {
    ...
    const safeScript = uncheckedconversions.safeScriptFromStringKnownToSatisfyTypeContract(
      Const.from('Safe because value is constant. Security review: b/7685625.'),
      script
    );
  ...
  testSafeStyleFromStringKnownToSatisfyTypeContract_ok() {
    ...
    const safeStyle = uncheckedconversions.safeStyleFromStringKnownToSatisfyTypeContract(
      Const.from('Safe because value is constant. Security review: b/7685625.'),
      style
    );
  ...
  testSafeStyleSheetFromStringKnownToSatisfyTypeContract_ok() {
    ...
    const safeStyleSheet = uncheckedconversions.safeStyleSheetFromStringKnownToSatisfyTypeContract(
      Const.from('Safe because value is constant. Security review: b/7685625.'),
      styleSheet
    );
  ...
  testTrustedResourceUrlFromStringKnownToSatisfyTypeContract_ok() {
    ...
    const trustedResourceUrl =
      uncheckedconversions.trustedResourceUrlFromStringKnownToSatisfyTypeContract(
        Const.from('Safe because value is constant. Security review: b/7685625.'),
        url
      );
