...
const HtmlSanitizerAttributePolicy = goog.requireType(
...
function assertSanitizedHtml(
  ...
  dom.assertHtmlMatches(expectedHtml, SafeHtml.unwrap(sanitized), true /* opt_strictAttributes */);
}
...
testSuite({
  ...
  testAllowUnknownAttributeWildCard() {
    ...
    assertSanitizedHtml(input, expected, null, [{ tagName: '*', attributeName: 'aB' }]);
  },

  testAllowUnknownAttributeOnSpecificTag() {
    const input = '<a www="3" zzz="4">fff</a><img www="3" />';
    const expected = '<a www="3">fff</a><img />';
    assertSanitizedHtml(input, expected, null, [{ tagName: 'a', attributeName: 'WwW' }]);
  },
    ...
    assertSanitizedHtml(input, expected, null, [
      {
        tagName: 'a',
        attributeName: 'href',
        policy: (value) => (value === 'yes' ? value : null),
      },
    ]);
  },
