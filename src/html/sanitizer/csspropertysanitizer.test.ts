...
function getProcessedPropertyValue(name, value) {
  ...
  div.innerHTML = '<div style="' + name + ': ' + value.replace(/"/g, '&quot;') + '"></div>';
...
testSuite({
  testBrowserBehavior_mixedCase() {
    ...
    assertEquals(
      'url("http://foo.com/a")',
      getProcessedPropertyValue('background-iMAge', 'uRL("http://foo.com/a")')
    );
  },

  testBrowserBehavior_spaces() {
    assertEquals('', getProcessedPropertyValue('background-color', 'rgba (1,2,3,0)'));
    assertEquals(
      'rgba(1, 2, 3, 0)',
      getProcessedPropertyValue('background-color', 'rgba(1,2,3,0)')
    );
    assertEquals('', getProcessedPropertyValue('background-image', 'url ( "http://foo.com/a" )'));
  },
  ...
  testBrowserBehavior_noComment() {
    ...
    assertEquals('red', getProcessedPropertyValue('color', ' /* a */ red /* b */'));
    assertEquals(
      'url("http://foo.com/u")',
      getProcessedPropertyValue('background-image', ' /* a */ url("http://foo.com/u")')
    );
  },

  testBrowserBehavior_noParanthesesInUnquotedUrl() {
    // Verify how browsers deal with extra parentheses in unquoted urls.
    assertEquals('', getProcessedPropertyValue('background-image', 'url(http://foo.com/a)b)'));
    // Parentheses can be escaped.
    assertEquals(
      'url("http://foo.com/a)b")',
      getProcessedPropertyValue('background-image', 'url(http://foo.com/a\\)b)')
    );
    assertEquals('', getProcessedPropertyValue('background-image', 'url(http://foo.com/a(b)c)'));
  },
  ...
  testBrowserBehavior_escapedQuotesMultiple() {
    ...
    assertEquals(
      expectedValue,
      getProcessedPropertyValue('background-image', 'url("http://foo.com/a\\"), rgba(1,1,1,0)')
    );
    assertEquals(
      '',
  ...
  testBrowserBehavior_urlArguments() {
    ...
    assertEquals('', getProcessedPropertyValue('background-image', 'url("http://foo.com" abc)'));
    assertEquals('', getProcessedPropertyValue('background-image', 'url("http://foo.com", abc)'));
  },
  ...
  testBrowserBehavior_relative() {
    ...
    assertTrue(
      getProcessedPropertyValue('background-image', 'url(/foo.com/a.jpg)').startsWith(
        'url("/foo.com'
      )
    );
    assertTrue(
      getProcessedPropertyValue('background-image', 'url(a.jpg)').startsWith('url("a.jpg')
    );
  },

  testSanitizeProperty_basic() {
    assertEquals('1px', CssPropertySanitizer.sanitizeProperty(NAME, '1px'));
    assertEquals('1px 2px', CssPropertySanitizer.sanitizeProperty(NAME, '1px 2px'));
    assertEquals('auto', CssPropertySanitizer.sanitizeProperty(NAME, 'auto'));
  },

  testSanitizeProperty_oneFunction() {
    assertEquals(null, CssPropertySanitizer.sanitizeProperty(NAME, 'foo(1,2,3,4)'));
    assertEquals('rgba(1,2,3,4)', CssPropertySanitizer.sanitizeProperty(NAME, 'rgba(1,2,3,4)'));
  },

  testSanitizeProperty_nestedFunctions() {
    assertEquals(
      null,
      CssPropertySanitizer.sanitizeProperty(NAME, 'linear-gradient(1,foo(2),3,4)')
    );
    assertEquals(
      'linear-gradient(red, rgba(1,2,3,4))',
      CssPropertySanitizer.sanitizeProperty(NAME, 'linear-gradient(red, rgba(1,2,3,4))')
    );
  },

  testSanitizeProperty_noStrings() {
    assertEquals(null, CssPropertySanitizer.sanitizeProperty(NAME, 'rgba("1",2,3,4)'));
    assertEquals(
      null,
      CssPropertySanitizer.sanitizeProperty(NAME, 'linear-gradient("a", rgba(1,2,3,4)')
    );
  },
  ...
  testSanitizeProperty_complexNested() {
    ...
    assertEquals(expectedValue, CssPropertySanitizer.sanitizeProperty(NAME, expectedValue));
  },
  ...
  testSanitizeProperty_url() {
    ...
    assertEquals(null, CssPropertySanitizer.sanitizeProperty(NAME, url, googFunctions.FALSE));
    assertEquals(url, CssPropertySanitizer.sanitizeProperty(NAME, url, SafeUrl.sanitize));
    assertEquals(
      'url("https://foo.com")',
      CssPropertySanitizer.sanitizeProperty(NAME, url, (url) =>
        SafeUrl.sanitize(url.replace('http', 'https'))
      )
    );
  },
  ...
  testSanitizeProperty_mixedCaseFunction() {
    ...
    assertEquals(lowerCaseValue, CssPropertySanitizer.sanitizeProperty(NAME, lowerCaseValue));
    const mixedCaseValue = 'translateX(10px)';
    assertEquals(mixedCaseValue, CssPropertySanitizer.sanitizeProperty(NAME, mixedCaseValue));
  },
});
