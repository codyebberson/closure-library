...
goog.format.InternationalizedEmailAddress = function (opt_address, opt_name) {
  goog.format.InternationalizedEmailAddress.base(this, 'constructor', opt_address, opt_name);
};
goog.inherits(goog.format.InternationalizedEmailAddress, goog.format.EmailAddress);
 ...
 */
goog.format.InternationalizedEmailAddress.isValidLocalPartSpec = (str) => {
  if (str == null) {
    return false;
  }
 ...
 */
goog.format.InternationalizedEmailAddress.isValidDomainPartSpec = (str) => {
  if (str == null) {
    return false;
  }
...
goog.format.InternationalizedEmailAddress.prototype.isValid = function () {
  return goog.format.InternationalizedEmailAddress.isValidAddrSpec(this.address);
};
 ...
 */
goog.format.InternationalizedEmailAddress.isValidAddress = (str) => {
  if (str == null) {
    return false;
  }
 ...
 */
goog.format.InternationalizedEmailAddress.isValidAddrSpec = (str) => {
  if (str == null) {
    return false;
  }
 ...
 */
goog.format.InternationalizedEmailAddress.parseList = (str) =>
  goog.format.EmailAddress.parseListInternal(
    str,
    goog.format.InternationalizedEmailAddress.parse,
 ...
 */
goog.format.InternationalizedEmailAddress.parse = (addr) =>
  goog.format.EmailAddress.parseInternal(addr, goog.format.InternationalizedEmailAddress);

/**
 * @param {string} ch The character to test.
 * @return {boolean} Whether the provided character is an address separator.
 */
goog.format.InternationalizedEmailAddress.isAddressSeparator = (ch) =>
  goog.string.contains(goog.format.InternationalizedEmailAddress.ADDRESS_SEPARATORS_, ch);
...
goog.format.InternationalizedEmailAddress.prototype.toString = function () {
  return this.toStringInternal(goog.format.InternationalizedEmailAddress.CHARS_REQUIRE_QUOTES_);
};
