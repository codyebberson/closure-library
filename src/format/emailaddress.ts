...
goog.format.EmailAddress.CHARS_REQUIRE_QUOTES_ =
  goog.format.EmailAddress.SPECIAL_CHARS + goog.format.EmailAddress.ADDRESS_SEPARATORS_;
 ...
 * @private {!RegExp}
 */
goog.format.EmailAddress.LOCAL_PART_ = new RegExp(
 ...
 * @private {!RegExp}
 */
goog.format.EmailAddress.DOMAIN_PART_ = new RegExp(
...
goog.format.EmailAddress.ALL_BIDI_FORMAT_CHARS_ = new RegExp(
  '[' + goog.object.getValues(goog.i18n.bidi.Format).join('') + ']',
  'g'
);
 ...
 * @private
 */
goog.format.EmailAddress.isQuoteNeeded_ = (name, specialChars) => {
 ...
 */
goog.format.EmailAddress.isValidAddress = (str) => goog.format.EmailAddress.parse(str).isValid();
 ...
 */
goog.format.EmailAddress.isValidAddrSpec = (str) => {
 ...
 */
goog.format.EmailAddress.isValidLocalPartSpec = (str) =>
  goog.format.EmailAddress.LOCAL_PART_.test(str);
 ...
 */
goog.format.EmailAddress.isValidDomainPartSpec = (str) =>
 ...
 * @protected
 */
goog.format.EmailAddress.parseInternal = (addr, ctor) => {
 ...
 * @return {!goog.format.EmailAddress} The parsed address.
 */
goog.format.EmailAddress.parse = (addr) =>
  goog.format.EmailAddress.parseInternal(addr, goog.format.EmailAddress);
 ...
 * @protected
 */
goog.format.EmailAddress.parseListInternal = (str, parser, separatorChecker) => {
  var result = [];
  var email = '';
  var token;
 ...
 * @return {!Array<!goog.format.EmailAddress>} The parsed emails.
 */
goog.format.EmailAddress.parseList = (str) =>
  goog.format.EmailAddress.parseListInternal(
    str,
 ...
 * @private
 */
goog.format.EmailAddress.getToken_ = (str, pos) => {
 ...
 * @private
 */
goog.format.EmailAddress.isEscapedDlQuote_ = (str, pos) => {
  if (str.charAt(pos) != '"') {
    return false;
  }
  var slashCount = 0;
 ...
 */
goog.format.EmailAddress.isAddressSeparator = (ch) =>
  goog.string.contains(goog.format.EmailAddress.ADDRESS_SEPARATORS_, ch);
 ...
 * @private
 */
goog.format.EmailAddress.stripBidiChars_ = (str) =>
  str.replace(goog.format.EmailAddress.ALL_BIDI_FORMAT_CHARS_, '');
