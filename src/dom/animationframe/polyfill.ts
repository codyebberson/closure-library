 ...
 */
goog.dom.animationFrame.polyfill.ENABLED = goog.define(
  ...
  if (goog.dom.animationFrame.polyfill.ENABLED) {
    ...
    for (let i = 0; (v = vendors[i] && !goog.global.requestAnimationFrame); ++i) {
      ...
      goog.global.cancelAnimationFrame =
        goog.global[v + 'CancelAnimationFrame'] || goog.global[v + 'CancelRequestAnimationFrame'];
    }

    if (!goog.global.requestAnimationFrame) {
      let lastTime = 0;
      goog.global.requestAnimationFrame = (callback) => {
      ...
      if (!goog.global.cancelAnimationFrame) {
        goog.global.cancelAnimationFrame = (id) => {
          clearTimeout(id);
        };
      }
    }
