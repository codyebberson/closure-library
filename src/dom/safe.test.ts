...
testSuite({
  ...
  testInsertAdjacentHtml() {
    ...
    safe.insertAdjacentHtml(
      mockNode,
      InsertAdjacentHtmlPosition.BEFOREBEGIN,
  ...
  testSetInnerHtml() {
    ...
    const html = '<script>somethingTrusted();<' + '/script>';
  ...
  testsetLinkHrefAndRel_trustedResourceUrl() {
    const mockLink = /** @type {!HTMLLinkElement} */ {
      href: null,
      rel: null,
      setAttribute: () => {},
      tagName: 'LINK',
    ...
    const url = TrustedResourceUrl.fromConstant(Const.from('javascript:trusted();'));
  ...
  testsetLinkHrefAndRel_string() {
    ...
    assertThrows(() => {
      safe.setLinkHrefAndRel(mockLink, 'javascript:evil();', 'foo, stylesheet, bar');
    });
  ...
  testsetLinkHrefAndRel_assertsType() {
    ...
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name LINK'));
  },
  ...
  testSetAnchorHref() {
    ...
    const ex = assertThrows(() => {
      safe.setAnchorHref(/** @type {!HTMLAnchorElement} */ otherElement, 'about:blank');
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name A'));
  },
  ...
  testSetAudioSrc() {
    ...
    const ex = assertThrows(() => {
      safe.setAudioSrc(/** @type {!HTMLAudioElement} */ otherElement, safeUrl);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name AUDIO'));
  },

  testSetAudioSrc_withDataUrl() {
    const mockAudioElement = /** @type {!HTMLAudioElement} */ {
      src: 'blarg',
      tagName: 'AUDIO',
      ...nodeAttributes,
    };
  ...
  testSetVideoSrc() {
    ...
    const otherElement = document.createElement('SCRIPT');
    const ex = assertThrows(() => {
      safe.setVideoSrc(/** @type {!HTMLVideoElement} */ otherElement, safeUrl);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name VIDEO'));
  },

  testSetVideoSrc_withDataUrl() {
    const mockVideoElement = /** @type {!HTMLVideoElement} */ {
      src: 'blarg',
      tagName: 'VIDEO',
      ...nodeAttributes,
    };
  ...
  testSetEmbedSrc() {
    ...
    const ex = assertThrows(() => {
      safe.setEmbedSrc(/** @type {!HTMLEmbedElement} */ otherElement, url);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name EMBED'));
  },
  ...
  testSetFrameSrc() {
    ...
    const otherElement = document.createElement('IMAGE');
    const ex = assertThrows(() => {
      safe.setFrameSrc(/** @type {!HTMLFrameElement} */ otherElement, url);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name FRAME'));
  },
  ...
  testSetIframeSrc() {
    ...
    const otherElement = document.createElement('IMAGE');
    const ex = assertThrows(() => {
      safe.setIframeSrc(/** @type {!HTMLIFrameElement} */ otherElement, url);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name IFRAME'));
  },
  ...
  testSetIframeSrcdoc() {
    ...
    const otherElement = document.createElement('IMAGE');
    const ex = assertThrows(() => {
      safe.setIframeSrcdoc(/** @type {!HTMLIFrameElement} */ otherElement, html);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name IFRAME'));
  },
  ...
  testSetObjectData() {
    ...
    const otherElement = document.createElement('IMAGE');
    const ex = assertThrows(() => {
      safe.setObjectData(/** @type {!HTMLObjectElement} */ otherElement, url);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name OBJECT'));
  },
  ...
  testSetScriptSrc() {
    ...
    const otherElement = document.createElement('IMAGE');
    const ex = assertThrows(() => {
      safe.setScriptSrc(/** @type {!HTMLScriptElement} */ otherElement, url);
    });
    assert(googString.contains(ex.message, 'Argument is not an HTML Element with tag name SCRIPT'));
  },
  ...
  testSetScriptContent() {
    ...
    try {
      assertEquals(SafeScript.unwrap(content), mockScriptElement.textContent.toString());
  ...
  testSetScriptContentWithSpecialCharacters() {
    ...
    const content = SafeScript.fromConstant(
      Const.from(`
  ...
  testOpenInWindow() {
    mockWindowOpen = /** @type {?} */ googTesting.createMethodMock(window, 'open');
    const fakeWindow = {};

    mockWindowOpen('about:invalid#zClosurez', 'name', 'specs').$returns(fakeWindow);
    mockWindowOpen.$replay();
    let retVal = withAssertionFailure(() =>
      safe.openInWindow('javascript:evil();', window, Const.from('name'), 'specs')
    );
    mockWindowOpen.$verify();
    assertEquals('openInWindow should return the created window', fakeWindow, retVal);
    ...
    mockWindowOpen.$replay();
    retVal = safe.openInWindow('about:blank', window, Const.from('name'), 'specs');
    mockWindowOpen.$verify();
    assertEquals('openInWindow should return the created window', fakeWindow, retVal);
  },
  ...
  testParseFromStringHtml() {
    ...
    const node = safe.parseFromStringHtml(new DOMParser(), html).body.firstChild;
  ...
  testParseFromString() {
    const html = SafeHtml.create('a', { class: 'b' }, 'c');
    const node = safe.parseFromString(new DOMParser(), html, 'application/xml').firstChild;
    assertEquals('a', node.tagName);
  ...
  testCreateImageFromBlob() {
    ...
    const mockCreateObject = /** @type {?} */ googTesting.createMethodMock(
      window.URL,
      'createObjectURL'
    );
    const mockRevokeObject = /** @type {?} */ googTesting.createMethodMock(
      window.URL,
      'revokeObjectURL'
    );
  ...
  testCreateContextualFragment() {
    const html = SafeHtml.create('A', { class: 'b' }, 'c');
    const node = safe.createContextualFragment(/** @type {!Range} */ document.createRange(), html)
      .childNodes[0];
    assertEquals('A', node.tagName);
    assertEquals('b', node.className);
    assertEquals('c', node.textContent);
