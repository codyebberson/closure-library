...
goog.dom.annotate.annotateTermsInNode_ = (
  ...
  if (node.nodeType == goog.dom.NodeType.TEXT) {
    ...
    if (html != null) {
      ...
      // children.
      var tempNode = goog.dom.getDomHelper(node).createElement(goog.dom.TagName.SPAN);
    ...
    node.hasChildNodes() &&
    !goog.dom.annotate.NODES_TO_SKIP_[/** @type {!Element} */ (node).tagName]
  ) {
    var classes = /** @type {!Element} */ (node).className.split(/\s+/);
    var skip = goog.array.some(classes, (className) =>
      goog.array.contains(classesToSkip, className)
    );
 ...
 */
goog.dom.annotate.annotateText = (text, terms, annotateFn, opt_ignoreCase) => {
  if (opt_ignoreCase) {
    terms = goog.dom.annotate.lowercaseTerms_(terms);
  }
 ...
 * @private
 */
goog.dom.annotate.helpAnnotateText_ = (text, terms, annotateFn, ignoreCase) => {
  var hit = false;
      ...
      while (pos < textLen) {
        ...
        if (hitPos == -1) {
          ...
          if (
            !matchWholeWordOnly ||
            ((prevCharPos < 0 ||
              goog.dom.annotate.NONWORD_RE_.test(textToSearch.charAt(prevCharPos))) &&
              (nextCharPos >= textLen ||
                goog.dom.annotate.NONWORD_RE_.test(textToSearch.charAt(nextCharPos))))
          ) {
            hits.push(hitPos);
            hit = true;
          }
  ...
  if (hit) {
    ...
    while (true) {
      ...
      html.push(annotateFn(goog.asserts.assertNumber(termIndexOfNextHit), termHtml));
 ...
 * @private
 */
goog.dom.annotate.lowercaseTerms_ = (terms) => {
  var lowercaseTerms = [];
