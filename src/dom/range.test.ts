...
function removeHelper(testNumber, range, outer, expectedChildCount, expectedContent) {
  range.removeContents();
  assertTrue(`${testNumber}: Removed range should now be collapsed`, range.isCollapsed());
  assertEquals(`${testNumber}: Removed range content should be ""`, '', range.getText());
  assertEquals(
    `${testNumber}: Outer div should contain correct text`,
    expectedContent,
    outer.innerHTML.toLowerCase()
  );
  assertEquals(
    `${testNumber}: Outer div should have ${expectedChildCount}` + ' children now',
    expectedChildCount,
    outer.childNodes.length
  );
  assertNotNull(`${testNumber}: Empty node should still exist`, dom.getElement('empty'));
}
...
function assertSurroundDoesntChangeSelectionWithOffsets(offset1, offset2, expectedHtml) {
  ...
  const rangeToSelect = Range.createFromNodes(div.firstChild, 2, div.firstChild, 5);
  rangeToSelect.select();

  const rangeToSurround = Range.createFromNodes(div.firstChild, offset1, div.firstChild, offset2);
  rangeToSurround.surroundWithNodes(dom.createDom(TagName.SPAN), dom.createDom(TagName.SPAN));

  // Make sure that the selection didn't change.
  assertHTMLEquals(
    'Selection must not change when contents are surrounded.',
    expectedHtml,
...
function assertForward(string, startNode, startOffset, endNode, endOffset) {
  ...
  assertFalse(string, Range.isReversed(startNode, startOffset, endNode, endOffset));
  assertTrue(string, Range.isReversed(endNode, endOffset, startNode, startOffset));
  assertEquals(`Contents should be unaffected after: ${string}`, root.innerHTML, originalInnerHtml);
}
...
testSuite({
  ...
  testTableRange() {
    ...
    assertEquals(
      'Selection should have correct html fragment',
      '1</td><td>2',
  ...
  testCreateFromNodes() {
    ...
    assertEquals('Start node should be selected at anchor endpoint', start, range.getAnchorNode());
    assertEquals('Selection should be anchored at offset 2', 2, range.getAnchorOffset());

    const div = dom.getElement('test2');
    assertEquals('DIV node should be selected at end endpoint', div, range.getEndNode());
    assertEquals('Selection should end at offset 1', 1, range.getEndOffset());
    assertEquals('DIV node should be selected at focus endpoint', div, range.getFocusNode());
    assertEquals('Selection should be focused at offset 1', 1, range.getFocusOffset());

    assertTrue('Text content should be "xt\\s*abc"', /xt\s*abc/.test(range.getText()));
  ...
  testCreateControlRange() {
    ...
    assertNotNull('Control range object can be created from browser range', range);
    assertEquals('Created range is a control range', RangeType.CONTROL, range.getType());
  },

  testTextNode() {
    const range = Range.createFromNodeContents(dom.getElement('test1').firstChild);

    assertEquals('Created range is a text range', RangeType.TEXT, range.getType());
    assertEquals(
  ...
  testDiv() {
    ...
    assertEquals('Container should be DIV', dom.getElement('test2'), range.getContainer());
  ...
  testEmptyNode() {
    ...
    assertEquals(
      'DIV be selected at start endpoint',
      dom.getElement('empty'),
      range.getStartNode()
    );
    assertEquals('Selection should start at offset 0', 0, range.getStartOffset());

    assertEquals(
      'DIV should be selected at end endpoint',
      dom.getElement('empty'),
      range.getEndNode()
    );
    assertEquals('Selection should end at offset 0', 0, range.getEndOffset());

    assertEquals('Container should be DIV', dom.getElement('empty'), range.getContainer());
  ...
  testReversedContents() {
    ...
    assertEquals('Post collapse start offset should be 4', 4, range.getStartOffset());

    range2.collapse(false);
    assertTrue('Range 2 is collapsed', range2.isCollapsed());
    assertFalse('Collapsed range 2 is not reversed', range2.isReversed());
    assertEquals('Post collapse start offset 2 should be 0', 0, range2.getStartOffset());
  },
  ...
  testRemoveContents() {
    ...
    assertEquals('Inner div should be empty', 0, outer.firstChild.childNodes.length);
  },
  ...
  testRemovePartialContents() {
    ...
    try {
      let range = Range.createFromNodes(outer.firstChild, 2, outer.firstChild, 4);
  ...
  testSurroundContents() {
    ...
    assertEquals('Outer element should contain new element', outer, output.parentNode);
    assertFalse('New element should have no id', !!output.id);
    assertEquals('New element should be red', 'red', output.style.color);
    assertEquals('Outer element should have three children', 3, outer.childNodes.length);
    assertEquals('New element should have three children', 3, output.childNodes.length);

    // TODO(robbyw): Ensure the range stays in a reasonable state.
  },

  testSurroundWithNodesDoesntChangeSelection1() {
    assertSurroundDoesntChangeSelectionWithOffsets(3, 4, 'o<span></span>b<span></span>a');
  },
  ...
  testInsertNode() {
    ...
    assertEquals('Element should have correct innerHTML', 'ABCD', outer.innerHTML);

    dom.setTextContent(outer, '12');
    range = Range.createFromNodes(outer.firstChild, 0, outer.firstChild, 1);
    const br = range.insertNode(dom.createDom(TagName.BR), false);
    assertEquals(
      'New element should have correct innerHTML',
      '1<br>2',
      outer.innerHTML.toLowerCase()
    );
  ...
  testSurroundWithNodes() {
    ...
    const range = Range.createFromNodes(outer.firstChild, 1, outer.firstChild, 2);

    range.surroundWithNodes(dom.createTextNode('B'), dom.createTextNode('D'));

    assertEquals('New element should have correct innerHTML', 'ABCDE', outer.innerHTML);
  },
  ...
  testIsRangeInDocument() {
    ...
    assertEquals('Should get correct start element', 'ABC', range.getStartNode().nodeValue);
  ...
  testRemovedNode() {
    ...
    assertTrue('The other browsers will just have an empty range.', newRange.isCollapsed());
  },

  testReversedRange() {
    if (userAgent.EDGE_OR_IE) return; // IE doesn't make this distinction.

    Range.createFromNodes(dom.getElement('test2'), 0, dom.getElement('test1'), 0).select();
  ...
  testIsReversed() {
    ...
    assertFalse('Same element position gives false', Range.isReversed(root, 0, root, 0));
    assertFalse('Same text position gives false', Range.isReversed(text1, 0, text2, 0));
    assertForward('Element offsets should compare against each other', root, 0, root, 2);
    assertForward('Text node offsets should compare against each other', text1, 0, text2, 2);
    ...
    assertForward('Text nodes should compare to earlier elements', br, 0, text2, 0);
    assertForward('Parent is before element child', root, 0, br, 0);
    assertForward('Parent is before text child', root, 0, text1, 0);
    assertFalse('Equivalent position gives false', Range.isReversed(root, 0, text1, 0));
    assertFalse('Equivalent position gives false', Range.isReversed(root, 1, br, 0));
  ...
  testRangeBeforeBreak() {
    ...
    if (userAgent.IE && !userAgent.isDocumentModeOrHigher(9)) {
      ...
      assertFalse('Should not contain partial <br>', range.containsNode(br, true));
    }
