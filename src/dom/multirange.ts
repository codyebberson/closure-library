 ...
 */
goog.dom.MultiRange.createFromBrowserSelection = (selection) => {
 ...
 * @return {!goog.dom.MultiRange} A range wrapper object.
 */
goog.dom.MultiRange.createFromBrowserRanges = (browserRanges) => {
  var range = new goog.dom.MultiRange();
 ...
 * @return {!goog.dom.MultiRange} A range wrapper object.
 */
goog.dom.MultiRange.createFromTextRanges = (textRanges) => {
  var range = new goog.dom.MultiRange();
  range.ranges_ = textRanges;
  range.browserRanges_ = textRanges.map((range) => range.getBrowserRangeObject());
  return range;
};
...
/** @override */
goog.dom.MultiRange.prototype.getType = () => goog.dom.RangeType.MULTI;
...
/** @override */
goog.dom.MultiRange.prototype.setBrowserRangeObject = (nativeRange) => {
...
goog.dom.MultiRange.prototype.isRangeInDocument = function () {
  return this.getTextRanges().every((range) => range.isRangeInDocument());
};
...
goog.dom.MultiRange.prototype.removeContents = function () {
  this.getTextRanges().forEach((range) => {
...
goog.dom.DomSavedMultiRange_ = function (range) {
   ...
   * @private
   */
  this.savedRanges_ = range.getTextRanges().map((range) => range.saveUsingDom());
};
...
goog.dom.DomSavedMultiRange_.prototype.restoreInternal = function () {
  var ranges = this.savedRanges_.map((savedRange) => savedRange.restore());
...
goog.dom.DomSavedMultiRange_.prototype.disposeInternal = function () {
  ...
  this.savedRanges_.forEach((savedRange) => {
    savedRange.dispose();
  });
...
goog.dom.MultiRangeIterator = function (range) {
  ...
  if (range) {
    this.iterators_ = range.getSortedRanges().map((r) => goog.iter.toIterator(r));
  }
