 ...
 * @private
 */
goog.exportPath_ = (name, object, overwriteImplicit, objectToExportTo) => {
 ...
 * @private
 */
goog.constructNamespace_ = (name, object, overwriteImplicit) => {
  if (!COMPILED) {
 ...
 * @private
 */
goog.getScriptNonce_ = (opt_window) => {
 ...
 */
goog.module.get = (name) => goog.module.getInternal_(name);
 ...
 *     initialized.
 */
goog.isInModuleLoader_ = () => goog.isInGoogModuleLoader_() || goog.isInEs6ModuleLoader_();
 ...
 * @suppress {missingProvide}
 */
goog.declareModuleId = (namespace) => {
  if (!COMPILED) {
 ...
 */
goog.getObjectByName = (name, opt_obj) => {
  var parts = name.split('.');
 ...
 */
goog.addDependency = (relPath, provides, requires, opt_loadFlags) => {
 ...
 */
goog.isDateLike = (val) => goog.isObject(val) && typeof val.getFullYear == 'function';
 ...
 */
goog.hasUid = (obj) => !!obj[goog.UID_PROPERTY_];
 ...
 * @private
 */
goog.bindNative_ = (fn, selfObj, var_args) => fn.call.apply(fn.bind, arguments);
 ...
 * @private
 */
goog.bindJs_ = (fn, selfObj, var_args) => {
  if (!fn) {
    throw new Error();
  }
...
goog.getCssName = (className, opt_modifier) => {
  ...
  if (String(className).charAt(0) == '.') {
    throw new Error(
      'className passed in goog.getCssName must not start with ".".' + ' You passed: ' + className
    );
  }

  var getMapping = (cssName) => goog.cssNameMapping_[cssName] || cssName;

  var renameByParts = (cssName) => {
 ...
 */
goog.setCssNameMapping = (mapping, opt_style) => {
 ...
 */
goog.getMsg = (str, opt_values, opt_options) => {
  ...
  if (opt_values) {
    str = str.replace(/\{\$([^}]+)}/g, (match, key) =>
      opt_values != null && key in opt_values ? opt_values[key] : match
    );
  }
  return str;
};
 ...
 *     is goog.global.
 */
goog.exportSymbol = (publicPath, object, objectToExportTo) => {
...
goog.inherits = (childCtor, parentCtor) => {
   ...
   */
  childCtor.base = (me, methodName, var_args) => {
 ...
 * @deprecated Use ECMAScript class syntax instead.
 */
goog.defineClass = (superClass, def) => {
 ...
 * @private
 */
goog.defineClass.createSealingConstructor_ = (ctr, superClass) => {
 ...
 * @private
 */
goog.defineClass.applyProperties_ = (target, source) => {
 ...
 */
goog.createTrustedTypesPolicy = (name) => {
  var policy = null;
...
if (!COMPILED && goog.DEPENDENCIES_ENABLED) {
   ...
   * @private
   */
  goog.protectScriptTag_ = (str) => str.replace(/<\/(SCRIPT)/gi, '\\x3c/$1');
  ...
  goog.DebugLoader_.prototype.load_ = function (namespace) {
    if (!this.getPathFromDeps_(namespace)) {
      ...
      /** @param {string} namespace */
      var visit = (namespace) => {
        var path = this.getPathFromDeps_(namespace);

        if (!path) {
          throw new Error('Bad dependency path or symbol: ' + namespace);
        }

        if (this.written_[path]) {
          return;
        }

        this.written_[path] = true;

        var dep = this.dependencies_[path];
  ...
  goog.DebugLoader_.prototype.loadDeps_ = function () {
    ...
    while (this.depsToLoad_.length && !paused) {
      (() => {
        ...
        var dep = this.depsToLoad_.shift();
        ...
        var controller = {
          ...
          pending: () => {
            ...
            for (var i = 0; i < this.loadingDeps_.length; i++) {
              pending.push(this.loadingDeps_[i]);
            }
            return pending;
          },
          /**
          ...
          registerEs6ModuleExports: (path, exports, opt_closureNamespace) => {
          ...
          registerGoogModuleExports: (moduleId, exports) => {
          ...
          defer: (callback) => {
            ...
            this.defer_(dep, callback);
          },
          areDepsLoaded: () => this.areDepsLoaded_(dep.requires),
        };
   ...
   */
  goog.LoadController.prototype.pause = () => {};
   ...
   * fire.
   */
  goog.LoadController.prototype.loaded = () => {};
   ...
   */
  goog.LoadController.prototype.pending = () => {};
   ...
   */
  goog.LoadController.prototype.setModuleState = (type) => {};
   ...
   * @param {!Function} callback
   */
  goog.LoadController.prototype.defer = (callback) => {};

  /**
   * @return {boolean}
   */
  goog.LoadController.prototype.areDepsLoaded = () => {};
   ...
   * @private
   */
  goog.Dependency.registerCallback_ = (callback) => {
   ...
   */
  goog.Dependency.callback_ = (key, var_args) => {
    if (key in goog.Dependency.callbackMap_) {
      ...
      var errorMessage =
        'Callback key ' + key + ' does not exist (was base.js loaded more than once?).';
  ...
  goog.Dependency.prototype.load = function (controller) {
    ...
    if (!goog.ENABLE_CHROME_APP_SAFE_SCRIPT_LOADING && goog.isDocumentLoading_()) {
      ...
      doc.write(
        goog.TRUSTED_TYPES_POLICY_ ? goog.TRUSTED_TYPES_POLICY_.createHTML(script) : script
      );
    } else {
  ...
  goog.Es6ModuleDependency.prototype.load = function (controller) {
    ...
    function write(src, contents) {
      ...
      if (contents) {
        var script =
          '<script type="module" crossorigin' + nonceAttr + '>' + contents + '</' + 'script>';
        doc.write(
          goog.TRUSTED_TYPES_POLICY_ ? goog.TRUSTED_TYPES_POLICY_.createHTML(script) : script
        );
      } else {
        var script =
          '<script type="module" crossorigin src="' + src + '"' + nonceAttr + '></' + 'script>';
        doc.write(
          goog.TRUSTED_TYPES_POLICY_ ? goog.TRUSTED_TYPES_POLICY_.createHTML(script) : script
        );
      }
    ...
    //    error in the module).
    var beforeKey = goog.Dependency.registerCallback_(() => {
    ...
    var registerKey = goog.Dependency.registerCallback_((exports) => {
      goog.Dependency.unregisterCallback_(registerKey);
      controller.registerEs6ModuleExports(this.path, exports, goog.moduleLoaderState_.moduleName);
    });
    create(
      undefined,
      'import * as m from "' + this.path + '"; goog.Dependency.callback_("' + registerKey + '", m)'
    );

    var afterKey = goog.Dependency.registerCallback_(() => {
  ...
  goog.TransformedDependency.prototype.load = function (controller) {
    ...
    function load() {
      ...
      if (isEs6) {
        ...
        // right now.
        goog.global['$jscomp']['require']['ensure']([dep.getPathName()], () => {
    ...
    function fetchInOwnScriptThenLoad() {
      ...
      var key = goog.Dependency.registerCallback_(() => {
        goog.Dependency.unregisterCallback_(key);
        load();
      });
    ...
    var needsAsyncLoading =
      goog.Dependency.defer_ && (anythingElsePending || goog.isDocumentLoading_());
    ...
    var isInternetExplorerOrEdge =
      goog.inHtmlDocument_() && ('ActiveXObject' in goog.global || goog.isEdge_());
   ...
   * @abstract
   */
  goog.TransformedDependency.prototype.transform = (contents) => {};
   ...
   * @return {string}
   */
  goog.PreTranspiledEs6ModuleDependency.prototype.transform = (contents) => contents;
   ...
   */
  goog.DependencyFactory.prototype.createDependency = (
    path,
    relativePath,
    provides,
    requires,
   ...
   * @param {!goog.DependencyFactory} factory
   */
  goog.setDependencyFactory = (factory) => {
   ...
   */
  goog.bootstrap = (namespaces, callback) => {
...
if (!COMPILED) {
  ...
  try {
    isChrome87 =
      eval(goog.global.trustedTypes.emptyScript) !== goog.global.trustedTypes.emptyScript;
  } catch (err) {}
  ...
  goog.CLOSURE_EVAL_PREFILTER_ =
    ...
    (goog.global.trustedTypes &&
      isChrome87 &&
      goog.createTrustedTypesPolicy('goog#base#devonly#eval')) || { createScript: goog.identity_ };
}
