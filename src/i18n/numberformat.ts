...
goog.scope(() => {
   ...
   */
  goog.i18n.NumberFormat.isEnforceAsciiDigits = () => goog.i18n.NumberFormat.enforceAsciiDigits_;
  ...
  goog.i18n.NumberFormat.prototype.getNumberFormatSymbols_ = function () {
    return (
      this.overrideNumberFormatSymbols_ ||
  ...
  goog.i18n.NumberFormat.prototype.parseNumber_ = function (text, pos) {
    ...
    for (; pos[0] < text.length; pos[0]++) {
      ...
      if (digit >= 0 && digit <= 9) {
        ...
        ('\u00a0' != grouping.charAt(0) ||
          (pos[0] + 1 < text.length && this.getDigit_(text.charAt(pos[0] + 1)) >= 0))
      ) {
    ...
    return Number.parseFloat(normalizedText) / scale;
  };
  ...
  goog.i18n.NumberFormat.prototype.SetUpIntlFormatter_ = function (inputPattern) {
    ...
    switch (inputPattern) {
      ...
      case goog.i18n.NumberFormat.Format.COMPACT_LONG:
        ...
        break;
      default:
        throw new Error('Unsupported ECMAScript NumberFormat custom pattern = ' + this.pattern_);
    }
  ...
  goog.i18n.NumberFormat.prototype.format = function (number) {
    ...
    const parts = [];
    const baseFormattingNumber =
      this.baseFormattingNumber_ === null ? number : this.baseFormattingNumber_;
  ...
  goog.i18n.NumberFormat.prototype.formatUsingNativeMode_ = function (number) {
    ...
    if (
      ...
      const parts = resultParts.map((element) =>
        element.type === 'percentSign' ? percentReplacement : element.value
      );
      return parts.join('');
    }
    ...
    if (this.showTrailingZeros_) {
      ...
      resultParts.forEach((element) =>
        element.type === 'integer' && element.value !== '0' ? (intSize += element.value.length) : 0
      );
    ...
    if (this.baseFormattingNumber_) {
      ...
      if (number != 0) {
        ...
        if (Math.abs(scaledForFraction) > 1.0) {
          ...
          if (Math.round(scaledForFraction) != scaledForFraction) {
            fractionDigits += 1; // Avoid an extra decimal place if exact divisor.
          }
        }
      ...
      const compactAdditions = scaledResult
        .filter((entry) => entry.type === 'compact' || entry.type === 'literal')
   ...
   * @private
   */
  goog.i18n.NumberFormat.prototype.formatNumberGroupingRepeatingDigitsParts_ = function (
    parts,
    zeroCode,
    intPart,
    groupingArray,
   ...
   * @private
   */
  goog.i18n.NumberFormat.prototype.formatNumberGroupingNonRepeatingDigitsParts_ = function (
    parts,
    zeroCode,
    intPart,
    groupingArray
    ...
    for (
      ...
      for (
        let rightDigitIndex = 0;
        rightDigitIndex < currentGroupSize && digitLenLeft - rightDigitIndex - 1 >= 0;
        rightDigitIndex++
      ) {
        rightToLeftParts.push(
          String.fromCharCode(
            zeroCode + Number(intPart.charAt(digitLenLeft - rightDigitIndex - 1)) * 1
          )
        );
      }
  ...
  goog.i18n.NumberFormat.prototype.subformatFixed_ = function (number, minIntDigits, parts) {
    ...
    if (fracPartSplit.length == 2) {
      ...
      const floatFrac = Number.parseFloat(fracPartSplit[0]);
      ...
      const exp = Number.parseInt(fracPartSplit[1], 10);
   ...
   * @private
   */
  goog.i18n.NumberFormat.prototype.getMantissa_ = (value, exponent) =>
    goog.i18n.NumberFormat.decimalShift_(value, -exponent);
      ...
      if (inQuote) {
        ...
        switch (ch) {
          ...
          case goog.i18n.NumberFormat.PATTERN_CURRENCY_SIGN_:
            if (
              pos[0] + 1 < len &&
              pattern.charAt(pos[0] + 1) == goog.i18n.NumberFormat.PATTERN_CURRENCY_SIGN_
            ) {
              pos[0]++;
    ...
    for (let loop = true; pos[0] < len && loop; pos[0]++) {
      ...
      switch (ch) {
        ...
        case goog.i18n.NumberFormat.PATTERN_EXPONENT_:
          ...
          while (
            pos[0] + 1 < len &&
            pattern.charAt(pos[0] + 1) == goog.i18n.NumberFormat.PATTERN_ZERO_DIGIT_
          ) {
            pos[0]++;
  ...
  goog.i18n.NumberFormat.prototype.getUnitFor_ = function (base, plurality) {
    ...
    if (base < 3) {
      ...
      let neg_prefix = '';
      let neg_suffix = '';
      const index_of_neg_part = pattern.indexOf(';');
  ...
  goog.i18n.NumberFormat.prototype.getUnitAfterRounding_ = function (
    ...
    const formattingAttempt = goog.i18n.NumberFormat.decimalShift_(
      formattingNumber,
      -initialDivisor
    );
   ...
   * @private
   */
  goog.i18n.NumberFormat.prototype.intLog10_ = (number) => {
    // Handle infinity.
    if (!isFinite(number)) {
   ...
   * @private
   */
  goog.i18n.NumberFormat.decimalShift_ = (number, digitCount) => {
    ...
    const magnitude = Number.parseInt(numParts[1] || 0, 10) + digitCount;
    return Number.parseFloat(numParts[0] + 'e' + magnitude);
  };
   ...
   * @private
   */
  goog.i18n.NumberFormat.decimalRound_ = (number, decimalCount) => {
    goog.asserts.assert(
      decimalCount % 1 == 0,
      'Cannot round to fractional digits "%s".',
      decimalCount
    );
   ...
   * @private
   */
  goog.i18n.NumberFormat.prototype.pluralForm_ = (quantity) => {
    /* TODO: Implement */
    return 'other';
  };
