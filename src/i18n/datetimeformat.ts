...
goog.scope(() => {
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.getHours_ = (date) =>
    date.getHours ? /** @type {?} */ (date).getHours() : 0;
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.getMinutes_ = (date) =>
    date.getMinutes ? /** @type {?} */ (date).getMinutes() : 0;
        ...
        if (m) {
          ...
          if (i == goog.i18n.DateTimeFormat.PartTypes_.QUOTED_STRING) {
            if (part == "''") {
              ...
              part = part.substring(1, m[1] == "'" ? part.length - 1 : part.length); // strip quotes
              part = part.replace(/\'\'/g, "'");
            }
          }
   ...
   */
  goog.i18n.DateTimeFormat.setEnforceAsciiDigits = (enforceAsciiDigits) => {
   ...
   */
  goog.i18n.DateTimeFormat.isEnforceAsciiDigits = () =>
   ...
   */
  goog.i18n.DateTimeFormat.localizeNumbers = (input, opt_dateTimeSymbols) => {
    input = String(input);
  ...
  goog.i18n.DateTimeFormat.prototype.formatFractionalSeconds_ = function (count, date) {
    ...
    const value = /** @type {!Date|!goog.date.DateTime} */ (date).getMilliseconds() / 1000;
  ...
  goog.i18n.DateTimeFormat.prototype.formatFlexibleDayPeriods_ = function (count, date) {
    ...
    if (dayPeriods) {
      ...
      for (let index = 0; index < keys.length; index++) {
        const testPeriod = dayPeriods[keys[index]];
        ...
        if (testPeriod.before > testPeriod.from) {
          ...
          // Two tests needed
          if (
            (fmtTime >= testPeriod.from && fmtTime < '24:00') ||
  ...
  goog.i18n.DateTimeFormat.prototype.formatSeconds_ = function (count, date) {
    goog.i18n.DateTimeFormat.validateDateHasTime_(date);
    return this.localizeNumbers_(
      goog.string.padNumber(/** @type {!goog.date.DateTime} */ (date).getSeconds(), count)
    );
  };
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.prototype.formatTimeZone_ = (count, date, opt_timeZone) => {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.prototype.formatTimeZoneId_ = (date, opt_timeZone) => {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.prototype.formatTimeZoneLocationId_ = (count, date, opt_timeZone) => {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  goog.i18n.DateTimeFormat.prototype.removeYearFormatFromPattern_ = (patternStr) => {
