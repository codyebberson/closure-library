...
testSuite({
  testClosureStylePlaceholdersForbidden() {
    const actualError = assertThrows(() =>
      declareIcuTemplate('{$GREETING}, {NAME}.', {
  ...
  testUnknownOptionName() {
    ...
    assertContains('unknown option name: "bad_option_name"', actualError.toString());
  },
  ...
  testExampleForUnknownPlaceholderName() {
    ...
    assertContains('example: unknown placeholder: "GREETING"', actualError.toString());
  },
  ...
  testOriginalCodeForUnknownPlaceholderName() {
    ...
    assertContains('original_code: unknown placeholder: "GREETING"', actualError.toString());
  },
