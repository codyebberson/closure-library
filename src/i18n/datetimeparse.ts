...
goog.scope(() => {
  ...
  goog.i18n.DateTimeParse = function (pattern, opt_dateTimeSymbols) {
    goog.asserts.assert(
      opt_dateTimeSymbols !== undefined || goog.i18n.DateTimeSymbols !== undefined,
   ...
   * @private
   */
  goog.i18n.DateTimeParse.prototype.getNextCharCount_ = (pattern, start) => {
   ...
   * @private
   */
  goog.i18n.DateTimeParse.prototype.isNumericField_ = (ch, count) => {
    if (count <= 0) {
      return false;
    }
  ...
  goog.i18n.DateTimeParse.prototype.assertPatternSupportsPredictive_ = function () {
    for (let i = 0; i < this.patternParts_.length; i++) {
      ...
      if (
        part.count > 0 &&
        (goog.i18n.DateTimeParse.PREDICTIVE_FORMAT_CHARS_.indexOf(part.text.charAt(0)) < 0 ||
          part.count > 2 ||
          part.abutStart)
      ) {
        throw new Error(
          `'predictive' parsing is not supported for symbol ` + `'${part.text.charAt(0)}'.`
        );
      }
    }
  };
  ...
  goog.i18n.DateTimeParse.prototype.subParseAbut_ = function (text, pos, abutStart, cal) {
    ...
    for (i = abutStart; i < this.patternParts_.length; i++) {
      ...
      const subParsed = this.subParse_(text, pos, part, count, cal, /* predictive= */ false);
    ...
    switch (part.text.charAt(0)) {
      ...
      case 'b': // Day periods of 'noon', 'midnight', 'am', or 'pm'
        ...
        const foundPeriod = this.subParseString_(
          ...
          pos,
          periodsData,
          (value) => (cal.dayPeriodIndex = value),
          predictive
        );
      ...
      case 'a': // AM_PM
        const success = this.subParseString_(
          text,
          pos,
          [this.dateTimeSymbols_.AMPMS],
          (value) => (cal.ampm = value),
          predictive
        );
      ...
      case 'Q': // QUARTER
        ...
        return this.subParseString_(text, pos, quarters, (value) => {
          cal.month = value * 3; // First month of quarter.
          cal.day = 1;
        });
      case 'd': // DATE
        this.subParseInt_(text, pos, part, digitCount, (value) => {
          cal.day = value;
        });
        return true;
  ...
  goog.i18n.DateTimeParse.prototype.subParseMonth_ = function (text, pos, digitCount, part, cal) {
    ...
    // Handle both short and long forms. Try count == 4 first.
    const months = [
      this.dateTimeSymbols_.MONTHS,
      this.dateTimeSymbols_.STANDALONEMONTHS,
      this.dateTimeSymbols_.SHORTMONTHS,
      this.dateTimeSymbols_.STANDALONESHORTMONTHS,
    ];
    return this.subParseString_(text, pos, months, (value) => {
      cal.month = value;
    });
  };
  ...
  goog.i18n.DateTimeParse.prototype.subParseFractionalSeconds_ = function (
    ...
    cal.milliseconds =
      len < 3 ? value * Math.pow(10, 3 - len) : Math.round(value / Math.pow(10, len - 3));
    return true;
  };
  ...
  goog.i18n.DateTimeParse.prototype.subParseTimeZoneInGMT_ = function (text, pos, cal) {
    ...
    const start = pos[0];
    let value = this.parseInt_(text, pos, /* digitCount= */ 0, /* allowSigned= */ true);
    if (value === null) {
      return false;
    }
   ...
   * @private
   */
  goog.i18n.DateTimeParse.prototype.skipSpace_ = (text, pos) => {
  ...
  goog.i18n.DateTimeParse.prototype.parseInt_ = function (text, pos, maxChars, allowSigned) {
    ...
    if (this.dateTimeSymbols_.ZERODIGIT) {
      ...
      for (let i = pos[0]; i < text.length; i++) {
        ...
        parts.push(0 <= c && c <= 9 ? String.fromCharCode(c + 0x30) : text.charAt(i));
      }
      text = parts.join('');
    } else {
   ...
   * @private
   */
  goog.i18n.DateTimeParse.prototype.matchString_ = (text, pos, data, predictive) => {
  ...
  goog.i18n.DateTimeParse.MyDate_.prototype.setTwoDigitYear_ = function (year) {
    ...
    year +=
      Math.floor(defaultCenturyStartYear / 100) * 100 + (year < ambiguousTwoDigitYear ? 100 : 0);
  ...
  goog.i18n.DateTimeParse.MyDate_.prototype.calcDate_ = function (date, validation) {
      ...
      if (this.hours < 12) {
        if (this.ampm != undefined && this.ampm > 0) {
          ...
          if (this.dayPeriodName !== undefined) {
            ...
            if (
              pmPeriods.includes(this.dayPeriodName) ||
              (['night1', 'night2'].includes(this.dayPeriodName) && this.hours >= 6)
            ) {
    ...
    if (this.ambiguousYear) {
      ...
      defaultCenturyStart.setFullYear(
        defaultCenturyStart.getFullYear() - goog.i18n.DateTimeParse.ambiguousYearCenturyStart
      );
