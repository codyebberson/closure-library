...
function spanWrap(formatter, html, dirReset = undefined) {
  return SafeHtml.unwrap(formatter.spanWrapSafeHtml(testing.newSafeHtmlForTest(html), dirReset));
}
...
function spanWrapWithKnownDir(formatter, dir, html) {
  return SafeHtml.unwrap(
    formatter.spanWrapSafeHtmlWithKnownDir(dir, testing.newSafeHtmlForTest(html))
  );
}
...
testSuite({
  ...
  testEstimateDirection() {
    ...
    assertEquals(RTL, ltrFmt.estimateDirection(`<some sort of tag/>${he} &amp;`, true));
  },

  testDirAttrValue() {
    assertEquals('overall dir is RTL, context dir is LTR', 'rtl', ltrFmt.dirAttrValue(he, true));
    assertEquals('overall dir and context dir are RTL', 'rtl', rtlFmt.dirAttrValue(he, true));
    assertEquals('overall dir is LTR, context dir is RTL', 'ltr', rtlFmt.dirAttrValue(en, true));
    assertEquals('overall dir and context dir are LTR', 'ltr', ltrFmt.dirAttrValue(en, true));
  ...
  testDirAttr() {
    ...
    assertEquals('overall dir matches context dir (RTL)', '', rtlFmt.dirAttr(he, true));
    ...
    assertEquals('overall dir matches context dir (LTR)', '', ltrFmt.dirAttr(en, true));
  ...
  testKnownDirAttr() {
    ...
    assertEquals('overall dir matches context dir (RTL)', '', rtlFmt.knownDirAttr(RTL));
    assertEquals(
      'overall dir (LTR) doesnt match context dir (RTL)',
      'dir="ltr"',
      rtlFmt.knownDirAttr(LTR)
    );
    assertEquals('overall dir matches context dir (LTR)', '', ltrFmt.knownDirAttr(LTR));
  },

  testSpanWrap() {
    // alwaysSpan is false and opt_isHtml is true, unless specified otherwise.
    assertEquals(
      'overall dir matches context dir (LTR), no dirReset',
      en,
      spanWrap(ltrFmt, en, false)
    );
    assertEquals('overall dir matches context dir (LTR), dirReset', en, spanWrap(ltrFmt, en, true));
    assertEquals(
      'overall dir matches context dir (RTL), no dirReset',
      he,
      spanWrap(rtlFmt, he, false)
    );
    assertEquals('overall dir matches context dir (RTL), dirReset', he, spanWrap(rtlFmt, he, true));

    assertEquals(
      'overall dir (RTL) doesnt match context dir (LTR), ' + 'no dirReset',
      `<span dir="rtl">${he}</span>`,
      spanWrap(ltrFmt, he, false)
    );
    assertEquals(
    ...
    assertEquals(
      ...
      `<span dir="ltr">${en}</span>`,
      spanWrap(unkFmt, en, false)
    );
    assertEquals(
      'overall dir (RTL) doesnt match context dir (unknown), ' + 'dirReset',
      `<span dir="rtl">${he}</span>`,
      spanWrap(unkFmt, he, true)
    );
    assertEquals(
    ...
    assertEquals(
      'exit dir (but not overall dir) is opposite to context dir, ' + 'dirReset',
      longEn + he + html + LRM,
    ...
    assertEquals(
      ...
      `<span>${en}</span>`,
      spanWrap(ltrAlwaysSpanFmt, en, false)
    );
    assertEquals(
      'alwaysSpan, overall dir matches context dir (LTR), dirReset',
      `<span>${en}</span>`,
      spanWrap(ltrAlwaysSpanFmt, en, true)
    );
    assertEquals(
      'alwaysSpan, overall dir matches context dir (RTL), ' + 'no dirReset',
      `<span>${he}</span>`,
      spanWrap(rtlAlwaysSpanFmt, he, false)
    );
    assertEquals(
      'alwaysSpan, overall dir matches context dir (RTL), dirReset',
      `<span>${he}</span>`,
      spanWrap(rtlAlwaysSpanFmt, he, true)
    );

    assertEquals(
      'alwaysSpan, overall dir (RTL) doesnt match ' + 'context dir (LTR), no dirReset',
      `<span dir="rtl">${he}</span>`,
      spanWrap(ltrAlwaysSpanFmt, he, false)
    );
    assertEquals(
      'alwaysSpan, overall dir (RTL) doesnt match ' + 'context dir (LTR), dirReset',
      `<span dir="rtl">${he}</span>${LRM}`,
      spanWrap(ltrAlwaysSpanFmt, he, true)
    );
    assertEquals(
      'alwaysSpan, overall dir (neutral) doesnt match ' + 'context dir (LTR), dirReset',
      '<span></span>',
  ...
  testSpanWrapWithKnownDir() {
    assertEquals('known LTR in LTR context', en, spanWrapWithKnownDir(ltrFmt, LTR, en));
    assertEquals('unknown LTR in LTR context', en, spanWrapWithKnownDir(ltrFmt, null, en));
    assertEquals(
      'overall LTR but exit RTL in LTR context',
      he + LRM,
      spanWrapWithKnownDir(ltrFmt, LTR, he)
    );
    assertEquals(
      'known RTL in LTR context',
      `<span dir="rtl">${he}</span>${LRM}`,
      spanWrapWithKnownDir(ltrFmt, RTL, he)
    );
    assertEquals(
      'unknown RTL in LTR context',
      `<span dir="rtl">${he}</span>${LRM}`,
    ...
    assertEquals('known neutral in LTR context', '.', spanWrapWithKnownDir(ltrFmt, NEUTRAL, '.'));
    assertEquals('unknown neutral in LTR context', '.', spanWrapWithKnownDir(ltrFmt, null, '.'));
    assertEquals(
    ...
    assertEquals(
      'overall neutral but exit RTL in LTR context',
      he + LRM,
      spanWrapWithKnownDir(ltrFmt, NEUTRAL, he)
    );

    assertEquals('known RTL in RTL context', he, spanWrapWithKnownDir(rtlFmt, RTL, he));
    assertEquals('unknown RTL in RTL context', he, spanWrapWithKnownDir(rtlFmt, null, he));
    assertEquals(
      'overall RTL but exit LTR in RTL context',
      en + RLM,
      spanWrapWithKnownDir(rtlFmt, RTL, en)
    );
    assertEquals(
      'known LTR in RTL context',
      `<span dir="ltr">${en}</span>${RLM}`,
      spanWrapWithKnownDir(rtlFmt, LTR, en)
    );
    assertEquals(
      'unknown LTR in RTL context',
      `<span dir="ltr">${en}</span>${RLM}`,
    ...
    assertEquals('known neutral in RTL context', '.', spanWrapWithKnownDir(rtlFmt, NEUTRAL, '.'));
    assertEquals('unknown neutral in RTL context', '.', spanWrapWithKnownDir(rtlFmt, null, '.'));
    assertEquals(
      'overall neutral but exit LTR in LTR context',
      he,
  ...
  testUnicodeWrap() {
    ...
    assertEquals(
      'overall dir matches context dir (LTR), dirReset',
      en,
    ...
    assertEquals(
      'overall dir matches context dir (RTL), dirReset',
      he,
    ...
    assertEquals(
      'overall dir (RTL) doesnt match context dir (LTR), ' + 'no dirReset',
      RLE + he + PDF,
      ltrFmt.unicodeWrap(he, true, false)
    );
    assertEquals(
      'overall dir (RTL) doesnt match context dir (LTR), dirReset',
      RLE + he + PDF + LRM,
      ltrFmt.unicodeWrap(he, true, true)
    );
    assertEquals(
      'overall dir (LTR) doesnt match context dir (RTL), ' + 'no dirReset',
      LRE + en + PDF,
      rtlFmt.unicodeWrap(en, true, false)
    );
    assertEquals(
      'overall dir (LTR) doesnt match context dir (RTL), dirReset',
      LRE + en + PDF + RLM,
      rtlFmt.unicodeWrap(en, true, true)
    );
    assertEquals(
      'overall dir (LTR) doesnt match context dir (unknown), ' + 'no dirReset',
      LRE + en + PDF,
      unkFmt.unicodeWrap(en, true, false)
    );
    assertEquals(
      'overall dir (RTL) doesnt match context dir (unknown), ' + 'dirReset',
      RLE + he + PDF,
      unkFmt.unicodeWrap(he, true, true)
    );
    assertEquals(
      'overall dir (neutral) doesnt match context dir (LTR), ' + 'dirReset',
      '',
  ...
  testUnicodeWrapWithKnownDir() {
    assertEquals('known LTR in LTR context', en, ltrFmt.unicodeWrapWithKnownDir(LTR, en));
    assertEquals('unknown LTR in LTR context', en, ltrFmt.unicodeWrapWithKnownDir(null, en));
    assertEquals(
      'overall LTR but exit RTL in LTR context',
      he + LRM,
    ...
    assertEquals('known neutral in LTR context', '.', ltrFmt.unicodeWrapWithKnownDir(NEUTRAL, '.'));
    assertEquals('unknown neutral in LTR context', '.', ltrFmt.unicodeWrapWithKnownDir(null, '.'));
    assertEquals(
      'overall neutral but exit LTR in LTR context',
      en,
    ...
    assertEquals('known RTL in RTL context', he, rtlFmt.unicodeWrapWithKnownDir(RTL, he));
    assertEquals('unknown RTL in RTL context', he, rtlFmt.unicodeWrapWithKnownDir(null, he));
    assertEquals(
      'overall RTL but exit LTR in RTL context',
      en + RLM,
    ...
    assertEquals('known neutral in RTL context', '.', rtlFmt.unicodeWrapWithKnownDir(NEUTRAL, '.'));
    assertEquals('unknown neutral in RTL context', '.', rtlFmt.unicodeWrapWithKnownDir(null, '.'));
    assertEquals(
      'overall neutral but exit LTR in LTR context',
      he,
  ...
  testMarkAfter() {
    assertEquals(
      'exit dir (RTL) is opposite to context dir (LTR)',
      LRM,
      ltrFmt.markAfter(longEn + he + html, true)
    );
    assertEquals(
      'exit dir (LTR) is opposite to context dir (RTL)',
      RLM,
    ...
    assertEquals(
      'overall dir (RTL) is opposite to context dir (LTR)',
      LRM,
      ltrFmt.markAfter(longHe + en, true)
    );
    assertEquals(
      'overall dir (LTR) is opposite to context dir (RTL)',
      RLM,
  ...
  testMarkAfterKnownDir() {
    assertEquals('known LTR in LTR context', '', ltrFmt.markAfterKnownDir(LTR, en));
    assertEquals('unknown LTR in LTR context', '', ltrFmt.markAfterKnownDir(null, en));
    assertEquals('overall LTR but exit RTL in LTR context', LRM, ltrFmt.markAfterKnownDir(LTR, he));
    assertEquals('known RTL in LTR context', LRM, ltrFmt.markAfterKnownDir(RTL, he));
    assertEquals('unknown RTL in LTR context', LRM, ltrFmt.markAfterKnownDir(null, he));
    assertEquals('overall RTL but exit LTR in LTR context', LRM, ltrFmt.markAfterKnownDir(RTL, en));
    assertEquals('known neutral in LTR context', '', ltrFmt.markAfterKnownDir(NEUTRAL, '.'));
    assertEquals('unknown neutral in LTR context', '', ltrFmt.markAfterKnownDir(null, '.'));
    assertEquals(
      'overall neutral but exit LTR in LTR context',
      '',
    ...
    assertEquals('known RTL in RTL context', '', rtlFmt.markAfterKnownDir(RTL, he));
    assertEquals('unknown RTL in RTL context', '', rtlFmt.markAfterKnownDir(null, he));
    assertEquals('overall RTL but exit LTR in RTL context', RLM, rtlFmt.markAfterKnownDir(RTL, en));
    assertEquals('known LTR in RTL context', RLM, rtlFmt.markAfterKnownDir(LTR, en));
    assertEquals('unknown LTR in RTL context', RLM, rtlFmt.markAfterKnownDir(null, en));
    assertEquals('LTR but exit RTL in RTL context', RLM, rtlFmt.markAfterKnownDir(LTR, he));
    assertEquals('known neutral in RTL context', '', rtlFmt.markAfterKnownDir(NEUTRAL, '.'));
    assertEquals('unknown neutral in RTL context', '', rtlFmt.markAfterKnownDir(null, '.'));
    assertEquals(
      'overall neutral but exit LTR in LTR context',
      '',
    ...
    assertEquals('known RTL in unknown context', '', unkFmt.markAfterKnownDir(RTL, he));
    assertEquals('unknown RTL in unknown context', '', unkFmt.markAfterKnownDir(null, he));
    assertEquals(
      'overall RTL but exit LTR in unknown context',
      '',
      unkFmt.markAfterKnownDir(RTL, en)
    );
    assertEquals('known LTR in unknown context', '', unkFmt.markAfterKnownDir(LTR, en));
    assertEquals('unknown LTR in unknown context', '', unkFmt.markAfterKnownDir(null, en));
    assertEquals('LTR but exit RTL in unknown context', '', unkFmt.markAfterKnownDir(LTR, he));
    assertEquals('known neutral in unknown context', '', unkFmt.markAfterKnownDir(NEUTRAL, '.'));
    assertEquals('unknown neutral in unknown context', '', unkFmt.markAfterKnownDir(null, '.'));
    assertEquals(
      'overall neutral but exit LTR in unknown context',
      '',
