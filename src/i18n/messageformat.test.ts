...
testSuite({
  ...
  testSimpleReplacement() {
    ...
    assertEquals('New York in the Summer is nice.', fmt.format({ SEASON: 'the Summer' }));
  },
  ...
  testNoLiteralsForSingleQuotes() {
    const fmt = new MessageFormat("Anna's house" + " 'has {NUM_COWS} cows'.");
  ...
  testFallbackToOtherOptionInPlurals() {
    ...
    const fmt = new MessageFormat('{NUM_MINUTES, plural, ' + 'other {# minutes}}');
  ...
  testSpecialCharactersInParamaterDontChangeFormat() {
    const fmt = new MessageFormat('{SOME_NUM, plural,' + 'other {# {GROUP}}}');

    // Test pound sign.
    assertEquals('10 group#1', fmt.format({ SOME_NUM: '10', GROUP: 'group#1' }));
  ...
  testMissingOrInvalidPluralParameter() {
    const fmt = new MessageFormat('{SOME_NUM, plural,' + 'other {result}}');

    // Key name doesn't match A != SOME_NUM.
    assertEquals('Undefined or invalid parameter - SOME_NUM', fmt.format({ A: '10' }));

    // Value is not a number.
    assertEquals('Undefined or invalid parameter - SOME_NUM', fmt.format({ SOME_NUM: 'Value' }));
  },

  testMissingSelectParameter() {
    const fmt = new MessageFormat('{GENDER, select,' + 'other {result}}');
  ...
  testPluralWithIgnorePound() {
    ...
    // Test pound sign.
    assertEquals('# group#1', fmt.formatIgnoringPound({ SOME_NUM: '10', GROUP: 'group#1' }));
    // Test other special characters in parameters, like { and }.
    assertEquals('# } {', fmt.formatIgnoringPound({ SOME_NUM: '10', GROUP: '} {' }));
  },
  ...
  testSimpleOrdinal() {
    ...
    assertEquals('Take the elevator to the 1st floor.', fmt.format({ NUM_FLOOR: 1 }));
    assertEquals('Take the elevator to the 2nd floor.', fmt.format({ NUM_FLOOR: 2 }));
    assertEquals('Take the elevator to the 3rd floor.', fmt.format({ NUM_FLOOR: 3 }));
    assertEquals('Take the elevator to the 4th floor.', fmt.format({ NUM_FLOOR: 4 }));
    assertEquals('Take the elevator to the 23rd floor.', fmt.format({ NUM_FLOOR: 23 }));
    // Esoteric example.
    assertEquals('Take the elevator to the 0th floor.', fmt.format({ NUM_FLOOR: 0 }));
  },
  ...
  testMissingOrInvalidOrdinalParameter() {
    const fmt = new MessageFormat('{SOME_NUM, selectordinal,' + 'other {result}}');
