...
testSuite({
  ...
  testGuardBracketInText() {
    ...
    assertEquals('\u05d0 a \u200f(asc:))\u200f', bidi.guardBracketInText(strWithRtl2));
    assertEquals('\u05d0 a \u200f(asc:))\u200f', bidi.guardBracketInText(strWithRtl2, true));
    assertEquals('\u05d0 a \u200e(asc:))\u200e', bidi.guardBracketInText(strWithRtl2, false));

    const strWithoutRtl = 'a (asc) {{123}}';
    assertEquals('a \u200e(asc)\u200e \u200e{{123}}\u200e', bidi.guardBracketInText(strWithoutRtl));
    assertEquals(
      'a \u200f(asc)\u200f \u200f{{123}}\u200f',
  ...
  testEnforceRtlInHtml() {
    ...
    assertEquals('<div dir=rtl> first <br> second </div>', bidi.enforceRtlInHtml(str));
    str = 'first second';
  ...
  testEnforceLtrInHtml() {
    let str = '<div> first <br> second </div>';
    assertEquals('<div dir=ltr> first <br> second </div>', bidi.enforceLtrInHtml(str));
    str = 'first second';
  ...
  testNormalizeHebrewQuote() {
    ...
    assertEquals('\u05d0\u05f3', bidi.normalizeHebrewQuote("\u05d0'"));
    assertEquals('\u05d0\u05f4\u05d0\u05f3', bidi.normalizeHebrewQuote('\u05d0"\u05d0\''));
  },
  ...
  testEstimateDirection() {
    ...
    assertEquals(
      'Farsi digits should count as weakly LTR',
      Dir.LTR,
    ...
    assertEquals(Dir.RTL, bidi.estimateDirection('9 \u05d0 -> 17.5, 23, 45, 19', false));
    assertEquals(
      'Native arabic numbers should count as RTL',
      Dir.RTL,
      bidi.estimateDirection('\u0660', false)
    );
    assertEquals(
      'Native adlam numbers should count as RTL',
      Dir.RTL,
    ...
    assertEquals(Dir.RTL, bidi.estimateDirection('\u200f\u202eArtielish\u202c\u200f'));
    assertEquals(
      Dir.LTR,
      bidi.estimateDirection('\u05d4\u05d3\u05dc <some quite nasty html mark up>', false)
    );
    assertEquals(
      Dir.RTL,
      bidi.estimateDirection('\u05d4\u05d3\u05dc <some quite nasty html mark up>', true)
    );
    assertEquals(
      Dir.LTR,
      bidi.estimateDirection('\u05d4\u05d3\u05dc\u05ea &amp; &lt; &gt;', false)
    );
    assertEquals(Dir.RTL, bidi.estimateDirection('\u05d4\u05d3\u05dc\u05ea &amp; &lt; &gt;', true));
  ...
  testDetectRtlDirectionality() {
    ...
    for (let i = 0; i < bidiText.length; i++) {
       ...
       * checking
       */
      const is_rtl = bidi.detectRtlDirectionality(bidiText[i].text, bidiText[i].isHtml);
  ...
  testSetElementDirByTextDirectionality() {
    ...
    text = ' ';
    bidi.setElementDirByTextDirectionality(el, text);
    assertEquals(`Expected no/empty dir value for neutral text:"${text}"`, '', el.dir);

    text = 'a';
    bidi.setElementDirByTextDirectionality(el, text);
    assertEquals(`Expected dir="ltr" value for LTR text:"${text}"`, 'ltr', el.dir);

    text = '\u05d0';
    bidi.setElementDirByTextDirectionality(el, text);
    assertEquals(`Expected dir="rtl" value for RTL text:"${text}"`, 'rtl', el.dir);
  },
});
