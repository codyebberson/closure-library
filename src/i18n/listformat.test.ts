...
function driveTests(locale, symbols, testCallbackFn) {
  ...
  for (const nativeMode of testECMAScriptOptions) {
    ...
    propertyReplacer.replace(LocaleFeature, 'USE_ECMASCRIPT_I18N_LISTFORMAT', nativeMode);
...
testSuite({
  ...
  testEnOrAnd() {
    ...
    driveTests(
      'en',
      ListSymbols.ListFormatSymbols_en,
      ...
      (nativeMode) => {
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          const expected = andExpected[i].replace(' and ', ' or ');
  ...
  testUk() {
    ...
    driveTests(
      'uk-UA',
      ListSymbolsExt.ListFormatSymbols_uk_UA,
      /** @param {boolean} nativeMode */
      (nativeMode) => {
        const andExpected = [
          '',
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          const expected = andExpected[i].replace(' і ', ' або ');
          assertEquals('Native=' + nativeMode, expected, result);
        }
      }
    );
  ...
  testZhHans() {
    driveTests(
      'zh-Hans',
      ListSymbolsExt.ListFormatSymbols_zh_Hans,
      /** @param {boolean} nativeMode */
      (nativeMode) => {
        ...
        const result1 = listConj.format(vehiclesZh);
        assertEquals('Native=' + nativeMode, expected1, result1);

        const listDisj = new ListFormat({ type: ListFormatType.OR });
        const result2 = listDisj.format(vehiclesZh);
  ...
  testArabic() {
    driveTests(
      'ar',
      ListSymbols.ListFormatSymbols_ar,
      /** @param {boolean} nativeMode */
      (nativeMode) => {
        // Standard Arabic
        const listConj = new ListFormat();
        ...
        result3 = result3.replaceAll('\u2069', '');
        assertEquals('result3 Native=' + nativeMode, 'Motorcycle وBus وCar', result3);

        let result2 = listConj.format(vehicles3.slice(0, 2));
        result2 = result2.replaceAll('\u2068', '');
        result2 = result2.replaceAll('\u2069', '');
        assertEquals('result2: Native=' + nativeMode, 'Motorcycle وBus', result2);
      }
    );
  },
  ...
  testMyanmarAnd() {
    driveTests(
      'my',
      ListSymbols.ListFormatSymbols_my,
      /** @param {boolean} nativeMode */
      (nativeMode) => {
