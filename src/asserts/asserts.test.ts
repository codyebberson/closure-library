...
testSuite({
  testAssert() {
    ...
    doTestMessage(goog.partial(asserts.assert, false, 'ouch %s', 1), 'Assertion failed: ouch 1');
  },
  ...
  testAssertExists() {
    ...
    asserts.assertExists(Number.NaN);
    ...
    assertThrows('assertExists(null)', goog.partial(asserts.assertExists, null));
    assertThrows('assertExists(undefined)', goog.partial(asserts.assertExists, undefined));
  ...
  testNumber() {
    ...
    assertThrows('assertNumber(null)', goog.partial(asserts.assertNumber, null));
    // Test error messages.
    doTestMessage(
  ...
  testString() {
    ...
    assertThrows('assertString(null)', goog.partial(asserts.assertString, null));
    // Test error messages.
    doTestMessage(
  ...
  testFunction() {
    ...
    assertThrows('assertFunction(null)', goog.partial(asserts.assertFunction, null));
    // Test error messages.
    doTestMessage(
  ...
  testObject() {
    ...
    assertThrows('assertObject(null)', goog.partial(asserts.assertObject, null));
    // Test error messages.
    doTestMessage(
  ...
  testElement() {
    ...
    assertThrows(goog.partial(asserts.assertElement, dom.createTextNode('foo')));
  ...
  testInstanceof() {
    ...
    doTestMessage(
      ...
      'Assertion failed: ' + 'Expected instanceof ' + name + ' but got number.'
    );
  ...
  testFailWithCustomErrorHandler() {
    try {
      ...
      assertTrue(
        `Message check failed.  Expected: ${expectedMessage} Actual: ` + handledException.message,
  ...
  testAssertFinite() {
    ...
    assertThrows(goog.partial(asserts.assertFinite, Number.NaN));
    assertThrows(goog.partial(asserts.assertFinite, Number.POSITIVE_INFINITY));
    assertThrows(goog.partial(asserts.assertFinite, Number.NEGATIVE_INFINITY));
    assertThrows(goog.partial(asserts.assertFinite, 'foo'));
    assertThrows(goog.partial(asserts.assertFinite, true));

    // Test error messages.
    doTestMessage(
      goog.partial(asserts.assertFinite, Number.NaN),
      'Assertion failed: Expected NaN to be a finite number but it is not.'
    );
    doTestMessage(
      goog.partial(asserts.assertFinite, Number.NaN, 'ouch %s', 1),
      'Assertion failed: ouch 1'
    );
  },
});
