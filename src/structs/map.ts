 ...
 */
goog.structs.Map.defaultEquals = (a, b) => a === b;
...
goog.structs.Map.prototype.entries = function () {
  return goog.collections.iters.map(this.keys(), (key) => [key, this.get(key)]);
};
...
goog.structs.Map.prototype.__iterator__ = function (opt_keys) {
  ...
  newIter.next = () => {
    ...
    if (i >= this.keys_.length) {
      return goog.iter.ES6_ITERATOR_DONE;
    }
    var key = this.keys_[i++];
    return goog.iter.createEs6IteratorYield(opt_keys ? key : this.map_[key]);
  };
 ...
 * @private
 */
goog.structs.Map.hasKey_ = (obj, key) => Object.prototype.hasOwnProperty.call(obj, key);
