...
testSuite({
  testConstructor() {
    ...
    assertEquals('testConstructor(), head element should be undefined', q.peek(), undefined);
  },
  ...
  testDequeue() {
    ...
    assertEquals('testDequeue(), should return undefined for empty queue', q.dequeue(), undefined);
  },

  testPeek() {
    const q = createQueue();
    assertEquals('testPeek(), should return b', q.peek(), 'b');
    assertEquals('testPeek(), dequeue should return peek() result', q.dequeue(), 'b');
    assertEquals('testPeek(), should return c', q.peek(), 'c');
    q.clear();
    assertEquals('testPeek(), should return undefined for empty queue', q.peek(), undefined);
  },
  ...
  testQueue() {
    const q = createQueue();
    assertEquals('testQueue(), contents must be bcabc', stringifyQueue(q), 'bcabc');
  },

  testRemove() {
    const q = createQueue();
    assertEquals('testRemove(), contents must be bcabc', stringifyQueue(q), 'bcabc');

    q.dequeue();
    assertEquals('testRemove(), contents must be cabc', stringifyQueue(q), 'cabc');

    q.enqueue('a');
    assertEquals('testRemove(), contents must be cabca', stringifyQueue(q), 'cabca');

    assertTrue('testRemove(), remove should have returned true', q.remove('c'));
    assertEquals('testRemove(), contents must be abca', stringifyQueue(q), 'abca');
    ...
    assertEquals('testRemove(), contents must be bcabc', stringifyQueue(q), 'bcabc');
    assertTrue('testRemove(), remove should have returned true', q.remove('c'));
    assertEquals('testRemove(), contents must be babc', stringifyQueue(q), 'babc');
  },

  testContains() {
    const q = createQueue();
    assertTrue('testContains(), contains should have returned true', q.contains('a'));
    assertFalse('testContains(), contains should have returned false', q.contains('foobar'));
  },
});
