 ...
 * @private
 */
goog.math.Path.segmentArgCounts_ = (() => {
  var counts = [];
 ...
 */
goog.math.Path.getSegmentCount = (segment) => goog.math.Path.segmentArgCounts_[segment];
 ...
 */
goog.math.Path.prototype.arc = function (cx, cy, rx, ry, fromAngle, extent, connect) {
 ...
 */
goog.math.Path.simplifySegmentMap_ = (() => {
  var map = {};
 ...
 */
goog.math.Path.createSimplifiedPath = (src) => {
  if (src.isSimple()) {
    return src.clone();
  }
  var path = new goog.math.Path();
  src.forEachSegment((segment, args) => {
