 ...
 *     size.
 */
goog.math.Rect.createFromPositionAndSize = (position, size) =>
...
goog.math.Rect.createFromBox = (box) =>
  new goog.math.Rect(box.left, box.top, box.right - box.left, box.bottom - box.top);
 ...
 */
goog.math.Rect.intersection = (a, b) => {
 ...
 *     null.
 */
goog.math.Rect.boundingRect = (a, b) => {
  if (!a || !b) {
    return null;
  }
...
goog.math.Rect.prototype.squaredDistance = function (point) {
  var dx =
    point.x < this.left ? this.left - point.x : Math.max(point.x - (this.left + this.width), 0);
  var dy =
    point.y < this.top ? this.top - point.y : Math.max(point.y - (this.top + this.height), 0);
  return dx * dx + dy * dy;
};
