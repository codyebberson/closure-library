...
FakeSingletonRenderer.getInstance = () => FakeSingletonRenderer.instance_;
...
testSuite({
  ...
  testSetDefaultRenderer() {
    const rx1 = registry.getDefaultRenderer(FakeComponentX);
    assertTrue("FakeComponentX's renderer must be FakeRenderer", rx1 instanceof FakeRenderer);
    ...
    assertEquals(
      'FakeComponentY must inherit the new default renderer ' + 'from its superclass',
      FakeSingletonRenderer.getInstance(),
      ry2
    );
    ...
    assertThrows(
      'Calling setDefaultRenderer with non-function component ' + 'must throw error',
      /** @suppress {checkTypes} suppression added to enable type checking */
      () => {
    ...
    assertThrows(
      'Calling setDefaultRenderer with non-function renderer ' + 'must throw error',
      /** @suppress {checkTypes} suppression added to enable type checking */
      () => {
  ...
  testGetDecoratorByClassName() {
    ...
    assertNotEquals(
      'Each call to getDecoratorByClassName must return a ' + 'new FakeComponentX instance',
      dx1,
      dx2
    );
    ...
    assertNotEquals(
      'Each call to getDecoratorByClassName must return a ' + 'new FakeComponentY instance',
      dy1,
      dy2
    );
  ...
  testSetDecoratorByClassName() {
    ...
    assertThrows(
      'Calling setDecoratorByClassName with invalid class name ' + 'must throw error',
      () => {
        registry.setDecoratorByClassName(
          '' /**
    ...
    assertThrows(
      'Calling setDecoratorByClassName with non-function ' + 'decorator must throw error',
      /** @suppress {checkTypes} suppression added to enable type checking */
      () => {
  ...
  testReset() {
    ...
    assertTrue('No renderers must be registered', googObject.isEmpty(registry.defaultRenderers_));
    assertTrue(
