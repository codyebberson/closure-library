 ...
 * @private
 */
goog.ui.SplitPane.HANDLE_CLASS_NAME_HORIZONTAL_ = goog.getCssName(
...
goog.ui.SplitPane.prototype.createDom = function () {
  ...
  var splitterHandle = dom.createDom(goog.dom.TagName.DIV, goog.ui.SplitPane.HANDLE_CLASS_NAME_);

  // Create the primary element, a DIV that holds the two containers and handle.
  this.setElementInternal(
    dom.createDom(
      goog.dom.TagName.DIV,
      goog.ui.SplitPane.CLASS_NAME_,
      firstContainer,
      secondContainer,
      splitterHandle
    )
 ...
 * @private
 */
goog.ui.SplitPane.prototype.getElementToDecorate_ = (rootElement, className) => {
...
goog.ui.SplitPane.prototype.enterDocument = function () {
  ...
  this.getHandler()
    .listen(this.splitpaneHandle_, goog.events.EventType.DBLCLICK, this.handleDoubleClick_)
    .listen(this.splitDragger_, goog.fx.Dragger.EventType.START, this.handleDragStart_)
    .listen(this.splitDragger_, goog.fx.Dragger.EventType.DRAG, this.handleDrag_)
    .listen(this.splitDragger_, goog.fx.Dragger.EventType.END, this.handleDragEnd_);
...
goog.ui.SplitPane.prototype.setUpHandle_ = function () {
  if (this.isVertical()) {
    ...
    goog.dom.classlist.add(this.splitpaneHandle_, goog.ui.SplitPane.HANDLE_CLASS_NAME_VERTICAL_);
  } else {
    this.splitpaneHandle_.style.width = this.handleSize_ + 'px';
    goog.dom.classlist.add(this.splitpaneHandle_, goog.ui.SplitPane.HANDLE_CLASS_NAME_HORIZONTAL_);
  }
};
    ...
    if (this.isInDocument()) {
      ...
      if (typeof this.firstComponentSize_ === 'number') {
        ...
        var ratio = isVertical
          ? splitpaneSize.height / splitpaneSize.width
 ...
 * @private
 */
goog.ui.SplitPane.prototype.moveAndSize_ = (element, rect) => {
...
goog.ui.SplitPane.prototype.setFirstComponentSize = function (opt_size) {
  this.setFirstComponentSize_(goog.style.getBorderBoxSize(this.getElement()), opt_size);
};
 ...
 * @private
 */
goog.ui.SplitPane.prototype.setFirstComponentSize_ = function (splitpaneSize, opt_size) {
  var top = 0,
    left = 0;
  ...
  var firstComponentSize =
    typeof opt_size === 'number'
      ? opt_size
...
goog.ui.SplitPane.prototype.handleDragStart_ = function (e) {
  ...
  if (!this.iframeOverlay_) {
    ...
    this.iframeOverlay_ = this.getDomHelper().createDom(goog.dom.TagName.DIV, { style: cssStyles });
