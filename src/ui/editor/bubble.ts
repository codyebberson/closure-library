...
goog.ui.editor.Bubble = function (parent, zIndex) {
   ...
   * @private
   */
  this.bubbleContainer_ = this.dom_.createDom(goog.dom.TagName.DIV, {
  ...
  goog.dom.safe.setInnerHtmlFromConstant(this.closeBox_, goog.string.Const.from('&nbsp;'));
 ...
 * @protected
 */
goog.ui.editor.Bubble.prototype.createBubbleDom = (dom, container) => container;
...
goog.ui.editor.Bubble.prototype.hasPanelOfType = function (type) {
  return goog.object.some(this.panels_, (panel) => panel.type == type);
};
...
goog.ui.editor.Bubble.prototype.addPanel = function (
  ...
  let nextElement;
  for (let i = 0, len = this.bubbleContents_.childNodes.length - 1; i < len; i++) {
...
goog.ui.editor.Bubble.prototype.openBubble_ = function () {
  this.eventHandler_
    ...
    .listen(this.viewPortSizeMonitor_, goog.events.EventType.RESIZE, this.handleWindowResize_)
    .listen(this.popup_, goog.ui.PopupBase.EventType.HIDE, this.handlePopupHide);
...
goog.ui.editor.Bubble.prototype.handlePopupHide = function () {
  ...
  for (const panelId in this.panels_) {
 ...
 * @protected
 */
goog.ui.editor.Bubble.prototype.getMarginBox = () => goog.ui.editor.Bubble.MARGIN_BOX_;
...
goog.ui.editor.Bubble.prototype.reposition = function () {
  ...
  const reverseLayout =
    goog.style.isRightToLeft(this.bubbleContainer_) != goog.style.isRightToLeft(targetElement);
  ...
  if (preferBottomPosition) {
    status = this.positionAtAnchor_(
      reverseLayout ? goog.positioning.Corner.BOTTOM_END : goog.positioning.Corner.BOTTOM_START,
  ...
  if (status & goog.positioning.OverflowStatus.FAILED) {
    ...
    // space at the bottom.
    status = this.positionAtAnchor_(
      reverseLayout ? goog.positioning.Corner.TOP_END : goog.positioning.Corner.TOP_START,
  ...
  if (status & goog.positioning.OverflowStatus.FAILED) {
    ...
    if (status & goog.positioning.OverflowStatus.FAILED) {
      goog.log.warning(this.logger, 'reposition(): positionAtAnchor() failed with ' + status);
    }
  }
};
...
goog.ui.editor.Bubble.Panel_ = function (
  ...
  this.element = dom.createDom(
    ...
    dom.createDom(
      ...
      title ? title + ':' : ''
    ), // TODO(robbyw): Does this work in bidi?
    dom.createDom(goog.dom.TagName.DIV, { className: goog.getCssName('tr_bubble_panel_content') })
  );
};
