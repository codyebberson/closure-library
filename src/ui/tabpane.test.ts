...
testSuite({
  setUp() {
    dom.getElement('testBody').innerHTML =
      '<div id="tabpane"></div>' +
      '<div id="page1Content">' +
      '  Content for page 1' +
      '</div>' +
  ...
  testAllPagesEnabledAndSelectable() {
    ...
    assertEquals('goog-tabpane-tab-selected', selected.getTitleElement().className);
  ...
  testDisabledPageIsNotSelectable() {
    ...
    selected = tabPane.getSelectedPage();
    assertEquals(
      'page1 should remain selected, as page2 is disabled',
      'page1',
      selected.getTitle()
    );
    assertEquals('goog-tabpane-tab-selected', selected.getTitleElement().className);
