...
testSuite({
  ...
  testConstructor() {
    assertNotNull('ControlRenderer singleton instance must not be null', controlRenderer);
    assertNotNull('TestRenderer singleton instance must not be null', testRenderer);
  },

  testGetCustomRenderer() {
    const cssClass = 'special-css-class';
    const renderer = ControlRenderer.getCustomRenderer(ControlRenderer, cssClass);
    assertEquals(
  ...
  testGetAriaRole() {
    assertUndefined("ControlRenderer's ARIA role must be undefined", controlRenderer.getAriaRole());
    assertEquals(
  ...
  testEnableExtraClassName() {
    ...
    controlRenderer.enableExtraClassName(control, 'foo', true);
    assertSameElements(
      'Extra class name must have been added',
      ['goog-control', 'foo'],
    ...
    assertSameElements(
      'Extra class name must have been removed',
      ['goog-control'],
      classlist.get(element)
    );
  },
  ...
  testDecorate() {
    ...
    assertEquals("Decorated control's state must be as expected", 0x00, control.getState());
    assertSameElements(
  ...
  testDecorateComplexDom() {
    ...
    assertEquals('decorate() must return its argument', foo, element);
    assertEquals("Decorated control's ID must be set", 'foo', control.getId());
    assertTrue("Decorated control's content must be an array", Array.isArray(control.getContent()));
    assertEquals(
      "Decorated control's content must have expected length",
      4,
  ...
  testDecorateWithClasses() {
    sandbox.innerHTML = '<div id="foo" class="app goog-base-disabled goog-base-hover"></div>';
    const foo = dom.getElement('foo');
    ...
    assertEquals(
      ...
      Component.State.DISABLED | Component.State.HOVER,
      control.getState()
    );
    assertSameElements(
      "Decorated element's classes must be as expected",
      ['app', 'extra', 'goog-base', 'goog-base-disabled', 'goog-base-hover', 'goog-button'],
      classlist.get(element)
    );
  },
  ...
  testDecorateOptimization() {
    ...
    // called at all.
    sandbox.innerHTML = '<div id="bar" class="goog-base goog-button">Bar' + '</div>';
    testRenderer.decorate(control, dom.getElement('bar'));

    // Since baz has all required classes, goog.dom.classlist.set() must not be
    // called at all.
    sandbox.innerHTML =
      '<div id="baz" class="goog-base goog-button ' + 'goog-button-disabled">Baz</div>';
  ...
  testSetAriaRole() {
    ...
    assertEquals('Element must have expected ARIA role', Role.BUTTON, aria.getRole(bar));
  },
  ...
  testSetAriaStatesHidden() {
    ...
    assertEquals('ControlRenderer did not set aria-hidden.', '', aria.getState(foo, State.HIDDEN));
  ...
  testSetAriaStatesSelected() {
    ...
    assertEquals(
      'ControlRenderer did not set aria-selected.',
      'true',
  ...
  testSetAriaStatesChecked() {
    ...
    assertEquals(
      'ControlRenderer did not set aria-checked.',
      'true',
  ...
  testSetAriaStatesExpanded() {
    ...
    assertEquals(
      'ControlRenderer did not set aria-expanded.',
      'true',
  ...
  testSetAllowTextSelection() {
    ...
    assertTrue('Parent element must be unselectable on all browsers', style.isUnselectable(foo));
    if (userAgent.IE) {
      assertTrue(
      ...
      assertFalse(
        'On browsers other than IE and Opera, the child element ' + 'must not be unselectable',
        style.isUnselectable(foo.firstChild)
      );
    }
    ...
    assertFalse('Child element must be unselectable', style.isUnselectable(foo.firstChild));
  },
  ...
  testSetRightToLeft() {
    ...
    assertSameElements(
      'Element must have right-to-left class applied',
      ['goog-control-rtl'],
      classlist.get(foo)
    );
    controlRenderer.setRightToLeft(foo, false);
    assertSameElements('Element must not have right-to-left class applied', [], classlist.get(foo));
    ...
    assertSameElements('Element must not have right-to-left class applied', [], classlist.get(bar));
  },
  ...
  testIsFocusableForNonFocusableControl() {
    ...
    control.render(sandbox);
    assertFalse(
      "Non-focusable control's key event target must not be " + 'focusable',
      controlRenderer.isFocusable(control)
    );
  },
  ...
  testIsFocusableForControlWithoutKeyEventTarget() {
    ...
    assertNull('Unrendered control must not have key event target', control.getKeyEventTarget());
    assertFalse(
  ...
  testSetState() {
    ...
    assertSameElements(
      'Control must have expected class names',
    ...
    assertEquals(
      'Control must have disabled ARIA state',
      'true',
      aria.getState(element, State.DISABLED)
    );
  ...
  testUpdateAriaStateDisabled() {
    ...
    assertEquals(
      'Control must no longer have disabled ARIA state',
      'false',
      aria.getState(element, State.DISABLED)
    );
  },
  ...
  testUpdateAriaStateSelected() {
    ...
    assertEquals(
      'Control must have selected ARIA state',
      'true',
    ...
    assertEquals(
      'Control must no longer have selected ARIA state',
      'false',
      aria.getState(element, State.SELECTED)
    );
  },
  ...
  testSetAriaStatesRender_ariaStateNotSelected() {
    ...
    assertEquals(
      'Control must have not-selected ARIA state',
      'false',
      aria.getState(element, State.SELECTED)
    );
  },
  ...
  testUpdateAriaStateChecked() {
    ...
    assertEquals(
      'Control must have checked ARIA state',
      'true',
    ...
    assertEquals(
      'Control must no longer have checked ARIA state',
      'false',
      aria.getState(element, State.CHECKED)
    );
  },
  ...
  testUpdateAriaStateOpened() {
    ...
    assertEquals(
      'Control must have expanded ARIA state',
      'true',
    ...
    assertEquals(
      'Control must no longer have expanded ARIA state',
      'false',
      aria.getState(element, State.EXPANDED)
    );
  },
  ...
  testSetAriaStateRoleNotInMap() {
    ...
    const element = dom.getElement('foo');
    control.decorate(element);
    assertEquals('Element should have ARIA role option.', Role.OPTION, aria.getRole(element));
    control.setStateInternal(Component.State.DISABLED, true);
    controlRenderer.setAriaStates(control, element);
    assertEquals(
      'Element should have aria-disabled true',
      'true',
      aria.getState(element, State.DISABLED)
    );
    ...
    controlRenderer.setAriaStates(control, element);
    assertEquals(
      'Element should have aria-checked true',
      'true',
      aria.getState(element, State.CHECKED)
    );
  },
  ...
  testSetAriaStateRoleInMapMatches() {
    ...
    const element = dom.getElement('foo');
    control.decorate(element);
    assertEquals('Element should have ARIA role checkbox.', Role.CHECKBOX, aria.getRole(element));
    control.setStateInternal(Component.State.DISABLED, true);
    controlRenderer.setAriaStates(control, element);
    assertEquals(
      'Element should have aria-disabled true',
  ...
  testSetAriaStateRoleInMapNotMatches() {
    ...
    const element = dom.getElement('foo');
    control.decorate(element);
    assertEquals('Element should have ARIA role button.', Role.BUTTON, aria.getRole(element));
    control.setStateInternal(Component.State.DISABLED, true);
    controlRenderer.setAriaStates(control, element);
    assertEquals(
      'Element should have aria-disabled true',
    ...
    controlRenderer.setAriaStates(control, element);
    assertEquals(
      'Element should have aria-pressed true',
      'true',
      aria.getState(element, State.PRESSED)
    );
    assertEquals('Element should not have aria-checked', '', aria.getState(element, State.CHECKED));
  },
  ...
  testToggleAriaStateMap() {
    ...
    for (const key in map) {
      assertTrue('Toggle ARIA state map incorrect.', key in ControlRenderer.TOGGLE_ARIA_STATE_MAP_);
      assertEquals(
  ...
  testSetContentNull() {
    ...
    assertEquals('Element must have no child nodes', 0, sandbox.childNodes.length);
    assertEquals('Element must contain expected text value', '', dom.getTextContent(sandbox));
  },
  ...
  testSetContentWhitespace() {
    ...
    assertEquals('Child must be a text node', NodeType.TEXT, sandbox.firstChild.nodeType);
    assertEquals('Element must contain expected text value', ' ', dom.getTextContent(sandbox));
  },
  ...
  testSetContentTextNode() {
    ...
    assertEquals('Element must have one child', 1, sandbox.childNodes.length);
    assertEquals('Child must be a text node', NodeType.TEXT, sandbox.firstChild.nodeType);
    assertEquals('Element must contain expected text value', 'Text', dom.getTextContent(sandbox));
  },

  testSetContentElementNode() {
    dom.setTextContent(sandbox, 'Hello, world!');
    controlRenderer.setContent(sandbox, dom.createDom(TagName.DIV, { id: 'foo' }, 'Foo'));
    assertEquals('Element must have one child', 1, sandbox.childNodes.length);
    assertEquals('Child must be an element node', NodeType.ELEMENT, sandbox.firstChild.nodeType);
    assertHTMLEquals(
      'Element must contain expected HTML',
      '<div id="foo">Foo</div>',
      sandbox.innerHTML
    );
  },
  ...
  testSetContentArray() {
    dom.setTextContent(sandbox, 'Hello, world!');
    controlRenderer.setContent(sandbox, ['Hello, ', dom.createDom(TagName.B, null, 'world'), '!']);
    assertEquals('Element must have three children', 3, sandbox.childNodes.length);
    assertEquals('1st child must be a text node', NodeType.TEXT, sandbox.childNodes[0].nodeType);
    assertEquals('2nd child must be an element', NodeType.ELEMENT, sandbox.childNodes[1].nodeType);
    assertEquals('3rd child must be a text node', NodeType.TEXT, sandbox.childNodes[2].nodeType);
    assertHTMLEquals(
      'Element must contain expected HTML',
      'Hello, <b>world</b>!',
      sandbox.innerHTML
  ...
  testGetCssClass() {
    ...
    assertEquals(
      ...
      TestRenderer.CSS_CLASS,
      testRenderer.getCssClass()
    );
  },
  ...
  testGetClassNames() {
    ...
    assertArrayEquals(
      'ControlRenderer must return expected class names ' + 'in the expected order',
      ['goog-control'],
      controlRenderer.getClassNames(control)
    );
    assertArrayEquals(
      'TestRenderer must return expected class names ' + 'in the expected order',
      ['goog-button', 'goog-base'],
  ...
  testGetClassNamesForControlWithStateAndExtraClassNames() {
    ...
    assertArrayEquals(
      'ControlRenderer must return expected class names ' + 'in the expected order',
      ['goog-control', 'goog-control-hover', 'goog-control-active', 'foo', 'bar'],
      controlRenderer.getClassNames(control)
    );
    assertArrayEquals(
      'TestRenderer must return expected class names ' + 'in the expected order',
      ['goog-button', 'goog-base', 'goog-base-hover', 'goog-base-active', 'foo', 'bar'],
      testRenderer.getClassNames(control)
    );
  },
  ...
  testGetClassNamesForState() {
    ...
    assertArrayEquals(
      ...
      controlRenderer.getClassNamesForState(Component.State.HOVER | Component.State.CHECKED)
    );
    assertArrayEquals(
      'TestRenderer must return expected class names ' + 'in the expected order',
      ['goog-base-hover', 'goog-base-checked'],
      testRenderer.getClassNamesForState(Component.State.HOVER | Component.State.CHECKED)
    );
  },
  ...
  testGetClassForState() {
    const renderer = new ControlRenderer();
    assertUndefined('State-to-class map must not exist until first use', renderer.classByState_);
    assertEquals(
  ...
  testGetStateFromClass() {
    const renderer = new ControlRenderer();
    assertUndefined('Class-to-state map must not exist until first use', renderer.stateByClass_);
    assertEquals(
      'Renderer must return expected state',
      Component.State.SELECTED,
      renderer.getStateFromClass('goog-control-selected')
    );
    assertEquals(
      'Renderer must return 0x00 for unknown class',
      0x00,
  ...
  testIe6ClassCombinationsCreateDom() {
    ...
    testRenderer.setState(control, Component.State.DISABLED, true);
    let expectedClasses = ['combined', 'goog-base', 'goog-base-disabled', 'goog-button'];
    assertSameElements(
      'Non IE6 browsers should not have a combined class',
      expectedClasses,
      classlist.get(element)
    );
    ...
    expectedClasses = ['combined', 'goog-base', 'goog-base-hover', 'goog-button'];
    if (isIe6()) {
      assertSameElements(
  ...
  testIe6ClassCombinationsDecorate() {
    ...
    const expectedClasses = ['combined', 'goog-base', 'goog-base-hover', 'goog-button'];
    if (isIe6()) {
      assertSameElements(
        'IE6 and lower should have one combined class',
        expectedClasses.concat(['combined_goog-base-hover_goog-button']),
