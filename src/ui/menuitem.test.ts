...
testSuite({
  ...
  testMenuItem() {
    ...
    assertEquals('Caption must default to the content', item.getContent(), item.getCaption());
    assertEquals('Value must default to the caption', item.getCaption(), item.getValue());
  },
  ...
  testMenuItemConstructor() {
    ...
    assertEquals('Content must have expected value', 'Item', menuItem.getContent());
    assertEquals(
      'Caption must default to the content',
      menuItem.getContent(),
      menuItem.getCaption()
    );
    assertEquals('Model must be set', model, menuItem.getModel());
    assertNotEquals('Value must not equal the caption', menuItem.getCaption(), menuItem.getValue());
  ...
  testGetValue() {
    ...
    assertEquals(
      'Without a model, value must default to the caption',
      item.getCaption(),
      item.getValue()
    );
    item.setModel('Foo');
    assertEquals('With a model, value must default to the model', item.getModel(), item.getValue());
  },
  ...
  testSetValue() {
    ...
    assertEquals('Value and model must be the same', item.getValue(), item.getModel());
  },

  testGetSetContent() {
    assertEquals('Content must have expected value', 'Item', item.getContent());
    item.setContent(dom.createDom(TagName.DIV, 'foo', 'Foo'));
    assertEquals('Content must be an element', NodeType.ELEMENT, item.getContent().nodeType);
    assertHTMLEquals(
      'Content must be the expected element',
  ...
  testGetSetCaption() {
    ...
    assertTrue('Caption must be a string', typeof item.getCaption() === 'string');
    assertEquals('Caption must have expected value', 'Hello, world!', item.getCaption());
    item.setContent(dom.createDom(TagName.DIV, 'foo', 'Foo'));
    assertTrue('Caption must be a string', typeof item.getCaption() === 'string');
  ...
  testGetSetCaptionAfterCreateDom() {
    ...
    const arrayContent = googArray.clone(
      dom.safeHtmlToNode(testing.newSafeHtmlForTest(' <b> \xa0foo</b><i>  bar</i> ')).childNodes
    );
    item.setContent(arrayContent);
    assertEquals('whitespaces must be normalized in the caption', '\xa0foo bar', item.getCaption());
  },
  ...
  testSetSelectable() {
    ...
    assertTrue('Item must be selectable', item.isSupportedState(Component.State.SELECTED));
  ...
  testSetCheckable() {
    ...
    assertTrue('Item must be checkable', item.isSupportedState(Component.State.CHECKED));
  ...
  testGetSetContentForItemWithCheckBox() {
    ...
    assertEquals('getContent() must not return the checkbox structure', 'Item', item.getContent());

    item.setContent('Hello');
    assertEquals('getContent() must not return the checkbox structure', 'Hello', item.getContent());
    assertTrue(
    ...
    assertEquals('getContent() must return element', NodeType.ELEMENT, item.getContent().nodeType);
    assertTrue(
      'Item must still have checkbox structure',
      item.getRenderer().hasCheckBoxStructure(item.getElement())
    );
  ...
  testGetSetCaptionForItemWithCheckBox() {
    ...
    assertEquals('getCaption() must not return the checkbox structure', 'Item', item.getCaption());

    item.setCaption('Hello');
    assertEquals('getCaption() must not return the checkbox structure', 'Hello', item.getCaption());
    assertTrue(
      'Item must still have checkbox structure',
      item.getRenderer().hasCheckBoxStructure(item.getElement())
    );
    ...
    item.setContent(dom.createDom(TagName.SPAN, 'foo', 'Foo'));
    assertEquals('getCaption() must return text content', 'Foo', item.getCaption());
    assertTrue(
      'Item must still have checkbox structure',
      item.getRenderer().hasCheckBoxStructure(item.getElement())
    );
  ...
  testGetSetCaptionForItemWithAccelerators() {
    const contentArr = [];
    contentArr.push(dom.createDom(TagName.SPAN, goog.getCssName('goog-menuitem-accel'), 'Ctrl+1'));
    contentArr.push(dom.createTextNode('Hello'));
    item.setCaption(contentArr);
    assertEquals('getCaption() must not return the accelerator', 'Hello', item.getCaption());

    item.setCaption([
      dom.createDom(TagName.SPAN, goog.getCssName('goog-menuitem-accel'), 'Ctrl+1'),
    ]);
    assertEquals('getCaption() must return empty string', '', item.getCaption());

    assertEquals('getAccelerator() should return the accelerator', 'Ctrl+1', item.getAccelerator());
  },

  testGetSetCaptionForItemWithMnemonics() {
    let contentArr = [];
    contentArr.push(
      dom.createDom(TagName.SPAN, goog.getCssName('goog-menuitem-mnemonic-hint'), 'H')
    );
    contentArr.push(dom.createTextNode('ello'));
    item.setCaption(contentArr);
    assertEquals('getCaption() must not return hint markup', 'Hello', item.getCaption());
    ...
    contentArr.push(
      dom.createDom(
        ...
        '(',
        dom.createDom(TagName.SPAN, goog.getCssName('goog-menuitem-mnemonic-hint'), 'J'),
        ')'
      )
    );
    item.setCaption(contentArr);
    assertEquals(
      'getCaption() must not return the paranethetical mnemonic',
      'Hello',
      item.getCaption()
    );

    item.setCaption('');
    assertEquals('getCaption() must return the empty string', '', item.getCaption());
  },
  ...
  testHandleKeyEventInternalWithMnemonic() {
    ...
    assertEquals(
      'performActionInternal must be called',
      1,
  ...
  testHandleKeyEventInternalWithoutMnemonic() {
    ...
    item.handleKeyEventInternal({ keyCode: KeyCodes.F });
    assertEquals(
      'performActionInternal must not be called without a' + ' mnemonic',
      0,
      item.performActionInternal.getCallCount()
    );
  },
  ...
  testRender() {
    ...
    assertTrue(
      ...
      classlist.contains(contentElement, item.getRenderer().getStructuralCssClass() + '-content')
    );
    assertHTMLEquals(
      'Content element must have expected structure',
      'Item',
  ...
  testDecorate() {
    ...
    assertTrue(
      ...
      classlist.contains(item.getElement(), item.getRenderer().getCssClass())
    );
    assertEquals(
      "Content element must be the decorated element's child",
      item.getContentElement(),
      item.getElement().firstChild
    );
    assertHTMLEquals(
      'Content must have expected structure',
      'Foo',
  ...
  testDecorateCheckedItem() {
    sandbox.innerHTML = '<div id="foo" class="goog-option goog-option-selected">Foo</div>';
    const foo = dom.getElement('foo');
    item.decorate(foo);
    assertEquals('Decorated element must be as expected', foo, item.getElement());
    assertSameElements(
  ...
  testDecorateTemplate() {
    sandbox.innerHTML =
      '<div id="foo" class="goog-menuitem">' + '<div class="goog-menuitem-content">Foo</div></div>';
    const foo = dom.getElement('foo');
    item.decorate(foo);
    assertEquals('Decorated element must be as expected', foo, item.getElement());
    assertTrue(
  ...
  testDecorateCheckableItemTemplate() {
    sandbox.innerHTML =
      '<div id="foo" class="goog-menuitem goog-option">' +
    ...
    assertEquals(
      'Item must have exactly one checkbox structure',
      1,
      dom.getElementsByTagNameAndClass(TagName.DIV, 'goog-menuitem-checkbox', item.getElement())
        .length
    );
    assertFalse('Item must not be checked', item.isChecked());
  },
  ...
  testSetAriaLabel() {
    ...
    assertEquals('Item element must have expected aria-role', 'menuitem', el.getAttribute('role'));
