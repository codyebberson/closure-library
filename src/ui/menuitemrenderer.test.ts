...
testSuite({
  ...
  testMenuItemRenderer() {
    ...
    assertEquals(
      'Singleton getter must always return same instance',
      renderer,
  ...
  testCreateDom() {
    ...
    assertEquals('Element must have exactly one child element', 1, element.childNodes.length);
    assertHTMLEquals(
  ...
  testCreateDomForCheckableItem() {
    ...
    assertEquals(
      'Item must have checked ARIA state',
      'true',
  ...
  testCreateDomForSelectableItem() {
    ...
    assertEquals(
      'Item must have selected ARIA state',
      'true',
      aria.getState(element, State.CHECKED)
    );
  },
  ...
  testGetContentElement() {
    assertNull('Content element must be the null initially', item.getContentElement());
    item.createDom();
    assertEquals(
  ...
  testDecorateWithContentStructure() {
    sandbox.innerHTML = '<div id="foo"><div class="goog-menuitem-content">Hello</div></div>';
    const foo = dom.getElement('foo');
  ...
  testDecorateWithHoverState() {
    sandbox.innerHTML = '<div id="foo" class="goog-menuitem-highlight">Hello</div>';
    const foo = dom.getElement('foo');
  ...
  testDecorateCheckableItem() {
    ...
    assertFalse('Item must not be checkable', item.isSupportedState(Component.State.CHECKED));
    const element = renderer.decorate(item, foo);
    assertSameElements(
      'Element must have the expected class names',
      ['goog-menuitem', 'goog-option'],
