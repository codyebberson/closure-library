...
/** @override */
goog.ui.TableSorter.prototype.canDecorate = (element) => element.tagName == goog.dom.TagName.TABLE;
...
goog.ui.TableSorter.prototype.sort = function (column, opt_reverse) {
  ...
  var cmpFn = (a, b) => multiplier * sortFunction(a[0], b[0]) || a[1] - b[1];

  // Sort all tBodies
  var table = this.getElement();
  goog.array.forEach(table.tBodies, (tBody) => {
    // Collect all of the rows into an array.
    var values = goog.array.map(tBody.rows, (row, rowIndex) => [
      goog.dom.getTextContent(row.cells[column]),
      rowIndex,
      row,
    ]);
 ...
 * @return {number} Negative if a < b, 0 if a = b, and positive if a > b.
 */
goog.ui.TableSorter.numericSort = (a, b) => {
 ...
 *     given sort function.
 */
goog.ui.TableSorter.createReverseSort = (sortFunction) => (a, b) => -1 * sortFunction(a, b);
