...
goog.ui.Tooltip = function (opt_el, opt_str, opt_domHelper) {
  ...
  this.dom_ =
    opt_domHelper ||
    (opt_el ? goog.dom.getDomHelper(goog.dom.getElement(opt_el)) : goog.dom.getDomHelper());
...
goog.ui.Tooltip.prototype.maybeHide = function (el) {
  ...
  if (el == this.anchor) {
    ...
    var tooltipContentFocused =
      focusedEl && this.getElement() && dom.contains(this.getElement(), focusedEl);
    if (
      (this.activeEl_ == null ||
        (this.activeEl_ != this.getElement() && !this.elements_.contains(this.activeEl_))) &&
...
goog.ui.Tooltip.prototype.handleMouseOutAndBlur = function (event) {
  ...
  if (
    this.isVisible() &&
    (!event.relatedTarget || !goog.dom.contains(this.getElement(), event.relatedTarget))
  ) {
    this.startHideTimer();
  } else {
    this.anchor = undefined;
