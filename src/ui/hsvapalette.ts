...
goog.ui.HsvaPalette = function (opt_domHelper, opt_color, opt_alpha, opt_class) {
  goog.ui.HsvaPalette.base(this, 'constructor', opt_domHelper, opt_color, opt_class);
...
goog.ui.HsvaPalette.prototype.createDom = function () {
  ...
  this.aImageEl_ = /** @type {!HTMLElement} */ (
    dom.createDom(goog.dom.TagName.DIV, goog.getCssName(this.className, 'a-image'))
  );
  this.aHandleEl_ = /** @type {!HTMLElement} */ (
    dom.createDom(goog.dom.TagName.DIV, goog.getCssName(this.className, 'a-handle'))
  );
  this.swatchBackdropEl_ = dom.createDom(
    goog.dom.TagName.DIV,
    goog.getCssName(this.className, 'swatch-backdrop')
  );
...
goog.ui.HsvaPalette.prototype.handleMouseMoveA_ = function (b, e) {
  ...
  /** @suppress {strictMissingProperties} Added to tighten compiler checks */
  var newA =
    (b.top + b.height - Math.min(Math.max(vportPos.y + e.clientY, b.top), b.top + b.height)) /
    b.height;
  this.setAlpha(newA);
};
 ...
 * @private
 */
goog.ui.HsvaPalette.parseUserInput_ = (value) => {
 ...
 * @private
 */
goog.ui.HsvaPalette.parseColorRgbaHex_ = (color) => {
  var hex = goog.color.alpha.parse(color).hex;
  return [
    goog.color.alpha.extractHexColor(hex),
    Number.parseInt(goog.color.alpha.extractAlpha(hex), 16) / 255,
  ];
};
