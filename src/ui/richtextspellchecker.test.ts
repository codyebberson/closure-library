...
const TEST_TEXT3 =
  'test another simple text with misspelled words' +
...
function assertCursorAtElement(expectedId) {
  ...
  if (isCaret(range)) {
    ...
    if (
      isCursorAtEndOfStartNode(range) &&
  ...
  assertEquals('The cursor is not at the expected misspelled word.', expectedId, focusedElementId);
}
...
testSuite({
  ...
  testExcludeMarkers() {
    const el = document.getElementById('test1');
    spellChecker.decorate(el);
    spellChecker.setExcludeMarker(['DIV.goog-quote', 'goog-comment', 'SPAN.goog-note']);
    assertArrayEquals(['goog-quote', 'goog-comment', 'goog-note'], spellChecker.excludeMarker);
    assertArrayEquals(
      [String(TagName.DIV), undefined, String(TagName.SPAN)],
      spellChecker.excludeTags
    );
    el.innerHTML =
      '<div class="goog-quote">misspelling</div>' +
  ...
  testKeyboardNavigateNext() {
    ...
    const keyEventProperties = googObject.create('ctrlKey', true, 'shiftKey', false);
    ...
    const defaultExecuted = events.fireKeySequence(el, KeyCodes.RIGHT, keyEventProperties);
  ...
  testKeyboardNavigateOpenSuggestions() {
    ...
    const defaultExecuted = events.fireKeySequence(el, KeyCodes.DOWN, keyEventProperties);
  ...
  testKeyboardNavigatePrevious() {
    ...
    const defaultExecuted = events.fireKeySequence(el, KeyCodes.LEFT, keyEventProperties);
