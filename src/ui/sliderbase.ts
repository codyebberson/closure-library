 ...
 * @private
 */
goog.ui.SliderBase.SLIDER_DRAGGING_CSS_CLASS_ = goog.getCssName('goog-slider-dragging');
 ...
 * @private
 */
goog.ui.SliderBase.THUMB_DRAGGING_CSS_CLASS_ = goog.getCssName('goog-slider-thumb-dragging');
 ...
 * @private
 */
goog.ui.SliderBase.DISABLED_CSS_CLASS_ = goog.getCssName('goog-slider-disabled');
...
goog.ui.SliderBase.prototype.handleBeforeDrag_ = function (e) {
  ...
  if (this.orientation_ == goog.ui.SliderBase.Orientation.VERTICAL) {
    ...
    value =
      ((availHeight - e.top) / availHeight) * (this.getMaximum() - this.getMinimum()) +
      this.getMinimum();
  } else {
...
goog.ui.SliderBase.prototype.handleKeyDown_ = function (e) {
  ...
  switch (e.keyCode) {
    ...
    case goog.events.KeyCodes.LEFT:
      ...
      this.moveThumbs(
        e.shiftKey ? sign * this.getBlockIncrement() : sign * this.getUnitIncrement()
      );
      break;
...
goog.ui.SliderBase.prototype.handleTimerTick_ = function () {
  ...
  if (value !== undefined) {
    // not all code paths sets the value variable
...
goog.ui.SliderBase.prototype.setThumbPosition_ = function (thumb, position) {
  ...
  var end =
    thumb == this.extentThumb
      ? roundedPosition
      : this.rangeModel.getValue() + this.rangeModel.getExtent();
  if (value >= this.getMinimum() && end >= value + this.minExtent_ && this.getMaximum() >= end) {
...
goog.ui.SliderBase.prototype.addRangeHighlightAnimations_ = function (
  ...
  if (this.orientation_ == goog.ui.SliderBase.Orientation.VERTICAL) {
    ...
    var slide = new goog.fx.dom.Slide(
      this.rangeHighlight,
      [this.getOffsetStart_(this.rangeHighlight), previousHighlightPositioning.offset],
      [this.getOffsetStart_(this.rangeHighlight), highlightPositioning.offset],
      goog.ui.SliderBase.ANIMATION_INTERVAL_
    );
    var resizeHeight = new goog.fx.dom.ResizeHeight(
      this.rangeHighlight,
      previousHighlightPositioning.size,
      highlightPositioning.size,
      goog.ui.SliderBase.ANIMATION_INTERVAL_
    );
    slide.enableRightPositioningForRtl(this.flipForRtl_);
 ...
 */
goog.ui.SliderBase.prototype.setFocusElementOnSliderDrag = function (focusElementOnSliderDrag) {
