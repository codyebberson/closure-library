...
function isHighlightedVisible(menu) {
  ...
  contRect = new GoogRect(contRect.left, contRect.top - 1, contRect.width, contRect.height + 2);
...
testSuite({
  ...
  testDecorate() {
    ...
    assertEquals(
      'Caption of menu item should match decorated element',
      'Apple',
      menu.getItemAt(0).getCaption()
    );
    assertEquals(
      'Accelerator of menu item should match accelerator element',
      'A',
  ...
  testFilter() {
    ...
    // Check menu items.
    assertEquals(
      'Family should be the first label in the move to menu',
      'Family',
      menu.getChildAt(0).getCaption()
    );
    assertEquals(
      'Friends should be the second label in the move to menu',
      'Friends',
      menu.getChildAt(1).getCaption()
    );
    assertEquals(
      'Photos should be the third label in the move to menu',
      'Photos',
      menu.getChildAt(2).getCaption()
    );
    assertEquals(
      'Work should be the fourth label in the move to menu',
      'Work',
    ...
    assertTrue('Work should be visible when the menu is filtered', menu.getChildAt(3).isVisible());
    // Check accelerator.
    assertEquals(
      'The accelerator for Work should be present',
      'W',
  ...
  testFilterWordBoundary() {
    ...
    assertFalse(
      'Receipts & Invoices should not be visible when the menu is ' + 'filtered',
      menu.getChildAt(2).isVisible()
    );
    assertFalse(
  ...
  testAriaRoles() {
    ...
    assertEquals(AutoCompleteValues.LIST, aria.getState(input, State.AUTOCOMPLETE));
