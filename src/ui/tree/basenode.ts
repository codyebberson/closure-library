 ...
 */
goog.ui.tree.BaseNode.prototype.deselect = () => {};
...
goog.ui.tree.BaseNode.prototype.setExpanded = function (expanded) {
  ...
  if (isStateChange) {
    this.dispatchEvent(
      expanded ? goog.ui.tree.BaseNode.EventType.EXPAND : goog.ui.tree.BaseNode.EventType.COLLAPSE
    );
  }
};
...
goog.ui.tree.BaseNode.prototype.toSafeHtml = function () {
  const tree = this.getTree();
  const hideLines = !tree.getShowLines() || (tree == this.getParent() && !tree.getShowRootLines());

  const childClass = hideLines ? this.config_.cssChildrenNoLines : this.config_.cssChildren;
...
goog.ui.tree.BaseNode.prototype.getRowSafeHtml = function () {
  ...
  const content = [this.getExpandIconSafeHtml(), this.getIconSafeHtml(), this.getLabelSafeHtml()];
...
goog.ui.tree.BaseNode.prototype.getBackgroundPosition = function () {
  return (
    (this.isLastSibling() ? '-100' : (this.getDepth() - 1) * this.config_.indentWidth) + 'px 0'
  );
};
