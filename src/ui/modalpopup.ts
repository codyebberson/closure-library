 ...
 * @protected
 */
goog.ui.ModalPopup.prototype.getCssClass = () => goog.getCssName('goog-modalpopup');
...
goog.ui.ModalPopup.prototype.manageBackgroundDom_ = function () {
  ...
  if (!this.bgEl_) {
    this.bgEl_ = this.getDomHelper().createDom(
      ...
      goog.getCssName(this.getCssClass(), 'bg')
    );
...
/** @override */
goog.ui.ModalPopup.prototype.canDecorate = (element) => {
 ...
 */
goog.ui.ModalPopup.prototype.setCenterInsideParentElement = function (centerInsideParent) {
  if (this.isInDocument()) {
    throw new Error("Can't set parent element after component is already in document.");
  }
...
goog.ui.ModalPopup.prototype.show_ = function () {
  ...
  if (this.centerInsideParent_ && window.ResizeObserver !== undefined) {
    ...
    this.parentElementResizeObserver_.observe(goog.asserts.assert(this.getElement().parentElement));
    this.getHandler().listen(
    ...
    this.getHandler()
      .listen(this.getDomHelper().getWindow(), goog.events.EventType.RESIZE, this.resizeBackground_)
      .listen(
        this.getDomHelper().getWindow(),
        goog.events.EventType.ORIENTATIONCHANGE,
...
goog.ui.ModalPopup.prototype.hide_ = function () {
  ...
  // popup is hidden.
  this.getHandler()
    .unlisten(this.getDomHelper().getWindow(), goog.events.EventType.RESIZE, this.resizeBackground_)
    .unlisten(
      this.getDomHelper().getWindow(),
      goog.events.EventType.ORIENTATIONCHANGE,
      this.resizeBackgroundTask_
