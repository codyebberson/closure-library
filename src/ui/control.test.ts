...
function getListenerCount(control) {
  return control.googUiComponentHandler_ ? googObject.getCount(control.getHandler().keys_) : 0;
}
...
function assertClickSequenceFires(msg) {
  ...
  assertEquals(msg, actionCount + 1, getEventCount(control, Component.EventType.ACTION));
}
...
function assertIsolatedClickDoesNotFire(msg) {
  const actionCount = getEventCount(control, Component.EventType.ACTION);
  testingEvents.fireClickEvent(control.getKeyEventTarget());
  assertEquals(msg, actionCount, getEventCount(control, Component.EventType.ACTION));
}
...
testSuite({
  ...
  testConstructor() {
    ...
    assertEquals('Content must have expected value', 'Hello', control.getContent());
    assertEquals(
      'Renderer must default to the registered renderer',
      registry.getDefaultRenderer(Control),
      control.getRenderer()
    );
    ...
    const content = dom.createDom(
      ...
      null,
      'Hello',
      dom.createDom(TagName.B, null, 'World')
    );
    ...
    assertEquals('Renderer must have expected value', testRenderer, foo.getRenderer());
    assertEquals('DOM helper must have expected value', fakeDomHelper, foo.getDomHelper());
    foo.dispose();
  },
  ...
  testGetHandler() {
    assertUndefined(
      'Event handler must be undefined before getHandler() ' + 'is called',
    ...
    assertEquals(
      'getHandler() must return the same instance if called again',
      handler,
      control.getHandler()
    );
  },
  ...
  testSetHandleMouseEvents() {
    ...
    assertTrue('Rendered control must handle its own mouse events', control.isHandleMouseEvents());
    control.setHandleMouseEvents(false);
    assertFalse(
  ...
  testGetKeyEventTarget() {
    assertNull('Key event target of control without DOM must be null', control.getKeyEventTarget());
    control.createDom();
    assertEquals(
  ...
  testGetKeyHandler() {
    assertUndefined(
      'Key handler must be undefined before getKeyHandler() ' + 'is called',
      control.keyHandler_
    );
    /** @suppress {visibility} suppression added to enable type checking */
    ...
    assertEquals(
      'getKeyHandler() must return the same instance if called ' + 'again',
      keyHandler,
      control.getKeyHandler()
    );
  },
  ...
  testSetRenderer() {
    ...
    assertNull('Control must not have a DOM after its renderer is reset', control.getElement());
    assertFalse('Control still must not be in the document', control.isInDocument());
    assertEquals('Renderer must have expected value', testRenderer, control.getRenderer());

    control.render(sandbox);
    assertTrue('Control must be in the document', control.isInDocument());

    assertThrows(
      'Resetting the renderer after the control has entered ' + 'the document must throw error',
      () => {
        control.setRenderer({});
      }
    );
  ...
  testAddRemoveClassName() {
    ...
    assertArrayEquals(
      'Control must have expected extra class names',
      ['foo'],
      control.getExtraClassNames()
    );
    ...
    assertNull('Control must not have any extra class names', control.getExtraClassNames());
    assertSameElements(
      "Control's element must have expected class names",
      ['goog-control'],
      classlist.get(control.getElement())
  ...
  testEnableClassName() {
    ...
    control.enableClassName('bar', true);
    assertArrayEquals(
      'Enabling the same class name again must be a no-op',
      ['foo', 'bar'],
      control.getExtraClassNames()
    );
    ...
    assertArrayEquals(
      'Enabling null class name must be a no-op',
      ['foo', 'bar'],
      control.getExtraClassNames()
    );
  ...
  testCreateDom() {
    ...
    assertFalse('Control must not allow text selection by default', control.isAllowTextSelection());
    ...
    assertTrue("Control's element must be visible", control.getElement().style.display != 'none');

    control.setAllowTextSelection(true);
    control.createDom();
    assertFalse("Control's element must be selectable", style.isUnselectable(control.getElement()));

    control.setVisible(false);
    control.createDom();
    assertTrue("Control's element must be hidden", control.getElement().style.display == 'none');
  },

  /** Tests {@link Control#getContentElement}. */
  testGetContentElement() {
    assertNull('Unrendered control must not have a content element', control.getContentElement());
    control.createDom();
    assertEquals(
  ...
  testDecorateInternal() {
    ...
    assertEquals("Decorated control's element must have expected value", foo, control.getElement());
    assertTrue('Element must be unselectable', style.isUnselectable(control.getElement()));
    assertTrue('Element must be visible', control.getElement().style.display != 'none');
  },
  ...
  testDecorateInternalForSelectableControl() {
    ...
    control.decorate(foo);
    assertEquals("Decorated control's element must have expected value", foo, control.getElement());
    assertFalse('Element must be selectable', style.isUnselectable(control.getElement()));
  ...
  testEnterDocument() {
    ...
    assertTrue('Control must be in the document', control.isInDocument());
    assertEquals(
      'Control must have 5 mouse and 3 key event listeners',
      8,
      getListenerCount(control)
    );
    assertEquals(
      "Control's key event handler must be attached to its " + 'key event target',
      control.getKeyEventTarget(),
  ...
  testEnterDocumentForControlWithoutMouseHandling() {
    ...
    control.render(sandbox);
    assertTrue('Control must be in the document', control.isInDocument());
    assertEquals('Control must have 3 key event listeners', 3, getListenerCount(control));
    assertEquals(
      "Control's key event handler must be attached to its " + 'key event target',
      control.getKeyEventTarget(),
      control.getKeyHandler().element_
  ...
  testEnterDocumentForNonFocusableControl() {
    ...
    control.render(sandbox);
    assertTrue('Control must be in the document', control.isInDocument());
    assertEquals('Control must have 5 mouse event listeners', 5, getListenerCount(control));
    assertUndefined('Control must not have a key event handler', control.keyHandler_);
  },
  ...
  testEnterDocumentForControlWithoutEventHandlers() {
    ...
    control.render(sandbox);
    assertTrue('Control must be in the document', control.isInDocument());
    assertEquals('Control must have 0 event listeners', 0, getListenerCount(control));
    assertUndefined('Control must not have an event handler', control.googUiComponentHandler_);
    assertUndefined('Control must not have a key event handler', control.keyHandler_);
  },
  ...
  testExitDocument() {
    ...
    assertFalse('Control must no longer be in the document', control.isInDocument());
    assertEquals('Control must have no event listeners', 0, getListenerCount(control));
    assertNull("Control's key event handler must be unattached", control.getKeyHandler().element_);
    assertFalse(
  ...
  testDispose() {
    ...
    assertUndefined('Event handler must have been deleted', control.googUiComponentHandler_);
    ...
    assertNull('Extra class names must have been nulled out', control.getExtraClassNames());
  },

  /** Tests {@link Control#getContent}. */
  testGetContent() {
    assertNull('Empty control must have null content', new Control(null).getContent());
    assertEquals('Control must have expected content', 'Hello', control.getContent());
    control.render(sandbox);
    assertEquals(
      'Control must have expected content after rendering',
      'Hello',
      control.getContent()
    );
  },
  ...
  testGetContentForDecoratedControl() {
    ...
    assertNull('Content of control decorating empty DIV must be null', empty.getContent());
    empty.dispose();

    const text = new Control(null);
    text.decorate(dom.getElement('text'));
    assertEquals(
      'Content of control decorating DIV with text contents ' + 'must be as expected',
      'Hello, world!',
      text.getContent().nodeValue
    );
    text.dispose();
    ...
    assertEquals(
      'Content of control decorating DIV with element child ' + 'must be as expected',
      dom.getElement('element').firstChild,
      element.getContent()
    );
    element.dispose();
    ...
    assertSameElements(
      'Content of control decorating DIV with mixed ' + 'contents must be as expected',
      dom.getElement('nodelist').childNodes,
      nodelist.getContent()
    );
    nodelist.dispose();
  },
  ...
  testSetAriaLabel_render() {
    assertNull('Controls must not have any aria label by default', control.getAriaLabel());
    ...
    assertEquals('Element must have the new aria label', 'new label', aria.getLabel(elem));
  },
  ...
  testSetAriaLabel_decorate() {
    ...
    sandbox.innerHTML = '<div id="nodelist" role="button">' + 'Hello, <b>world</b>!</div>';
    ...
    assertEquals('Element must have the correct role', 'button', elem.getAttribute('role'));
  ...
  testSetContent() {
    ...
    assertEquals(
      'Unrendered control control must have expected contents',
      'Bye',
      control.getContent()
    );
    ...
    assertNull('Rendered control must have expected contents', control.getContent());
    assertEquals(
      "Rendered control's DOM must have expected contents",
      '',
      control.getElement().innerHTML
    ...
    control.setContent([
      dom.createDom(TagName.DIV, null, dom.createDom(TagName.SPAN, null, 'Hello')),
      'World',
    ]);
    assertHTMLEquals(
      "Control's DOM must be updated",
      '<div><span>Hello</span></div>World',
      control.getElement().innerHTML
    );
  },
  ...
  testSetContentInternal() {
    ...
    assertEquals('Control must have expected contents', 'Bye', control.getContent());
    assertEquals("Control's DOM must be unchanged", 'Hello', control.getElement().innerHTML);
  },
  ...
  testGetCaption() {
    ...
    assertEquals('Caption must have expected value', 'Hello', control.getCaption());

    sandbox.innerHTML = '<div id="nodelist">Hello, <b>world</b>!</div>';
    control.decorate(dom.getElement('nodelist'));
    assertEquals('Caption must have expected value', 'Hello, world!', control.getCaption());

    const arrayContent = googArray.clone(
      dom.safeHtmlToNode(testing.newSafeHtmlForTest(' <b> foo</b><i>  bar</i> ')).childNodes
    );
    control.setContent(arrayContent);
    assertEquals('whitespaces must be normalized in the caption', 'foo bar', control.getCaption());

    control.setContent('\xa0foo');
    assertEquals('indenting spaces must be kept', '\xa0foo', control.getCaption());
  },
  ...
  testSetCaption() {
    ...
    assertEquals(
      'Control must have a string caption "Hello, world!"',
      'Hello, world!',
      control.getCaption()
    );
  },
  ...
  testSetRightToLeft() {
    ...
    control.render(sandbox);
    assertThrows(
      'Changing the render direction of a control already in ' + 'the document is an error',
      () => {
  ...
  testSetAllowTextSelection() {
    ...
    control.setAllowTextSelection(true);
    assertTrue('Control must allow text selection', control.isAllowTextSelection());

    control.setAllowTextSelection(false);
    assertFalse('Control must no longer allow text selection', control.isAllowTextSelection());
    ...
    control.setAllowTextSelection(true);
    assertTrue('Control must once again allow text selection', control.isAllowTextSelection());
  },
  ...
  testSetVisible() {
    assertFalse('setVisible(true) must return false if already visible', control.setVisible(true));
    ...
    assertEquals(
      'One HIDE event must have been dispatched',
      1,
      getEventCount(control, Component.EventType.HIDE)
    );
    assertFalse('Control must no longer be visible', control.isVisible());

    assertTrue('setVisible(true) must return true if previously hidden', control.setVisible(true));
    assertEquals(
      'One SHOW event must have been dispatched',
      1,
  ...
  testSetVisibleForRenderedControl() {
    control.render(sandbox);
    assertTrue('No events must have been dispatched during rendering', noEventsDispatched());
  ...
  testSetVisibleForDisabledNonFocusableControl() {
    ...
    assertEquals(
      'Control must not have dispatched FOCUS',
      0,
    ...
    assertEquals(
      'Control must have dispatched HIDE',
      1,
      getEventCount(control, Component.EventType.HIDE)
    );
    assertEquals(
      'Control must not have dispatched BLUR',
      0,
  ...
  testSetVisibleForEnabledFocusableControl() {
    ...
    if (testFocus) {
      ...
      try {
        ...
        assertEquals(
          'Control must have dispatched FOCUS',
          1,
          getEventCount(control, Component.EventType.FOCUS)
        );
      ...
      try {
        // IE dispatches focus and blur events asynchronously!
        assertEquals(
          'Control must have dispatched BLUR',
          1,
          getEventCount(control, Component.EventType.BLUR)
        );
        assertFalse('Control must no longer have keyboard focus', control.isFocused());
      } catch (e) {
        expectedFailures.handleException(e);
      }
    }
  ...
  testSetEnabled() {
    ...
    assertEquals(
      'Control element must have a tabIndex of 0',
      0,
    ...
    control.setEnabled(false);
    assertEquals(
      'One DISABLE event must have been dispatched',
      1,
    ...
    assertEquals(
      'Control element must have aria-disabled true',
      'true',
    ...
    assertEquals(
      'Control element must have aria-disabled false',
      'false',
      aria.getState(elem, State.DISABLED)
    );
    assertEquals(
      'Control element must have tabIndex 0',
      0,
      googString.toNumber(elem.getAttribute('tabIndex') || '')
    );
  },
  ...
  testSetStateWithDisabled() {
    ...
    assertTrue(
      'Control element must not have aria-disabled',
      googString.isEmptyOrWhitespace(aria.getState(control.getElementStrict(), State.DISABLED))
    );
    assertEquals(
      'Control element must have a tabIndex of 0',
      0,
      googString.toNumber(control.getElement().getAttribute('tabIndex') || '')
    );
  ...
  testSetEnabledWithParent() {
    ...
    assertTrue('No events must have been dispatched when child is disabled', noEventsDispatched());
    ...
    control.setEnabled(true);
    assertEquals(
      'One ENABLE event must have been dispatched by the parent',
      1,
    ...
    child.setEnabled(false);
    assertEquals(
      'One DISABLE event must have been dispatched by the child',
      1,
    ...
    control.setEnabled(false);
    assertEquals(
      'One DISABLE event must have been dispatched by the parent',
      1,
      getEventCount(control, Component.EventType.DISABLE)
    );
  ...
  testIsHighlighted() {
    assertFalse('Controls must not be highlighted by default', control.isHighlighted());
  },
  ...
  testSetHighlighted() {
    ...
    control.setHighlighted(true);
    assertFalse(
      "Control must not be highlighted, because it isn't " + 'highlightable',
      control.isHighlighted()
    );
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertTrue('Control must be highlighted', control.isHighlighted());
    assertEquals(
      'Control must have dispatched a HIGHLIGHT event',
      1,
    ...
    assertEquals(
      'Control must not dispatch more HIGHLIGHT events',
      1,
      getEventCount(control, Component.EventType.HIGHLIGHT)
    );
    ...
    assertFalse('Control must not be highlighted', control.isHighlighted());
    assertEquals(
      'Control must have dispatched an UNHIGHLIGHT event',
      1,
    ...
    control.setHighlighted(true);
    assertTrue('Control must be highlighted, even when disabled', control.isHighlighted());
    assertEquals(
      'Control must have dispatched another HIGHLIGHT event',
      2,
      getEventCount(control, Component.EventType.HIGHLIGHT)
    );
  },
  ...
  testSetActive() {
    ...
    control.setActive(true);
    assertFalse("Control must not be active, because it isn't activateable", control.isActive());
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertTrue('Control must be active', control.isActive());
    assertEquals(
      'Control must have dispatched an ACTIVATE event',
      1,
    ...
    assertEquals(
      'Control must not dispatch more ACTIVATE events',
      1,
      getEventCount(control, Component.EventType.ACTIVATE)
    );
    ...
    assertFalse('Control must not be active', control.isActive());
    assertEquals(
      'Control must have dispatched a DEACTIVATE event',
      1,
  ...
  testSetSelected() {
    ...
    assertFalse("Control must not be selected, because it isn't selectable", control.isSelected());
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertEquals(
      'Control must have dispatched a SELECT event',
      1,
    ...
    assertEquals(
      'Control must not dispatch more SELECT events',
      1,
      getEventCount(control, Component.EventType.SELECT)
    );
    ...
    assertEquals(
      'Control must have dispatched an UNSELECT event',
      1,
    ...
    control.setSelected(true);
    assertTrue('Control must be selected, even when disabled', control.isSelected());
    assertEquals(
      'Control must have dispatched another SELECT event',
      2,
      getEventCount(control, Component.EventType.SELECT)
    );
  },
  ...
  testSetChecked() {
    ...
    assertFalse("Control must not be checked, because it isn't checkable", control.isChecked());
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertEquals(
      'Control must have dispatched a CHECK event',
      1,
    ...
    assertEquals(
      'Control must not dispatch more CHECK events',
      1,
      getEventCount(control, Component.EventType.CHECK)
    );
    ...
    assertEquals(
      'Control must have dispatched an UNCHECK event',
      1,
    ...
    control.setChecked(true);
    assertTrue('Control must be checked, even when disabled', control.isChecked());
    assertEquals(
      'Control must have dispatched another CHECK event',
      2,
      getEventCount(control, Component.EventType.CHECK)
    );
  },
  ...
  testSetFocused() {
    ...
    assertFalse("Control must not be focused, because it isn't focusable", control.isFocused());
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertTrue('Control must be focused', control.isFocused());
    assertEquals(
      'Control must have dispatched a FOCUS event',
      1,
      getEventCount(control, Component.EventType.FOCUS)
    );
    ...
    assertEquals(
      'Control must not dispatch more FOCUS events',
      1,
      getEventCount(control, Component.EventType.FOCUS)
    );
    ...
    assertFalse('Control must not be focused', control.isFocused());
    assertEquals(
      'Control must have dispatched an BLUR event',
      1,
      getEventCount(control, Component.EventType.BLUR)
    );
    control.setEnabled(false);
    ...
    control.setFocused(true);
    assertTrue('Control must be focused, even when disabled', control.isFocused());
    assertEquals(
      'Control must have dispatched another FOCUS event',
      2,
      getEventCount(control, Component.EventType.FOCUS)
    );
  },
  ...
  testSetOpen() {
    ...
    assertFalse("Control must not be opened, because it isn't openable", control.isOpen());
    assertTrue('Control must not have dispatched any events', noEventsDispatched());
    ...
    assertEquals(
      'Control must have dispatched a OPEN event',
      1,
    ...
    assertEquals(
      'Control must not dispatch more OPEN events',
      1,
      getEventCount(control, Component.EventType.OPEN)
    );
    ...
    assertEquals(
      'Control must have dispatched an CLOSE event',
      1,
    ...
    assertEquals(
      'Control must have dispatched another OPEN event',
      2,
      getEventCount(control, Component.EventType.OPEN)
    );
  },
  ...
  testGetState() {
    assertEquals('Controls must be in the default state', 0x00, control.getState());
  },
   ...
   */
  testHasState() {
    assertFalse('Control must not be disabled', control.hasState(Component.State.DISABLED));
    assertFalse('Control must not be in the HOVER state', control.hasState(Component.State.HOVER));
    assertFalse('Control must not be active', control.hasState(Component.State.ACTIVE));
    assertFalse('Control must not be selected', control.hasState(Component.State.SELECTED));
    assertFalse('Control must not be checked', control.hasState(Component.State.CHECKED));
    assertFalse('Control must not be focused', control.hasState(Component.State.FOCUSED));
    assertFalse('Control must not be open', control.hasState(Component.State.OPEN));
  },
  ...
  testSetState() {
    ...
    assertFalse(
      "Control must still be inactive (because it doesn't " + 'support the ACTIVE state)',
    ...
    control.setState(Component.State.ACTIVE, true);
    assertTrue('Control must be active', control.hasState(Component.State.ACTIVE));
    assertTrue(
    ...
    control.setState(Component.State.ACTIVE, true);
    assertTrue('Control must still be active', control.hasState(Component.State.ACTIVE));
    assertTrue(
  ...
  testIsSupportedState() {
    assertTrue('Control must support DISABLED', control.isSupportedState(Component.State.DISABLED));
    assertTrue('Control must support HOVER', control.isSupportedState(Component.State.HOVER));
    assertTrue('Control must support ACTIVE', control.isSupportedState(Component.State.ACTIVE));
    assertTrue('Control must support FOCUSED', control.isSupportedState(Component.State.FOCUSED));
    assertFalse(
    ...
    assertFalse('Control must no support OPENED', control.isSupportedState(Component.State.OPENED));
  },

  /** Tests {@link Control#setSupportedState}. */
  testSetSupportedState() {
    control.setSupportedState(Component.State.HOVER, true);
    assertTrue('Control must still support HOVER', control.isSupportedState(Component.State.HOVER));
  ...
  testSetAutoStates() {
    ...
    control.setAutoStates(Component.State.ACTIVE | Component.State.FOCUSED, false);
    assertFalse(
    ...
    assertFalse(
      'Control must not have FOCUSED as an auto-state if it no ' + 'longer supports FOCUSED',
      control.isAutoState(Component.State.FOCUSED)
    );
  ...
  testIsTransitionAllowed() {
    ...
    assertEquals(
      'Control must have dispatched one HIGHLIGHT event',
      1,
      getEventCount(control, Component.EventType.HIGHLIGHT)
    );
    assertFalse('Control must not be highlighted', control.hasState(Component.State.HOVER));
    ...
    assertEquals(
      'Control must not have dispatched any UNHIGHLIGHT events',
      0,
      getEventCount(control, Component.EventType.UNHIGHLIGHT)
    );
    assertTrue('Control must still be highlighted', control.hasState(Component.State.HOVER));
    ...
    assertFalse('Control must not be FOCUSED', control.hasState(Component.State.FOCUSED));
    assertFalse(
      'Control must not be allowed to transition to the FOCUSED ' + 'state',
      control.isTransitionAllowed(Component.State.FOCUSED, true)
    );
    assertEquals(
      'Control must not have dispatched any FOCUS events',
      0,
      getEventCount(control, Component.EventType.FOCUS)
    );
    ...
    assertTrue('Control must be DISABLED', control.hasState(Component.State.DISABLED));
    assertFalse(
    ...
    assertEquals(
      'Control must not have dispatched any ENABLE events',
      0,
      getEventCount(control, Component.EventType.ENABLE)
    );
  },
  ...
  testHandleKeyEvent() {
    ...
    assertEquals(
      'Control must not have dispatched an ACTION event',
      0,
    ...
    assertEquals(
      'Control must have dispatched an ACTION event',
      1,
      getEventCount(control, Component.EventType.ACTION)
    );
  },
  ...
  testPerformActionInternal() {
    ...
    assertEquals(
      'Control must have dispatched another ACTION event',
      2,
      getEventCount(control, Component.EventType.ACTION)
    );
    ...
    assertEquals(
      'Control must not have dispatched an UNSELECT event',
      0,
      getEventCount(control, Component.EventType.UNSELECT)
    );
    assertEquals(
      'Control must have dispatched a CLOSE event',
      1,
      getEventCount(control, Component.EventType.CLOSE)
    );
    assertEquals(
  ...
  testHandleMouseOver() {
    ...
    assertTrue('Caption must be contained within the control', dom.contains(element, caption));
    assertTrue('Control must be enabled', control.isEnabled());
    assertTrue('HOVER must be an auto-state', control.isAutoState(Component.State.HOVER));
    assertFalse('Control must not start out highlighted', control.isHighlighted());

    // Scenario 1:  relatedTarget is contained within the control's DOM.
    testingEvents.fireMouseOverEvent(element, caption);
    assertTrue('No events must have been dispatched for internal mouse move', noEventsDispatched());
    assertFalse('Control must not be highlighted for internal mouse move', control.isHighlighted());
    resetEventCount();
    ...
    assertEquals(
      'Control must have dispatched 1 ENTER event',
      1,
      getEventCount(control, Component.EventType.ENTER)
    );
    assertFalse('Control must not be highlighted if ENTER is canceled', control.isHighlighted());
    ...
    testingEvents.fireMouseOverEvent(element, sandbox);
    assertEquals(
      'Control must dispatch ENTER event on mouseover even if ' + 'disabled',
      1,
      getEventCount(control, Component.EventType.ENTER)
    );
    assertFalse('Control must not be highlighted if it is disabled', control.isHighlighted());
    control.setEnabled(true);
    resetEventCount();
    ...
    testingEvents.fireMouseOverEvent(element, sandbox);
    assertEquals(
      'Control must dispatch ENTER event on mouseover even if ' + 'HOVER is not an auto-state',
      1,
      getEventCount(control, Component.EventType.ENTER)
    );
    assertFalse(
      "Control must not be highlighted if HOVER isn't an auto-" + 'state',
      control.isHighlighted()
    );
    ...
    testingEvents.fireMouseOverEvent(element, sandbox);
    assertEquals(
      'Control must dispatch ENTER event on mouseover',
      1,
      getEventCount(control, Component.EventType.ENTER)
    );
    assertEquals(
      'Control must dispatch HIGHLIGHT event on mouseover',
      1,
      getEventCount(control, Component.EventType.HIGHLIGHT)
    );
    assertTrue('Control must be highlighted', control.isHighlighted());
  ...
  testHandleMouseOut() {
    ...
    assertTrue('Control must be enabled', control.isEnabled());
    assertTrue('HOVER must be an auto-state', control.isAutoState(Component.State.HOVER));
    assertTrue('ACTIVE must be an auto-state', control.isAutoState(Component.State.ACTIVE));
    ...
    assertTrue('Control must not be deactivated for internal mouse move', control.isActive());
    resetEventCount();
    ...
    assertEquals(
      'Control must have dispatched 1 LEAVE event',
      1,
      getEventCount(control, Component.EventType.LEAVE)
    );
    assertTrue('Control must not be un-highlighted if LEAVE is canceled', control.isHighlighted());
    assertTrue('Control must not be deactivated if LEAVE is canceled', control.isActive());
    googEvents.unlistenByKey(key);
    resetEventCount();
    ...
    testingEvents.fireMouseOutEvent(element, sandbox);
    assertEquals(
      'Control must dispatch LEAVE event on mouseout even if ' + 'ACTIVE is not an auto-state',
      1,
      getEventCount(control, Component.EventType.LEAVE)
    );
    assertTrue(
      "Control must not be deactivated if ACTIVE isn't an auto-" + 'state',
      control.isActive()
    );
    assertFalse(
      "Control must be un-highlighted even if ACTIVE isn't an " + 'auto-state',
      control.isHighlighted()
    );
    ...
    testingEvents.fireMouseOutEvent(element, sandbox);
    assertEquals(
      'Control must dispatch LEAVE event on mouseout even if ' + 'HOVER is not an auto-state',
      1,
      getEventCount(control, Component.EventType.LEAVE)
    );
    assertFalse(
      "Control must be deactivated even if HOVER isn't an auto-" + 'state',
      control.isActive()
    );
    assertTrue(
      "Control must not be un-highlighted if HOVER isn't an auto-" + 'state',
      control.isHighlighted()
    );
    control.setAutoStates(Component.State.HOVER, true);
    control.setActive(true);
    ...
    testingEvents.fireMouseOutEvent(element, sandbox);
    assertEquals(
      'Control must dispatch LEAVE event on mouseout',
      1,
      getEventCount(control, Component.EventType.LEAVE)
    );
    assertEquals(
      'Control must dispatch DEACTIVATE event on mouseout',
      1,
      getEventCount(control, Component.EventType.DEACTIVATE)
    );
    assertEquals(
      'Control must dispatch UNHIGHLIGHT event on mouseout',
      1,
      getEventCount(control, Component.EventType.UNHIGHLIGHT)
    );
  ...
  testIsMouseEventWithinElement() {
    ...
    assertTrue('Event is within element', Control.isMouseEventWithinElement_(event, parent));

    event = new GoogTestingEvent('mouseout');
    event.relatedTarget = notChild;
    assertFalse('Event is not within element', Control.isMouseEventWithinElement_(event, parent));
  },

  testHandleMouseDown() {
    control.render(sandbox);
    assertFalse(
      'preventDefault() must have been called for control that ' + "doesn't support text selection",
  ...
  testHandleMouseDownForRightMouseButton() {
    control.render(sandbox);
    assertTrue(
      'preventDefault() must not have been called for right ' + 'mouse button',
      fireMouseDownAndFocus(control.getElement(), BrowserEvent.MouseButton.RIGHT)
    );
    assertTrue('Control must be highlighted', control.isHighlighted());
    assertFalse('Control must not be active', control.isActive());
  ...
  testHandleMouseUp() {
    ...
    assertFalse('Disabled control must not highlight on mouseup', control.isHighlighted());
    ...
    testingEvents.fireMouseUpEvent(element);
    assertFalse(
      "Control must not highlight on mouseup if HOVER isn't an " + 'auto-state',
      control.isHighlighted()
    );
    assertTrue(
      "Action must have been performed even if HOVER isn't an " + 'auto-state',
      actionPerformed
    );
    assertFalse(
      'Control must have been deactivated on mouseup even if ' + "HOVER isn't an auto-state",
      control.isActive()
    );
    ...
    testingEvents.fireMouseUpEvent(element);
    assertTrue('Control must highlight on mouseup, even if inactive', control.isHighlighted());
    assertFalse('No action must have been performed if control is inactive', actionPerformed);
    assertFalse('Inactive control must remain inactive after mouseup', control.isActive());
    control.setHighlighted(false);
    control.setActive(true);
    ...
    testingEvents.fireMouseUpEvent(element);
    assertTrue(
      'Control must highlight on mouseup, even if no action is ' + 'performed',
      control.isHighlighted()
    );
    assertTrue('performActionInternal must have been called', actionPerformed);
    assertTrue(
      'Control must not deactivate if performActionInternal ' + 'returns false',
      control.isActive()
    );
    control.setHighlighted(false);
    actionPerformed = false;
    ...
    testingEvents.fireMouseUpEvent(element);
    assertTrue(
      "Control must highlight on mouseup even if ACTIVE isn't an " + 'auto-state',
      control.isHighlighted()
    );
    assertTrue(
      "Action must have been performed even if ACTIVE isn't an " + 'auto-state',
      actionPerformed
    );
    assertTrue(
      'Control must not have been deactivated on mouseup if ' + "ACTIVE isn't an auto-state",
      control.isActive()
    );
    actionPerformed = false;
    control.setHighlighted(false);
  ...
  testIeMouseEventSequenceSimulator() {
    ...
    assertClickSequenceFires('ACTION event expected after a second consecutive click sequence');
    if (userAgent.IE) {
      assertIsolatedClickFires(
        'ACTION event expected after an isolated click immediately ' + 'following a click sequence'
      );
      assertIsolatedClickFires('ACTION event expected after second consecutive isolated click');
    } else {
    ...
    assertClickSequenceFires(
      'ACTION event expected after click sequence immediately following ' + 'an isolated click '
    );
  },
  ...
  testIeMouseEventSequenceSimulatorStrictMode() {
    ...
    if (userAgent.IE) {
      assertEquals(
        'ACTION event expected after an isolated click',
        actionCount + 1,
        getEventCount(control, Component.EventType.ACTION)
      );
    } else {
      assertEquals(
        'No ACTION event expected after an isolated click',
        actionCount,
        getEventCount(control, Component.EventType.ACTION)
      );
    }
  ...
  testSetPointerEventsEnabled() {
    ...
    assertFalse('Control should not be active before pointerdown event.', control.isActive());
    ...
    assertTrue('Control should be active after pointerdown event.', control.isActive());

    const pointerup = new GoogTestingEvent(
      PointerFallbackEventType.POINTERUP,
      control.getElement()
    );
    pointerup.button = BrowserEvent.MouseButton.LEFT;
    testingEvents.fireBrowserEvent(pointerup);

    assertFalse('Control should not be active after pointerup event.', control.isActive());
  },
});
