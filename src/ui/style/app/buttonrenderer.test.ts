...
testSuite({
  ...
  testDecoratedButtonBoxWithSpaceInContent() {
    ...
    if (userAgent.IE && !userAgent.isDocumentModeOrHigher(9)) {
      assertEquals('Hello Decorated Box with space ', button.getContentElement().innerHTML);
    } else {
      assertEquals(
  ...
  testExistingContentIsUsed() {
    ...
    assertEquals('Unexpected number of child nodes', 3, content.childNodes.length);
    assertEquals('Unexpected tag', 'STRONG', content.childNodes[0].tagName);
    assertEquals('Unexpected text content', 'Hello', content.childNodes[0].innerHTML);
