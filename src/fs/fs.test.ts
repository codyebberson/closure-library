...
const fsExists =
  globalThis.requestFileSystem !== undefined || globalThis.webkitRequestFileSystem !== undefined;
/** @suppress {checkTypes} suppression added to enable type checking */
const deferredFs = fsExists ? googFs.getTemporary() : null;
const stubs = new PropertyReplacer();

function loadTestDir() {
  return deferredFs.then((fs) =>
    fs.getRoot().getDirectory(TEST_DIR, FsDirectoryEntry.Behavior.CREATE)
  );
}
...
function checkFileRemoved(filename) {
  return loadFile(filename).then(goog.partial(fail, 'expected file to be removed'), (err) => {
...
testSuite({
  setUpPage() {
    ...
    return loadTestDir().then(null, (err) => {
      ...
      if (err.code == FsError.ErrorCode.QUOTA_EXCEEDED) {
        msg =
          err.message +
          ". If you're using Chrome, you probably need to " +
          'pass --unlimited-quota-for-files on the command line.';
      } else if (err.code == FsError.ErrorCode.SECURITY && window.location.href.match(/^file:/)) {
        msg = err.message + ". file:// URLs can't access the filesystem API.";
      } else {
        msg = err.message;
      }
      const body = dom.getDocument().body;
      dom.insertSiblingBefore(dom.createDom(TagName.H1, {}, msg), body.childNodes[0]);
    });
  },
  ...
  testIsFile() {
    ...
    return loadFile('test', FsDirectoryEntry.Behavior.CREATE).then((fileEntry) => {
  ...
  testIsDirectory() {
    ...
    return loadDirectory('test', FsDirectoryEntry.Behavior.CREATE).then((fileEntry) => {
  ...
  testReadFileAsArrayBuffer() {
    ...
    return loadFile('test', FsDirectoryEntry.Behavior.CREATE)
      .then(goog.partial(writeToFile, arr.buffer))
      .then(goog.partial(checkFileContentAs, arr.buffer, 'ArrayBuffer', undefined));
  },
  ...
  testMoveFile() {
    ...
    const deferredSubdir = loadDirectory('subdir', FsDirectoryEntry.Behavior.CREATE);
    const deferredWrittenFile = loadFile('test', FsDirectoryEntry.Behavior.CREATE).then(
  ...
  testCopyFile() {
    ...
    const deferredSubdir = loadDirectory('subdir', FsDirectoryEntry.Behavior.CREATE);
    const deferredWrittenFile = deferredFile.then(goog.partial(writeToFile, 'test content'));
    return GoogPromise.all([deferredSubdir, deferredWrittenFile])
  ...
  testListDirectory() {
    ...
    return loadDirectory('testDir', FsDirectoryEntry.Behavior.CREATE)
      ...
      .then((entries) => {
        ...
        const dir = googArray.find(entries, (entry) => entry.getName() == 'testDir');
        assertNotNull(dir);
        assertTrue(dir.isDirectory());

        const file = googArray.find(entries, (entry) => entry.getName() == 'testFile');
        assertNotNull(file);
  ...
  testGetBlobThrowsError() {
    ...
    try {
      ...
      assertEquals("This browser doesn't seem to support creating Blobs", e.message);
    }
  ...
  testGetBlobWithProperties() {
    ...
    const blob = googFsBlob.getBlobWithProperties(['test'], 'text/test', 'native');
