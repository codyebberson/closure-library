 ...
 */
goog.vec.mat4d.create = () => new Float64Array(16);
 ...
 *     chained together.
 */
goog.vec.mat4d.setFromMat4d = (mat, src) => {
  mat[0] = src[0];
  mat[1] = src[1];
  mat[2] = src[2];
  mat[3] = src[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.setFromMat4f = (mat, src) => {
  mat[0] = src[0];
  mat[1] = src[1];
  mat[2] = src[2];
  mat[3] = src[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.setFromArray = (mat, src) => {
  mat[0] = src[0];
  mat[1] = src[1];
  mat[2] = src[2];
  mat[3] = src[3];
 ...
 */
goog.vec.mat4d.getElement = (mat, row, column) => mat[row + column * 4];
 ...
 *     chained together.
 */
goog.vec.mat4d.setElement = (mat, row, column, value) => {
 ...
 *     chained together.
 */
goog.vec.mat4d.setDiagonalValues = (mat, v00, v11, v22, v33) => {
  mat[0] = v00;
  mat[5] = v11;
  mat[10] = v22;
  mat[15] = v33;
 ...
 *     chained together.
 */
goog.vec.mat4d.setDiagonal = (mat, vec) => {
  mat[0] = vec[0];
  mat[5] = vec[1];
  mat[10] = vec[2];
  mat[15] = vec[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.getDiagonal = (mat, vec, opt_diagonal) => {
  if (!opt_diagonal) {
 ...
 *     chained together.
 */
goog.vec.mat4d.setColumnValues = (mat, column, v0, v1, v2, v3) => {
  const i = column * 4;
  mat[i] = v0;
  mat[i + 1] = v1;
  mat[i + 2] = v2;
 ...
 *     chained together.
 */
goog.vec.mat4d.setColumn = (mat, column, vec) => {
  const i = column * 4;
  mat[i] = vec[0];
  mat[i + 1] = vec[1];
  mat[i + 2] = vec[2];
 ...
 *     chained together.
 */
goog.vec.mat4d.getColumn = (mat, column, vec) => {
  const i = column * 4;
  vec[0] = mat[i];
  vec[1] = mat[i + 1];
  vec[2] = mat[i + 2];
 ...
 *     chained together.
 */
goog.vec.mat4d.setColumns = (mat, vec0, vec1, vec2, vec3) => {
  mat[0] = vec0[0];
  mat[1] = vec0[1];
  mat[2] = vec0[2];
  mat[3] = vec0[3];
 ...
 */
goog.vec.mat4d.getColumns = (mat, vec0, vec1, vec2, vec3) => {
  vec0[0] = mat[0];
  vec0[1] = mat[1];
  vec0[2] = mat[2];
  vec0[3] = mat[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.setRowValues = (mat, row, v0, v1, v2, v3) => {
  mat[row] = v0;
  mat[row + 4] = v1;
  mat[row + 8] = v2;
  mat[row + 12] = v3;
 ...
 *     chained together.
 */
goog.vec.mat4d.setRow = (mat, row, vec) => {
  mat[row] = vec[0];
  mat[row + 4] = vec[1];
  mat[row + 8] = vec[2];
  mat[row + 12] = vec[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.getRow = (mat, row, vec) => {
  vec[0] = mat[row];
  vec[1] = mat[row + 4];
  vec[2] = mat[row + 8];
  vec[3] = mat[row + 12];
 ...
 *     chained together.
 */
goog.vec.mat4d.setRows = (mat, vec0, vec1, vec2, vec3) => {
  mat[0] = vec0[0];
  mat[1] = vec1[0];
  mat[2] = vec2[0];
  mat[3] = vec3[0];
 ...
 */
goog.vec.mat4d.getRows = (mat, vec0, vec1, vec2, vec3) => {
  vec0[0] = mat[0];
  vec1[0] = mat[1];
  vec2[0] = mat[2];
  vec3[0] = mat[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.addMat = (mat0, mat1, resultMat) => {
 ...
 *     chained together.
 */
goog.vec.mat4d.subMat = (mat0, mat1, resultMat) => {
  resultMat[0] = mat0[0] - mat1[0];
  resultMat[1] = mat0[1] - mat1[1];
  resultMat[2] = mat0[2] - mat1[2];
  resultMat[3] = mat0[3] - mat1[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.multScalar = (mat, scalar, resultMat) => {
 ...
 *     chained together.
 */
goog.vec.mat4d.multMat = (mat0, mat1, resultMat) => {
  const a00 = mat0[0];
  const a10 = mat0[1];
  const a20 = mat0[2];
  const a30 = mat0[3];
 ...
 *     chained together.
 */
goog.vec.mat4d.transpose = (mat, resultMat) => {
  if (resultMat == mat) {
    const a10 = mat[1];
    const a20 = mat[2];
    const a30 = mat[3];
 ...
 */
goog.vec.mat4d.determinant = (mat) => {
  const m00 = mat[0];
  const m10 = mat[1];
  const m20 = mat[2];
  const m30 = mat[3];
 ...
 */
goog.vec.mat4d.invert = (mat, resultMat) => {
  const m00 = mat[0];
  const m10 = mat[1];
  const m20 = mat[2];
  const m30 = mat[3];
 ...
 */
goog.vec.mat4d.equals = (mat0, mat1) =>
 ...
 *     chained together.
 */
goog.vec.mat4d.multVec3 = (mat, vec, resultVec) => {
  const x = vec[0];
  const y = vec[1];
  const z = vec[2];
 ...
 *     chained together.
 */
goog.vec.mat4d.multVec3NoTranslate = (mat, vec, resultVec) => {
  const x = vec[0];
  const y = vec[1];
  const z = vec[2];
 ...
 *     chained together.
 */
goog.vec.mat4d.multVec3Projective = (mat, vec, resultVec) => {
  const x = vec[0];
  const y = vec[1];
  const z = vec[2];
 ...
 *     chained together.
 */
goog.vec.mat4d.multVec4 = (mat, vec, resultVec) => {
  const w = vec[3];
  const x = vec[0];
  const y = vec[1];
  const z = vec[2];
 ...
 *     chained.
 */
goog.vec.mat4d.makeTranslate = (mat, x, y, z) => {
  mat[0] = 1;
  mat[1] = 0;
  mat[2] = 0;
  mat[3] = 0;
 ...
 *     chained.
 */
goog.vec.mat4d.makeScale = (mat, x, y, z) => {
  mat[0] = x;
  mat[1] = 0;
  mat[2] = 0;
  mat[3] = 0;
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotate = (mat, angle, ax, ay, az) => {
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotateX = (mat, angle) => {
  const c = Math.cos(angle);
  const s = Math.sin(angle);
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotateY = (mat, angle) => {
  const c = Math.cos(angle);
  const s = Math.sin(angle);
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotateZ = (mat, angle) => {
  const c = Math.cos(angle);
  const s = Math.sin(angle);
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotationTranslation = (mat, rotation, translation) => {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotationTranslationScale = (mat, rotation, translation, scale) => {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 *     chained.
 */
goog.vec.mat4d.makeRotationTranslationScaleOrigin = (mat, rotation, translation, scale, origin) => {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 *     chained.
 */
goog.vec.mat4d.makeFrustum = (mat, left, right, bottom, top, near, far) => {
 ...
 *     chained.
 */
goog.vec.mat4d.makePerspective = (mat, fovy, aspect, near, far) => {
  const angle = fovy / 2;
  const dz = far - near;
 ...
 *     chained.
 */
goog.vec.mat4d.makeOrtho = (mat, left, right, bottom, top, near, far) => {
 ...
 *     chained.
 */
goog.vec.mat4d.makeLookAt = (mat, eyePt, centerPt, worldUpVec) => {
 ...
 */
goog.vec.mat4d.toLookAt = (mat, eyePt, fwdVec, worldUpVec) => {
 ...
 *     chained.
 */
goog.vec.mat4d.makeEulerZXZ = (mat, theta1, theta2, theta3) => {
 ...
 *     chained together.
 */
goog.vec.mat4d.toEulerZXZ = (mat, euler, opt_theta2IsNegative) => {
 ...
 *     chained.
 */
goog.vec.mat4d.translate = (mat, x, y, z) => {
 ...
 *     chained.
 */
goog.vec.mat4d.scale = (mat, x, y, z) => {
  mat[0] = mat[0] * x;
  mat[1] = mat[1] * x;
  mat[2] = mat[2] * x;
  mat[3] = mat[3] * x;
 ...
 *     chained.
 */
goog.vec.mat4d.rotate = (mat, angle, x, y, z) => {
  const m00 = mat[0];
  const m10 = mat[1];
  const m20 = mat[2];
  const m30 = mat[3];
 ...
 *     chained.
 */
goog.vec.mat4d.rotateX = (mat, angle) => {
  const m01 = mat[4];
  const m11 = mat[5];
  const m21 = mat[6];
  const m31 = mat[7];
 ...
 *     chained.
 */
goog.vec.mat4d.rotateY = (mat, angle) => {
  const m00 = mat[0];
  const m10 = mat[1];
  const m20 = mat[2];
  const m30 = mat[3];
 ...
 *     chained.
 */
goog.vec.mat4d.rotateZ = (mat, angle) => {
  const m00 = mat[0];
  const m10 = mat[1];
  const m20 = mat[2];
  const m30 = mat[3];
