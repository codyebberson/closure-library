...
testSuite({
  ...
  testSlerp() {
    ...
    v5 = vec3f.slerp(v0, v4, 0.25, v5);
    assertRoughlyEquals(Math.PI / 4, Math.acos(vec3f.dot(v5, v0)), vec.EPSILON);
    assertRoughlyEquals((Math.PI * 3) / 4, Math.acos(vec3f.dot(v5, v4)), vec.EPSILON);

    // f = 0.75, result should be 135-degrees to v0, and 45 to v4.
    v5 = vec3f.slerp(v0, v4, 0.75, v5);
    assertRoughlyEquals((Math.PI * 3) / 4, Math.acos(vec3f.dot(v5, v0)), vec.EPSILON);
    ...
    v5 = vec3f.slerp(v1, v2, 0.25, v5);
    assertRoughlyEquals(Math.PI / 4, Math.acos(vec3f.dot(v5, v1)), vec.EPSILON);
    assertRoughlyEquals((Math.PI * 3) / 4, Math.acos(vec3f.dot(v5, v2)), vec.EPSILON);

    // f = 0.75, result should be 135-degrees to v1, and 45 to v2.
    v5 = vec3f.slerp(v1, v2, 0.75, v5);
    assertRoughlyEquals((Math.PI * 3) / 4, Math.acos(vec3f.dot(v5, v1)), vec.EPSILON);
    ...
    vec3f.slerp(v1, v3, 0.5, v5);
    // Axis of rotation is arbitrary, but result should be 90 degrees from both
    // v1 and v3.
    assertRoughlyEquals(
      Math.PI / 2,
      Math.acos(vec3f.dot(v5, v1) / (vec3f.magnitude(v5) * vec3f.magnitude(v1))),
      vec.EPSILON
    );
    assertRoughlyEquals(
      Math.PI / 2,
      Math.acos(vec3f.dot(v5, v3) / (vec3f.magnitude(v3) * vec3f.magnitude(v5))),
      vec.EPSILON
    );
