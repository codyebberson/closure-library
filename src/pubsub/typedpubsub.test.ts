...
testSuite({
  ...
  testConstructor() {
    ...
    assertTrue('PubSub instance must have the expected type', pubsub instanceof TypedPubSub);
  },

  testDispose() {
    assertFalse('PubSub instance must not have been disposed of', pubsub.isDisposed());
    pubsub.dispose();
  ...
  testSubscribeUnsubscribe() {
    ...
    assertEquals('Topic "foo" must not have any subscribers', 0, pubsub.getCount(FOO));
    assertEquals('Topic "bar" must not have any subscribers', 0, pubsub.getCount(BAR));
  ...
  testSubscribeUnsubscribeWithContext() {
    ...
    assertEquals('Topic "X" must not have any subscribers', 0, pubsub.getCount(TOPIC_X));
    ...
    assertEquals('Topic "X" must have 2 subscribers', 2, pubsub.getCount(TOPIC_X));

    pubsub.subscribe(TOPIC_X, bar, contextB);
    assertEquals('Topic "X" must have 3 subscribers', 3, pubsub.getCount(TOPIC_X));
    ...
    assertEquals('Topic "X" must have no subscribers', 0, pubsub.getCount(TOPIC_X));
  },
  ...
  testSubscribeOnce() {
    ...
    assertEquals('Topic must have one subscriber', 1, pubsub.getCount(SOME_TOPIC));
    assertFalse('Subscriber must not have been called yet', called);

    pubsub.publish(SOME_TOPIC);
    assertEquals('Topic must have no subscribers', 0, pubsub.getCount(SOME_TOPIC));
  ...
  testUnsubscribeByKey() {
    ...
    assertEquals('Topic "X" must have 1 subscriber', 1, pubsub.getCount(TOPIC_X));
    assertEquals('Topic "Y" must have no subscribers', 0, pubsub.getCount(TOPIC_Y));
  ...
  testSubscribeUnsubscribeMultiple() {
    ...
    assertEquals('Pubsub channel must not have any subscribers', 0, pubsub.getCount());

    assertEquals('Topic "X" must not have any subscribers', 0, pubsub.getCount(TOPIC_X));
    assertEquals('Topic "Y" must not have any subscribers', 0, pubsub.getCount(TOPIC_Y));
    assertEquals('Topic "Z" must not have any subscribers', 0, pubsub.getCount(TOPIC_Z));
    ...
    assertEquals('Topic "X" must have 2 subscribers', 2, pubsub.getCount(TOPIC_X));
    assertEquals('Topic "Y" must have 2 subscribers', 2, pubsub.getCount(TOPIC_Y));
    assertEquals('Topic "Z" must have 2 subscribers', 2, pubsub.getCount(TOPIC_Z));

    assertEquals('Pubsub channel must have a total of 6 subscribers', 6, pubsub.getCount());
  ...
  testPublishEmptyTopic() {
    ...
    assertFalse('Publishing to nonexistent topic must return false', pubsub.publish(SOME_TOPIC));

    pubsub.subscribe(SOME_TOPIC, foo);
    assertTrue('Publishing to topic with subscriber must return true', pubsub.publish(SOME_TOPIC));
    assertTrue('Foo must have been called', fooCalled);
  ...
  testSubscribeWhilePublishing() {
    ...
    assertEquals('Topic must have one subscriber', 1, pubsub.getCount(SOME_TOPIC));
    assertFalse('No subscriber must have been called yet', firstCalled || secondCalled);

    pubsub.publish(SOME_TOPIC);
    assertEquals('Topic must have two subscribers', 2, pubsub.getCount(SOME_TOPIC));
    assertTrue('The first subscriber must have been called', firstCalled);
    assertFalse('The second subscriber must not have been called yet', secondCalled);

    pubsub.publish(SOME_TOPIC);
    assertEquals('Topic must have three subscribers', 3, pubsub.getCount(SOME_TOPIC));
    assertTrue('The first subscriber must have been called', firstCalled);
  ...
  testUnsubscribeWhilePublishing() {
    ...
    function first() {
      ...
      assertEquals('Topic "X" must still have 3 subscribers', 3, pubsub.getCount(TOPIC_X));
      firstCalled = true;
    }
    ...
    assertEquals('Topic "X" must have 1 subscriber after publishing', 1, pubsub.getCount(TOPIC_X));
  },
  ...
  testUnsubscribeSelfWhilePublishing() {
    ...
    const selfDestruct = () => {
      ...
      assertEquals('Topic must still have 1 subscriber', 1, pubsub.getCount(SOME_TOPIC));
      selfDestructCalled = true;
    };

    pubsub.subscribe(SOME_TOPIC, selfDestruct);
    assertEquals('Topic must have 1 subscriber', 1, pubsub.getCount(SOME_TOPIC));
    assertFalse('selfDestruct() must not have been called yet', selfDestructCalled);

    pubsub.publish(SOME_TOPIC);
    assertTrue('selfDestruct() must have been called', selfDestructCalled);
    assertEquals('Topic must have no subscribers after publishing', 0, pubsub.getCount(SOME_TOPIC));
  },
  ...
  testClear() {
    ...
    const topics = [new TopicId('W'), new TopicId('X'), new TopicId('Y'), new TopicId('Z')];
