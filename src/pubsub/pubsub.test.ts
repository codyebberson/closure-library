...
testSuite({
  ...
  testConstructor() {
    ...
    assertTrue('PubSub instance must have the expected type', pubsub instanceof PubSub);
  },
  ...
  testSubscribeUnsubscribeWithContext() {
    ...
    assertEquals('Topic "X" must not have any subscribers', 0, pubsub.getCount('X'));
  ...
  testSubscribeOnce() {
    ...
    assertEquals('Topic must have one subscriber', 1, pubsub.getCount('someTopic'));
    assertFalse('Subscriber must not have been called yet', called);

    pubsub.publish('someTopic');
    assertEquals('Topic must have no subscribers', 0, pubsub.getCount('someTopic'));
  ...
  testAsyncSubscribeOnce() {
    ...
    assertEquals('Topic must have one subscriber', 1, asyncPubsub.getCount('someTopic'));
    mockClock.tick();
    ...
    asyncPubsub.publish('someTopic');
    mockClock.tick();
    assertEquals('Topic must have no subscribers', 0, asyncPubsub.getCount('someTopic'));
  ...
  testAsyncSubscribeOnceWithContext() {
    ...
    assertEquals('Topic must have one subscriber', 1, asyncPubsub.getCount('someTopic'));
    mockClock.tick();
    assertEquals('Subscriber must not have been called yet', 0, context.callCount);
  ...
  testSelfResubscribe() {
    ...
    assertEquals('Topic must have 1 subscriber', 1, pubsub.getCount('someTopic'));
    assertEquals(
      'Pubsub must not have any pending unsubscribe keys',
      0,
  ...
  testSubscribeUnsubscribeMultiple() {
    ...
    assertEquals('Topic "X" must not have any subscribers', 0, pubsub.getCount('X'));
    assertEquals('Topic "Y" must not have any subscribers', 0, pubsub.getCount('Y'));
    assertEquals('Topic "Z" must not have any subscribers', 0, pubsub.getCount('Z'));
  ...
  testPublishEmptyTopic() {
    ...
    assertFalse('Publishing to nonexistent topic must return false', pubsub.publish('someTopic'));

    pubsub.subscribe('someTopic', foo);
    assertTrue('Publishing to topic with subscriber must return true', pubsub.publish('someTopic'));
    assertTrue('Foo must have been called', fooCalled);
  ...
  testSubscribeWhilePublishing() {
    ...
    pubsub.publish('someTopic');
    assertEquals('Topic must have two subscribers', 2, pubsub.getCount('someTopic'));
    ...
    pubsub.publish('someTopic');
    assertEquals('Topic must have three subscribers', 3, pubsub.getCount('someTopic'));
    assertTrue('The first subscriber must have been called', firstCalled);
  ...
  testUnsubscribeWhilePublishing() {
    ...
    function first() {
      ...
      assertEquals('Topic "X" must still have 3 subscribers', 3, pubsub.getCount('X'));
      firstCalled = true;
    }
    ...
    assertEquals('Topic "X" must have 1 subscriber after publishing', 1, pubsub.getCount('X'));
    assertEquals(
      'PubSub must not have any subscriptions pending removal',
      0,
      pubsub.pendingKeys_.length
    );
  },
  ...
  testUnsubscribeSelfWhilePublishing() {
    ...
    const selfDestruct = () => {
      ...
      assertEquals('Topic must still have 1 subscriber', 1, pubsub.getCount('someTopic'));
