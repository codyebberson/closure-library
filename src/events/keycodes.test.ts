...
testSuite({
  ...
  testKeyCodeZero() {
    ...
    assertEquals(userAgent.WEBKIT || userAgent.EDGE, KeyCodes.isCharacterKey(0));
  },
  ...
  testNormalizeGeckoKeyCode() {
    ...
    assertEquals(KeyCodes.normalizeGeckoKeyCode(KeyCodes.FF_EQUALS), KeyCodes.EQUALS);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.FF_EQUALS), KeyCodes.EQUALS);

    assertEquals(KeyCodes.normalizeGeckoKeyCode(KeyCodes.FF_SEMICOLON), KeyCodes.SEMICOLON);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.FF_SEMICOLON), KeyCodes.SEMICOLON);

    assertEquals(KeyCodes.normalizeGeckoKeyCode(KeyCodes.MAC_FF_META), KeyCodes.META);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.MAC_FF_META), KeyCodes.META);

    assertEquals(KeyCodes.normalizeGeckoKeyCode(KeyCodes.WIN_KEY_FF_LINUX), KeyCodes.WIN_KEY);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.WIN_KEY_FF_LINUX), KeyCodes.WIN_KEY);

    // Test general key codes.
    assertEquals(KeyCodes.normalizeGeckoKeyCode(KeyCodes.COMMA), KeyCodes.COMMA);
  ...
  testNormalizeMacWebKitKeyCode() {
    ...
    assertEquals(KeyCodes.normalizeMacWebKitKeyCode(KeyCodes.MAC_WK_CMD_LEFT), KeyCodes.META);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.MAC_WK_CMD_LEFT), KeyCodes.META);

    assertEquals(KeyCodes.normalizeMacWebKitKeyCode(KeyCodes.MAC_WK_CMD_RIGHT), KeyCodes.META);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.MAC_WK_CMD_RIGHT), KeyCodes.META);

    // Test general key codes.
    assertEquals(KeyCodes.normalizeMacWebKitKeyCode(KeyCodes.COMMA), KeyCodes.COMMA);
    assertEquals(KeyCodes.normalizeKeyCode(KeyCodes.COMMA), KeyCodes.COMMA);
  },
});
