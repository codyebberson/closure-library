...
class PerfectlySlowStat {
  ...
  getStats(now) {
    const end = Math.floor(now / this.slotSize_) * this.slotSize_ + this.slotSize_;
    const start = end - this.interval_;
    const events = this.events_.filter((e) => e.time >= start);
    return {
      count: events.reduce((sum, e) => sum + e.count, 0),
      min: events.reduce((min, e) => Math.min(min, e.count), Number.MAX_VALUE),
      max: events.reduce((max, e) => Math.max(max, e.count), Number.MIN_VALUE),
    };
  }
}
...
testSuite({
  ...
  testConstantIncrementPerSlot() {
    ...
    for (let i = 0; i < 50; ++i) {
      ...
      const msg = format('now=%d i=%d newMin=%d newMax=%d', now, i, newMin, newMax);
