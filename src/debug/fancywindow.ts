...
goog.debug.FancyWindow.prototype.getDropDown_ = function (id, selected) {
  ...
  sel.appendChild(
    dh.createDom(goog.dom.TagName.OPTION, { selected: selected == 'INHERIT' }, 'INHERIT')
  );
  return sel;
};
...
goog.debug.FancyWindow.prototype.closeOptions_ = function () {
  ...
  for (let i = 0; i < loggers.length; i++) {
    ...
    const sel = /** @type {?HTMLSelectElement} */ (dh.getElement('sel' + logger.getName()));
    if (!sel) {
...
goog.debug.FancyWindow.prototype.resizeStuff_ = function () {
  ...
  logel.style.height =
    dh.getDocument().body.offsetHeight - headel.offsetHeight - (goog.userAgent.IE ? 4 : 0) + 'px';
};
...
goog.debug.FancyWindow.prototype.getHtml_ = function () {
  ...
  var body = SafeHtml.create(
    ...
    {},
    SafeHtml.concat(
      SafeHtml.create('div', { id: 'log', style: goog.string.Const.from('overflow:auto') }),
      SafeHtml.create(
        'div',
        { id: 'head' },
        SafeHtml.concat(
          SafeHtml.create('p', {}, SafeHtml.create('b', {}, 'Logging: ' + this.identifier)),
      ...
      SafeHtml.create(
        ...
        { id: 'options' },
        SafeHtml.concat(
          SafeHtml.create('big', {}, SafeHtml.create('b', {}, 'Options:')),
 ...
 * @private
 */
goog.debug.FancyWindow.prototype.writeOptionsToLocalStorage_ = () => {
 ...
 * @private
 */
goog.debug.FancyWindow.prototype.readOptionsFromLocalStorage_ = () => {
  if (!goog.debug.FancyWindow.HAS_LOCAL_STORE) {
    return;
  }
  var storedKeys = goog.debug.FancyWindow.getStoredKeys_();
 ...
 * @private
 */
goog.debug.FancyWindow.getLoggers_ = () => {
