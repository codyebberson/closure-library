...
testSuite({
  ...
  testPositionAtAnchorLeftToRight() {
    ...
    positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.TOP_LEFT);
    ...
    assertRoundedEquals(
      'Popup should have the same y position as the anchor.',
      anchorRect.top,
      popupRect.top
    );

    // Anchor top left to bottom left.
    positioning.positionAtAnchor(anchor, corner.BOTTOM_LEFT, popup, corner.TOP_LEFT);
    ...
    positioning.positionAtAnchor(anchor, corner.TOP_RIGHT, popup, corner.TOP_LEFT);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
    ...
    positioning.positionAtAnchor(anchor, corner.BOTTOM_RIGHT, popup, corner.TOP_RIGHT);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
    ...
    positioning.positionAtAnchor(anchor, corner.BOTTOM_START, popup, corner.TOP_START);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should line up with left edge ' + 'of anchor.',
    ...
    positioning.positionAtAnchor(anchor, corner.TOP_CENTER, popup, corner.TOP_CENTER);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    ...
    assertRoundedEquals(
      'The center of popup should line up with the center ' + 'of anchor.',
    ...
    positioning.positionAtAnchor(anchor, corner.TOP_CENTER, popup, corner.TOP_LEFT);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should line up with the center ' + 'of anchor.',
      Math.floor(anchorRect.left + anchorRect.width / 2),
    ...
    positioning.positionAtAnchor(anchor, corner.BOTTOM_CENTER, popup, corner.TOP_LEFT);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should line up with the middle ' + 'of anchor.',
      Math.floor(anchorRect.left + anchorRect.width / 2),
      Math.floor(popupRect.left)
    );
    assertRoundedEquals(
    ...
    positioning.positionAtAnchor(anchor, corner.BOTTOM_LEFT, popup, corner.TOP_CENTER);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should line up with the middle ' + 'of anchor.',
  ...
  testPositionAtAnchorWithOffset() {
    ...
    let popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should be fifteen pixels from ' + 'anchor.',
      anchorRect.left,
      popupRect.left + 15
    );
    assertRoundedEquals(
    ...
    // anchor.
    positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.TOP_LEFT, newCoord(3, 1));
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertRoundedEquals(
      'Left edge of popup should be three pixels right of ' + "the anchor's left edge",
      anchorRect.left,
      popupRect.left - 3
    );
    assertRoundedEquals(
      'Top edge of popup should be one pixel below of the ' + "anchor's top edge",
      anchorRect.top,
      popupRect.top - 1
    );
  },
  ...
  testPositionAtAnchorOverflowLeftEdgeRightToLeft() {
    ...
    assertFalse('Positioning operation should have failed.', (status & OverflowStatus.FAILED) == 0);
  ...
  testPositionAtAnchorWithMargin() {
    ...
    assertRoundedEquals(
      'Top edge of popup should be one pixels from anchor.',
      anchorRect.top,
      popupRect.top - 1
    );
  ...
  testPositionAtAnchorBodyViewport() {
    ...
    positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.TOP_RIGHT);
    anchorRect = style.getBounds(anchor);
    popupRect = style.getBounds(popup);
    assertEquals(
  ...
  testPositionAtAnchorSpecificViewport() {
    ...
    assertFalse('X position should have been adjusted.', (status & OverflowStatus.ADJUSTED_X) == 0);
    assertTrue(
      'Y position should not have been adjusted.',
      (status & OverflowStatus.ADJUSTED_Y) == 0
    );
  ...
  testPositionAtAnchorOutsideViewport() {
    ...
    let status = positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.TOP_RIGHT);
    let anchorRect = style.getBounds(anchor);
    let popupRect = style.getBounds(popup);
    assertTrue(
      'Positioning operation should have been successful.',
  ...
  testAdjustForViewportAdjustXFailY() {
    ...
    size = newSize(100, 50);
    assertEquals(
      'Element position should be adjusted not to overflow right ' + 'edge of viewport.',
      OverflowStatus.ADJUSTED_X,
      f(pos, size, viewport, overflow)
    );
    ...
    size = newSize(100, 50);
    assertEquals(
      'Element position should be adjusted not to overflow left ' + 'edge of viewport.',
      OverflowStatus.ADJUSTED_X,
      f(pos, size, viewport, overflow)
    );
    assertEquals('X Position should be adjusted to 100.', 100, pos.x);
  ...
  testAdjustForViewportResizeHeight() {
    ...
    assertEquals(
      ...
      OverflowStatus.HEIGHT_ADJUSTED,
      f(pos, size, viewport, overflow)
    );
    assertEquals('Height should be resized to 50.', 50, size.height);
    assertTrue(
      'Output box is within viewport',
      viewport.contains(new Box(pos.y, pos.x + size.width, pos.y + size.height, pos.x))
    );
    ...
    pos = newCoord(150, 150);
    size = newSize(50, 50);
    assertEquals('No Viewport overflow.', OverflowStatus.NONE, f(pos, size, viewport, overflow));
    assertTrue(
      'Output box is within viewport',
      viewport.contains(new Box(pos.y, pos.x + size.width, pos.y + size.height, pos.x))
    );
    ...
    assertTrue(
      'Output box is within viewport',
      offsetViewport.contains(new Box(pos.y, pos.x + size.width, pos.y + size.height, pos.x))
    );
  },
  ...
  testPositionAtAnchorWithResizeHeight() {
    ...
    assertEquals('Status should be HEIGHT_ADJUSTED.', OverflowStatus.HEIGHT_ADJUSTED, status);
    ...
    assertTrue(
      'Popup ' + style.getBounds(popup) + ' not is within viewport' + viewport,
  ...
  testPositionAtCoordinateResizeHeight() {
    ...
    assertEquals('Height should be resized to the size of the viewport.', 50, bounds.height);
  },
  ...
  testGetPositionAtCoordinateResizeHeight() {
    ...
    assertEquals('Height should be resized to the size of the viewport.', 50, result.rect.height);
  },
  ...
  testGetEffectiveCornerLeftToRight() {
    ...
    assertEquals('TOP_LEFT should be unchanged for ltr.', corner.TOP_LEFT, f(el, corner.TOP_LEFT));
    assertEquals(
      'TOP_RIGHT should be unchanged for ltr.',
      corner.TOP_RIGHT,
      f(el, corner.TOP_RIGHT)
    );
    assertEquals(
      'BOTTOM_LEFT should be unchanged for ltr.',
      corner.BOTTOM_LEFT,
      f(el, corner.BOTTOM_LEFT)
    );
    assertEquals(
      'BOTTOM_RIGHT should be unchanged for ltr.',
      corner.BOTTOM_RIGHT,
      f(el, corner.BOTTOM_RIGHT)
    );

    assertEquals('TOP_START should be TOP_LEFT for ltr.', corner.TOP_LEFT, f(el, corner.TOP_START));
    assertEquals('TOP_END should be TOP_RIGHT for ltr.', corner.TOP_RIGHT, f(el, corner.TOP_END));
    assertEquals(
      'BOTTOM_START should be BOTTOM_LEFT for ltr.',
      corner.BOTTOM_LEFT,
      f(el, corner.BOTTOM_START)
    );
    assertEquals(
      'BOTTOM_END should be BOTTOM_RIGHT for ltr.',
      corner.BOTTOM_RIGHT,
  ...
  testGetEffectiveCornerRightToLeft() {
    ...
    assertEquals('TOP_LEFT should be unchanged for rtl.', corner.TOP_LEFT, f(el, corner.TOP_LEFT));
    assertEquals(
      'TOP_RIGHT should be unchanged for rtl.',
      corner.TOP_RIGHT,
      f(el, corner.TOP_RIGHT)
    );
    assertEquals(
      'BOTTOM_LEFT should be unchanged for rtl.',
      corner.BOTTOM_LEFT,
      f(el, corner.BOTTOM_LEFT)
    );
    assertEquals(
      'BOTTOM_RIGHT should be unchanged for rtl.',
      corner.BOTTOM_RIGHT,
      f(el, corner.BOTTOM_RIGHT)
    );

    assertEquals(
      'TOP_START should be TOP_RIGHT for rtl.',
      corner.TOP_RIGHT,
      f(el, corner.TOP_START)
    );
    assertEquals('TOP_END should be TOP_LEFT for rtl.', corner.TOP_LEFT, f(el, corner.TOP_END));
    assertEquals(
      'BOTTOM_START should be BOTTOM_RIGHT for rtl.',
      corner.BOTTOM_RIGHT,
      f(el, corner.BOTTOM_START)
    );
    assertEquals(
      'BOTTOM_END should be BOTTOM_LEFT for rtl.',
      corner.BOTTOM_LEFT,
      f(el, corner.BOTTOM_END)
    );
  },
  ...
  testFlipCornerHorizontal() {
    ...
    assertEquals('TOP_LEFT should be flipped to TOP_RIGHT.', corner.TOP_RIGHT, f(corner.TOP_LEFT));
    assertEquals('TOP_RIGHT should be flipped to TOP_LEFT.', corner.TOP_LEFT, f(corner.TOP_RIGHT));
    assertEquals(
      'BOTTOM_LEFT should be flipped to BOTTOM_RIGHT.',
      corner.BOTTOM_RIGHT,
      f(corner.BOTTOM_LEFT)
    );
    assertEquals(
      'BOTTOM_RIGHT should be flipped to BOTTOM_LEFT.',
      corner.BOTTOM_LEFT,
      f(corner.BOTTOM_RIGHT)
    );

    assertEquals('TOP_START should be flipped to TOP_END.', corner.TOP_END, f(corner.TOP_START));
    assertEquals('TOP_END should be flipped to TOP_START.', corner.TOP_START, f(corner.TOP_END));
    assertEquals(
      'BOTTOM_START should be flipped to BOTTOM_END.',
      corner.BOTTOM_END,
      f(corner.BOTTOM_START)
    );
    assertEquals(
      'BOTTOM_END should be flipped to BOTTOM_START.',
      corner.BOTTOM_START,
      f(corner.BOTTOM_END)
    );
  },
  ...
  testFlipCornerVertical() {
    ...
    assertEquals(
      'TOP_LEFT should be flipped to BOTTOM_LEFT.',
      corner.BOTTOM_LEFT,
      f(corner.TOP_LEFT)
    );
    assertEquals(
      'TOP_RIGHT should be flipped to BOTTOM_RIGHT.',
      corner.BOTTOM_RIGHT,
      f(corner.TOP_RIGHT)
    );
    assertEquals(
      'BOTTOM_LEFT should be flipped to TOP_LEFT.',
      corner.TOP_LEFT,
      f(corner.BOTTOM_LEFT)
    );
    assertEquals(
      'BOTTOM_RIGHT should be flipped to TOP_RIGHT.',
      corner.TOP_RIGHT,
      f(corner.BOTTOM_RIGHT)
    );

    assertEquals(
      'TOP_START should be flipped to BOTTOM_START.',
      corner.BOTTOM_START,
      f(corner.TOP_START)
    );
    assertEquals('TOP_END should be flipped to BOTTOM_END.', corner.BOTTOM_END, f(corner.TOP_END));
    assertEquals(
      'BOTTOM_START should be flipped to TOP_START.',
      corner.TOP_START,
      f(corner.BOTTOM_START)
    );
    assertEquals('BOTTOM_END should be flipped to TOP_END.', corner.TOP_END, f(corner.BOTTOM_END));
  },
  ...
  testFlipCorner() {
    ...
    assertEquals(
      'TOP_LEFT should be flipped to BOTTOM_RIGHT.',
      corner.BOTTOM_RIGHT,
      f(corner.TOP_LEFT)
    );
    assertEquals(
      'TOP_RIGHT should be flipped to BOTTOM_LEFT.',
      corner.BOTTOM_LEFT,
      f(corner.TOP_RIGHT)
    );
    assertEquals(
      'BOTTOM_LEFT should be flipped to TOP_RIGHT.',
      corner.TOP_RIGHT,
      f(corner.BOTTOM_LEFT)
    );
    assertEquals(
      'BOTTOM_RIGHT should be flipped to TOP_LEFT.',
      corner.TOP_LEFT,
      f(corner.BOTTOM_RIGHT)
    );

    assertEquals(
      'TOP_START should be flipped to BOTTOM_END.',
      corner.BOTTOM_END,
      f(corner.TOP_START)
    );
    assertEquals(
      'TOP_END should be flipped to BOTTOM_START.',
      corner.BOTTOM_START,
      f(corner.TOP_END)
    );
    assertEquals(
      'BOTTOM_START should be flipped to TOP_END.',
      corner.TOP_END,
      f(corner.BOTTOM_START)
    );
    assertEquals(
      'BOTTOM_END should be flipped to TOP_START.',
      corner.TOP_START,
      f(corner.BOTTOM_END)
    );
  },
  ...
  testPositionAtAnchorFrameViewportStandard() {
    ...
    const popupRect = style.getBounds(popup);
    assertEquals('Status should not have any ADJUSTED and FAILED.', OverflowStatus.NONE, status);
    assertRoundedEquals(
  ...
  testPositionAtAnchorNestedFrames() {
    ...
    let status = positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.BOTTOM_LEFT);
    assertEquals('Status should not have any ADJUSTED and FAILED.', OverflowStatus.NONE, status);
    ...
    status = positioning.positionAtAnchor(anchor, corner.TOP_LEFT, popup, corner.BOTTOM_LEFT);
    assertEquals('Status should not have any ADJUSTED and FAILED.', OverflowStatus.NONE, status);
  ...
  testPositionAtAnchorWithOverflowScrollOffsetParent() {
    ...
    assertObjectRoughlyEquals(
      new Coordinate(testAreaOffset.x + 400, testAreaOffset.y + 100 - 50 - scrollbarWidth),
      style.getPageOffset(popup),
      1
    );
    ...
    assertObjectRoughlyEquals(
      new Coordinate(testAreaOffset.x + 200 - 75 - scrollbarWidth, testAreaOffset.y + 400),
      style.getPageOffset(popup),
      1
    );
  ...
  testPositionAtAnchorWithOverflowHiddenParent() {
    ...
    assertObjectRoughlyEquals(
      new Coordinate(testAreaOffset.x + 200 - 75, testAreaOffset.y + 100 - 50),
      style.getPageOffset(popup),
      1
    );
