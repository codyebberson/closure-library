...
goog.positioning.Corner = {
  ...
  BOTTOM_RIGHT: goog.positioning.CornerBit.BOTTOM | goog.positioning.CornerBit.RIGHT,
  TOP_START: goog.positioning.CornerBit.FLIP_RTL,
  TOP_END: goog.positioning.CornerBit.FLIP_RTL | goog.positioning.CornerBit.RIGHT,
  BOTTOM_START: goog.positioning.CornerBit.BOTTOM | goog.positioning.CornerBit.FLIP_RTL,
  BOTTOM_END:
  ...
  BOTTOM_CENTER: goog.positioning.CornerBit.BOTTOM | goog.positioning.CornerBit.CENTER,
};
...
goog.positioning.positionAtAnchor = (
  ...
  var absolutePos = new goog.math.Coordinate(
    offsetLeft,
    anchorRect.top + (corner & goog.positioning.CornerBit.BOTTOM ? anchorRect.height : 0)
  );
    ...
    if (!isBody || goog.style.getComputedPosition(parent) != 'static') {
      ...
      if (!isBody) {
        movableParentTopLeft = goog.math.Coordinate.difference(
          ...
          new goog.math.Coordinate(goog.style.bidi.getScrollLeft(parent), parent.scrollTop)
        );
      }
    }
  }
 ...
 * @private
 */
goog.positioning.getVisiblePart_ = (el) => {
 ...
 */
goog.positioning.getPositionAtCoordinate = (
  absolutePos,
  elementSize,
  elementCorner,
  opt_margin,
  ...
  if (opt_overflow) {
    status = opt_viewport
      ? goog.positioning.adjustForViewport_(absolutePos, elementSize, opt_viewport, opt_overflow)
 ...
 * @private
 */
goog.positioning.adjustForViewport_ = (pos, size, viewport, overflow) => {
  var status = goog.positioning.OverflowStatus.NONE;
  ...
  if (
    (overflow & ADJUST_X_EXCEPT_OFFSCREEN) == ADJUST_X_EXCEPT_OFFSCREEN &&
    (pos.x < viewport.left || pos.x >= viewport.right)
  ) {
    overflow &= ~goog.positioning.Overflow.ADJUST_X;
  }
  if (
    (overflow & ADJUST_Y_EXCEPT_OFFSCREEN) == ADJUST_Y_EXCEPT_OFFSCREEN &&
  ...
  if (overflow & goog.positioning.Overflow.FAIL_X) {
    status |=
      (pos.x < viewport.left ? goog.positioning.OverflowStatus.FAILED_LEFT : 0) |
      (pos.x + size.width > viewport.right ? goog.positioning.OverflowStatus.FAILED_RIGHT : 0);
  }
  ...
  if (overflow & goog.positioning.Overflow.FAIL_Y) {
    status |=
      (pos.y < viewport.top ? goog.positioning.OverflowStatus.FAILED_TOP : 0) |
      (pos.y + size.height > viewport.bottom ? goog.positioning.OverflowStatus.FAILED_BOTTOM : 0);
  }
 ...
 */
goog.positioning.getEffectiveCorner = (element, corner) =>
  (corner & goog.positioning.CornerBit.FLIP_RTL && goog.style.isRightToLeft(element)
 ...
 */
goog.positioning.flipCornerHorizontal = (corner) => corner ^ goog.positioning.CornerBit.RIGHT;
 ...
 */
goog.positioning.flipCornerVertical = (corner) => corner ^ goog.positioning.CornerBit.BOTTOM;
 ...
 *     vertically.
 */
goog.positioning.flipCorner = (corner) =>
  corner ^ goog.positioning.CornerBit.BOTTOM ^ goog.positioning.CornerBit.RIGHT;
