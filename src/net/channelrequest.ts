...
goog.net.ChannelRequest.createChannelRequest = (
  ...
  opt_requestId,
  opt_retryId
) => new goog.net.ChannelRequest(channel, channelDebug, opt_sessionId, opt_requestId, opt_retryId);
 ...
 */
goog.net.ChannelRequest.errorStringFromCode = (errorCode, statusCode) => {
  switch (errorCode) {
...
goog.net.ChannelRequest.prototype.onXmlHttpReadyStateChanged_ = function () {
  ...
  if (
    ...
    (goog.userAgent.WEBKIT &&
      !goog.userAgent.isVersionOrHigher(goog.net.ChannelRequest.MIN_WEBKIT_FOR_INTERACTIVE_))
  ) {
    ...
    if (
      ...
      (readyState == goog.net.XmlHttp.ReadyState.INTERACTIVE && !this.xmlHttp_.getResponseText())
    ) {
      // not yet ready
      return;
    }
  ...
  if (!this.successful_) {
    if (status == 400 && responseText.indexOf('Unknown SID') > 0) {
      ...
      goog.net.browserchannelinternal.stats.notifyStatEvent(
        goog.net.browserchannelinternal.stats.Stat.REQUEST_UNKNOWN_SESSION_ID
      );
...
goog.net.ChannelRequest.prototype.tridentGet_ = function (usingSecondaryDomain) {
  ...
  if (usingSecondaryDomain) {
    const escapedHostname = goog.net.ChannelRequest.escapeForStringInScript_(hostname);
    body += '<script>document.domain="' + escapedHostname + '"</scr' + 'ipt>';
  }
  body += '</body></html>';
  const bodyHtml = goog.html.uncheckedconversions.safeHtmlFromStringKnownToSatisfyTypeContract(
    goog.string.Const.from('b/12014412'),
    body
  );

  this.trident_.open();
  goog.dom.safe.documentWrite(/** @type {!Document} */ (this.trident_), bodyHtml);
  this.trident_.close();
  ...
  const sanitizedEscapedUrl = goog.string.htmlEscape(goog.html.SafeUrl.unwrap(safeUrl));
  const iframeHtml = goog.html.uncheckedconversions.safeHtmlFromStringKnownToSatisfyTypeContract(
    goog.string.Const.from('b/12014412'),
 ...
 * @private
 */
goog.net.ChannelRequest.escapeForStringInScript_ = (string) => {
  let escaped = '';
...
goog.net.ChannelRequest.prototype.onTridentRpcMessage_ = function (msg) {
  ...
  goog.net.browserchannelinternal.hooks.setTimeout(
    goog.bind(this.onTridentRpcMessageAsync_, this, msg),
    0
  );
};
...
goog.net.ChannelRequest.prototype.onTridentDone_ = function (successful) {
  // need to do async b/c this gets called off of the context of the ActiveX
  goog.net.browserchannelinternal.hooks.setTimeout(
    goog.bind(this.onTridentDoneAsync_, this, successful),
    0
  );
};
