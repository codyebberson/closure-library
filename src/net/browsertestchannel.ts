...
goog.net.BrowserTestChannel.prototype.connect = function (path) {
  ...
  this.request_ = goog.net.ChannelRequest.createChannelRequest(this, this.channelDebug_);
...
goog.net.BrowserTestChannel.prototype.checkBlocked_ = function () {
  const uri = this.channel_.createDataUri(this.blockedPrefix_, '/mail/images/cleardot.gif');
  uri.makeUnique();
...
goog.net.BrowserTestChannel.prototype.connectStage2_ = function () {
  ...
  if (secondTestResults != null) {
    this.channelDebug_.debug(
      'TestConnection: skipping stage 2, precomputed result is ' + secondTestResults
        ? 'Buffered'
        : 'Unbuffered'
    );
    goog.net.browserchannelinternal.stats.notifyStatEvent(
 ...
 */
goog.net.BrowserTestChannel.prototype.isClosed = () => false;
  ...
  if (this.state_ == goog.net.BrowserTestChannel.State_.INIT) {
    ...
    if (this.receivedIntermediateResult_) {
      ...
      if (responseText == '11111') {
        ...
        goog.net.browserchannelinternal.stats.notifyStatEvent(
          goog.net.browserchannelinternal.stats.Stat.TEST_STAGE_TWO_DATA_BOTH
        );
 ...
 */
goog.net.BrowserTestChannel.prototype.notifyServerReachabilityEvent = function (reachabilityType) {
