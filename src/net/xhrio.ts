...
goog.scope(() => {
  ...
  goog.net.XhrIo.protectEntryPoints = (errorHandler) => {
    goog.net.XhrIo.prototype.onReadyStateChangeEntryPoint_ = errorHandler.protectEntryPoint(
   ...
   * @private
   */
  goog.net.XhrIo.shouldUseXhr2Timeout_ = (xhr) =>
    goog.userAgent.IE &&
  ...
  goog.net.XhrIo.prototype.onProgressHandler_ = function (e, opt_isDownload) {
    ...
    this.dispatchEvent(
      goog.net.XhrIo.buildProgressEvent_(
        e,
        opt_isDownload ? goog.net.EventType.DOWNLOAD_PROGRESS : goog.net.EventType.UPLOAD_PROGRESS
      )
    );
  };
   ...
   * @private
   */
  goog.net.XhrIo.buildProgressEvent_ = (e, eventType) => ({
    type: eventType,
  ...
  goog.net.XhrIo.prototype.cleanUpXhr_ = function (opt_fromDispose) {
    if (this.xhr_) {
      ...
      const xhr = this.xhr_;
      const clearedOnReadyStateChange = this.xhrOptions_[
  ...
  goog.net.XhrIo.prototype.getResponse = function () {
    try {
      ...
      goog.log.error(
        this.logger_,
        'Response type ' + this.responseType_ + ' is not ' + 'supported on this browser'
      );
      return null;
    } catch (e) {
  ...
  goog.net.XhrIo.prototype.getResponseHeaders = function () {
    ...
    for (let i = 0; i < headersArray.length; i++) {
      ...
      const keyValue = goog.string.splitLimit(headersArray[i], ':', /* maxSplitCount= */ 1);
    ...
    return goog.object.map(headersObject, (values) => values.join(', '));
  };
