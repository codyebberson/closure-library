...
testSuite({
  testSingleHeader: () => {
    ...
    const encoded_value = HttpCors.generateEncodedHttpHeadersOverwriteParam(headers);
  ...
  testSetUrl: () => {
    ...
    const newUrl = HttpCors.setHttpHeadersWithOverwriteParam(url, '$httpHeaders', headers);
    assertEquals('/example.com/?%24httpHeaders=foo%3Abar%0D%0A', newUrl.toString());
  },
  ...
  testSetUrlAppend: () => {
    ...
    assertEquals('/example.com/?abc=12&%24httpHeaders=foo%3Abar%0D%0A', newUrlString);

    const url = new GoogUri(urlString);
    const newUrl = HttpCors.setHttpHeadersWithOverwriteParam(url, '$httpHeaders', headers);
    assertEquals('/example.com/?abc=12&%24httpHeaders=foo%3Abar%0D%0A', newUrl.toString());
  },

  testSetUrlMultiHeaders: () => {
    const headers = { foo1: 'bar1', foo2: 'bar2' };
    const urlString = '/example.com/';
    const newUrlString = HttpCors.setHttpHeadersWithOverwriteParam(
      urlString,
    ...
    assertEquals('/example.com/?%24httpHeaders=foo1%3Abar1%0D%0Afoo2%3Abar2%0D%0A', newUrlString);
