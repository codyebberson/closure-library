...
const UTF8_SURROGATE_PAIR_RANGES_STRING = '\uD800\uDC00\uD800\uDFFF\uDBFF\uDC00\uDBFF\uDFFF';
...
testSuite({
  ...
  testTextToByteArray() {
    ...
    assertArrayEquals('Latin', [83, 99, 104, 195, 182, 110], crypt.textToByteArray('Sch\u00f6n'));
  ...
  testUtf8ByteArrayToString() {
    ...
    assertEquals(
      'ASCII',
      'Hello, world',
      crypt.utf8ByteArrayToString([72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100])
    );

    assertEquals('Latin', 'Sch\u00f6n', crypt.utf8ByteArrayToString([83, 99, 104, 195, 182, 110]));
  ...
  testByteArrayToText() {
    ...
    assertEquals(
      'ASCII',
      'Hello, world',
      crypt.byteArrayToText([72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100])
    );

    assertEquals('Latin', 'Sch\u00f6n', crypt.byteArrayToText([83, 99, 104, 195, 182, 110]));
  ...
  testUint8ArrayToString() {
    ...
    let arr = new Uint8Array([72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100]);
  ...
  testByteArrayToBinaryString() {
    ...
    assertEquals('\xa0\x12\xa1', crypt.byteArrayToBinaryString([0xa0, 0x12, 0xa1]));
  },

  testStringToByteArray() {
    assertArrayEquals([], crypt.stringToByteArray(''));
    assertArrayEquals([97, 98, 99], crypt.stringToByteArray('abc'));
    assertArrayEquals([0xa0, 0x12, 0xa1], crypt.stringToByteArray('\xa0\x12\xa1'));
  ...
  testBinaryStringToByteArray() {
    ...
    assertArrayEquals([0xa0, 0x12, 0xa1], crypt.binaryStringToByteArray('\xa0\x12\xa1'));
  ...
  testByteArrayToHex() {
    assertEquals(
      // Java magic number
      'cafedead',
      crypt.byteArrayToHex([202, 254, 222, 173])
    );

    assertEquals(
      // IBM magic number
      'deadbeef',
      crypt.byteArrayToHex([222, 173, 190, 239])
    );
  ...
  testUint8ArrayToHex() {
    ...
    assertEquals(
      // Java magic number
      'cafedead',
      crypt.byteArrayToHex(new Uint8Array([202, 254, 222, 173]))
    );

    assertEquals(
      // IBM magic number
      'deadbeef',
      crypt.byteArrayToHex(new Uint8Array([222, 173, 190, 239]))
    );

    assertEquals('c0:ff:ee', crypt.byteArrayToHex(new Uint8Array([192, 255, 238]), ':'));
  },
  ...
  testXorUint8Array() {
    ...
    assertElementsEquals(
      [20, 83, 96, 66],
      crypt.xorByteArray(new Uint8Array([202, 254, 222, 173]), new Uint8Array([222, 173, 190, 239]))
    );
  },
