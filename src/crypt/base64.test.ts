...
function assertEncodings(input, expectedOutputs) {
  const { text, binary } = typeof input === 'string' ? { text: input, binary: input } : input;
  ...
  for (const name in base64.Alphabet) {
    ...
    assertEquals(expectedOutputs[alphabet], base64.encodeStringUtf8(text, alphabet));
    assertEquals(expectedOutputs[alphabet], base64.encodeText(text, alphabet));
    assertEquals(expectedOutputs[alphabet], base64.encodeString(binary, alphabet));
    assertEquals(expectedOutputs[alphabet], base64.encodeBinaryString(binary, alphabet));
  }
  // default case
  assertEquals(expectedOutputs[base64.Alphabet.DEFAULT], base64.encodeText(text));
  assertEquals(expectedOutputs[base64.Alphabet.DEFAULT], base64.encodeBinaryString(binary));

  // encodeByteArray with Array<number>
  for (const name in base64.Alphabet) {
    const alphabet = base64.Alphabet[name];
    assertEquals(expectedOutputs[alphabet], base64.encodeByteArray(arr, alphabet));
  }
  // default case
  assertEquals(expectedOutputs[base64.Alphabet.DEFAULT], base64.encodeByteArray(arr));
  ...
  if (SUPPORT_TYPED_ARRAY) {
    ...
    for (const name in base64.Alphabet) {
      const alphabet = base64.Alphabet[name];
      assertEquals(expectedOutputs[alphabet], base64.encodeByteArray(uint8Arr, alphabet));
    }
    // default case
    assertEquals(expectedOutputs[base64.Alphabet.DEFAULT], base64.encodeByteArray(uint8Arr));
  }
}
...
function assertDecodings(inputs, expectedOutput) {
  const textOutput = typeof expectedOutput === 'string' ? expectedOutput : expectedOutput.text;
  const binaryOutput = typeof expectedOutput === 'string' ? expectedOutput : expectedOutput.binary;
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    if (SUPPORT_TYPED_ARRAY) {
      assertObjectEquals(base64.decodeStringToUint8Array(input), uint8ArrOutput);
    }
  }
}
  ...
  testDecodeIgnoresSpace() {
    ...
    for (const [encoded, decoded] of spaceTests) {
      ...
      if (SUPPORT_TYPED_ARRAY) {
        ...
        assertObjectEquals(base64.decodeStringToUint8Array(encoded), decodedUint8Arr);
      }
    }
  },
});
