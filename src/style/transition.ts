...
goog.style.transition.set = (element, properties) => {
  ...
  var values = properties.map((p) => {
    if (typeof p === 'string') {
      ...
      goog.asserts.assert(
        p.property && typeof p.duration === 'number' && p.timing && typeof p.delay === 'number',
 ...
 */
goog.style.transition.isSupported = goog.functions.cacheReturnValue(() => {
 ...
 * @private
 */
goog.style.transition.setPropertyValue_ = (element, transitionValue) => {
