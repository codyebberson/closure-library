 ...
 * @return {void}
 */
goog.style.setStyle = (element, style, opt_value) => {
 ...
 * @private
 */
goog.style.setStyle_ = (element, value, style) => {
 ...
 * @private
 */
goog.style.getVendorStyleName_ = (element, style) => {
  var camelStyle = goog.string.toCamelCase(style);
 ...
 */
goog.style.getStyle = (element, property) => {
 ...
 * @private
 */
goog.style.getStyle_ = (element, style) =>
 ...
 */
goog.style.getComputedBoxSizing = (element) =>
 ...
 */
goog.style.getComputedPosition = (element) => goog.style.getStyle_(element, 'position');
 ...
 */
goog.style.getBackgroundColor = (element) => goog.style.getStyle_(element, 'backgroundColor');
 ...
 */
goog.style.getComputedOverflowX = (element) => goog.style.getStyle_(element, 'overflowX');
 ...
 */
goog.style.getComputedOverflowY = (element) => goog.style.getStyle_(element, 'overflowY');
 ...
 */
goog.style.getComputedZIndex = (element) => goog.style.getStyle_(element, 'zIndex');
 ...
 */
goog.style.getComputedTextAlign = (element) => goog.style.getStyle_(element, 'textAlign');
 ...
 */
goog.style.getComputedCursor = (element) => goog.style.getStyle_(element, 'cursor');
 ...
 */
goog.style.getComputedTransform = (element) => {
 ...
 * @return {void}
 */
goog.style.setPosition = (el, arg1, opt_arg2) => {
  var x, y;
 ...
 */
goog.style.getPosition = (element) =>
 ...
 */
goog.style.getClientViewportElement = (opt_node) => {
  var doc;
  if (opt_node) {
 ...
 */
goog.style.getViewportPageOffset = (doc) => {
  var body = doc.body;
 ...
 * @private
 */
goog.style.getBoundingClientRect_ = (el) => {
  try {
 ...
 */
goog.style.getOffsetParent = (element) => {
 ...
 */
goog.style.getVisibleRectForElement = (element) => {
  var visibleRect = new goog.math.Box(0, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, 0);
 ...
 */
goog.style.getContainerOffsetToScrollInto = (element, opt_container, opt_center) => {
 ...
 *     Defaults to false.
 */
goog.style.scrollIntoContainerView = (element, opt_container, opt_center) => {
  var container = opt_container || goog.dom.getDocumentScrollElement();
 ...
 */
goog.style.getClientLeftTop = (el) => new goog.math.Coordinate(el.clientLeft, el.clientTop);
 ...
 */
goog.style.getPageOffsetLeft = (el) => goog.style.getPageOffset(el).x;
 ...
 */
goog.style.getPageOffsetTop = (el) => goog.style.getPageOffset(el).y;
 ...
 * @return {!goog.math.Coordinate} The page offset.
 */
goog.style.getFramedPageOffset = (el, relativeWin) => {
 ...
 */
goog.style.translateRectForAnotherFrame = (rect, origBase, newBase) => {
 ...
 */
goog.style.getRelativePosition = (a, b) => {
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
goog.style.getClientPositionForElement_ = (el) => {
  var box = goog.style.getBoundingClientRect_(el);
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
goog.style.getClientPosition = (el) => {
 ...
 * @return {void}
 */
goog.style.setPageOffset = (el, x, opt_y) => {
 ...
 * @return {void}
 */
goog.style.setSize = (element, w, opt_h) => {
  var h;
 ...
 * @private
 */
goog.style.getPixelStyleValue_ = (value, round) => {
 ...
 * @private
 */
goog.style.evaluateWithTemporaryDisplay_ = (fn, element) => {
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
goog.style.getTransformedSize = (element) => {
 ...
 */
goog.style.toCamelCase = (selector) => goog.string.toCamelCase(String(selector));
 ...
 */
goog.style.toSelectorCase = (selector) => goog.string.toSelectorCase(selector);
 ...
 * @return {void}
 */
goog.style.setTransparentBackgroundImage = (el, src) => {
  var style = el.style;
 ...
 */
goog.style.showElement = (el, display) => {
 ...
 * @return {void}
 */
goog.style.setElementShown = (el, isShown) => {
 ...
 * @see #setElementShown
 */
goog.style.isElementShown = (el) => el.style.display != 'none';
 ...
 */
goog.style.installSafeStyleSheet = (safeStyleSheet, opt_node) => {
...
goog.style.uninstallStyles = (styleSheet) => {
  /** @suppress {strictMissingProperties} Added to tighten compiler checks */
  var node =
    styleSheet.ownerNode || styleSheet.owningElement || /** @type {Element} */ (styleSheet);
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
goog.style.setSafeStyleSheet = (element, safeStyleSheet) => {
 ...
 */
goog.style.isRightToLeft = (el) => 'rtl' == goog.style.getStyle_(el, 'direction');
 ...
 */
goog.style.setUnselectable = (el, unselectable, opt_noRecurse) => {
 ...
 */
goog.style.setBorderBoxSize = (element, size) => {
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
goog.style.getContentBoxSize = (element) => {
  var doc = goog.dom.getOwnerDocument(element);
 ...
 * @param {goog.math.Size} size  The new size.
 */
goog.style.setContentBoxSize = (element, size) => {
 ...
 * @private
 */
goog.style.setBoxSizingSize_ = (element, size, boxSizing) => {
  var style = element.style;
  if (goog.userAgent.GECKO) {
 ...
 * @private
 */
goog.style.getIePixelValue_ = (element, value, name, pixelName) => {
 ...
 * @private
 */
goog.style.getIePixelDistance_ = (element, propName) => {
 ...
 * @private
 */
goog.style.getBox_ = (element, stylePrefix) => {
 ...
 */
goog.style.getPaddingBox = (element) => goog.style.getBox_(element, 'padding');
 ...
 */
goog.style.getMarginBox = (element) => goog.style.getBox_(element, 'margin');
 ...
 * @private
 */
goog.style.getIePixelBorder_ = (element, prop) => {
 ...
 */
goog.style.getLengthUnits = (value) => {
  ...
  if (goog.userAgent.IE) {
    if (String(sizeUnits) in goog.style.ABSOLUTE_CSS_LENGTH_UNITS_) {
      ...
      return goog.style.getIePixelValue_(
        parentElement,
        fontSize == parentSize ? '1em' : fontSize,
        'left',
        'pixelLeft'
      );
    }
 ...
 */
goog.style.parseStyleAttribute = (value) => {
  var result = {};
  value.split(/\s*;\s*/).forEach((pair) => {
 ...
 */
goog.style.toStyleAttribute = (obj) => {
  var buffer = [];
  goog.object.forEach(obj, (value, key) => {
 ...
 *     element.
 */
goog.style.getFloat = (el) => el.style[goog.userAgent.IE ? 'styleFloat' : 'cssFloat'] || '';
 ...
 */
goog.style.getScrollbarWidth = (opt_className) => {
 ...
 */
goog.style.getCssTranslation = (element) => {
  ...
  return new goog.math.Coordinate(Number.parseFloat(matches[1]), Number.parseFloat(matches[2]));
};
