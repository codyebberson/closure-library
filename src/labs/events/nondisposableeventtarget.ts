...
goog.events.Listenable.addImplementation(goog.labs.events.NonDisposableEventTarget);
...
/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.getParentEventTarget = function () {
 ...
 */
goog.labs.events.NonDisposableEventTarget.prototype.setParentEventTarget = function (parent) {
  this.parentEventTarget_ = parent;
};

/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.dispatchEvent = function (e) {
  ...
  if (ancestor) {
    ...
    for (; ancestor; ancestor = ancestor.getParentEventTarget()) {
      ...
      goog.asserts.assert(
        ++ancestorCount < goog.labs.events.NonDisposableEventTarget.MAX_ANCESTORS_,
        'infinite loop'
      );
    }
  }

  return goog.labs.events.NonDisposableEventTarget.dispatchEventInternal_(this, e, ancestorsTree);
};
...
/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.unlistenByKey = function (key) {
  return this.eventTargetListeners_.removeByKey(key);
};

/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.removeAllListeners = function (opt_type) {
...
/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.getListeners = function (type, capture) {
...
/** @override */
goog.labs.events.NonDisposableEventTarget.prototype.hasListener = function (opt_type, opt_capture) {
 ...
 * @private
 */
goog.labs.events.NonDisposableEventTarget.prototype.assertInitialized_ = function () {
  goog.asserts.assert(
 ...
 */
goog.labs.events.NonDisposableEventTarget.dispatchEventInternal_ = (
  target,
  e,
  opt_ancestorsTree
) => {
  ...
  if (opt_ancestorsTree) {
    for (let i = opt_ancestorsTree.length - 1; !e.hasPropagationStopped() && i >= 0; i--) {
  ...
  if (opt_ancestorsTree) {
    for (let i = 0; !e.hasPropagationStopped() && i < opt_ancestorsTree.length; i++) {
      currentTarget = e.currentTarget = opt_ancestorsTree[i];
      rv = currentTarget.fireListeners(type, false, e) && rv;
    }
  }
