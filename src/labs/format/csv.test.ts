...
testSuite({
  ...
  testBadDelimitersTwoCharacter() {
    ...
    assertEquals('Assertion failed: Delimiter must be a single character.', e.message);
  },
  ...
  testIgnoringErrors() {
    ...
    assertObjectEquals([['a', '"Hello"World!"']], csv.parse('a,"Hello"World!"', true));
  ...
  testIgnoringErrorsTrailingTabs() {
    assertObjectEquals([['"a\tb"\t'], ['c,d']], csv.parse('"a\tb"\t\n"c,d"', true));
  },
  ...
  testGetLineDebugString() {
    ...
    assertEquals(ParseError.getLineDebugString_(str, column), 'abcdefghijklmnop\n' + '         ^');
  },
