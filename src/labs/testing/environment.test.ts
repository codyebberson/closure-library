...
function assertTestFailure(testCase, name, message) {
  assertContains(message, asserts.assert(testCase).getResult().resultsByName[name][0].toString());
}
...
testingTestSuite(
  (testSuite = {
    ...
    async testLifecycle_withObject() {
      ...
      const envThree = mockControl.createStrictMock(Environment);
      const testObject = {
        configureEnvironment: mockControl.createFunctionMock('configureEnvironment1'),
        setUp: mockControl.createFunctionMock('setUp1'),
        testSuite: {
          configureEnvironment: mockControl.createFunctionMock('configureEnvironment2'),
    ...
    async testLifecycle_withPromises() {
      ...
      registerEnvironment(testCase, envTwo);
      const testObj = {
        configureEnvironment: mockControl.createFunctionMock('configureEnvironment'),
        ...
        testBar: {
          configureEnvironment: mockControl.createFunctionMock('configureEnvironmentTest'),
    ...
    testAutoDiscoverTests() {
       ...
       * checking
       */
      globalThis.testDummy1 = () => {};
      /**
       * @suppress {strictMissingProperties} suppression added to enable type
       * checking
       */
      globalThis.testDummy2 = () => {};
    ...
    async testAsyncMockClock() {
      ...
      testCase.tearDown = async () => {
        await env.mockClock.tickAsync(Number.POSITIVE_INFINITY);
      };

      await runTestsReturningPromise(testCase);
      assertTestFailure(testCase, 'testThatThrowsEventuallyAsync', 'LateErrorMessage');
