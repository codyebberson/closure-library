...
goog.scope(() => {
  const WebChannelBaseTransport = goog.labs.net.webChannel.WebChannelBaseTransport;
   ...
   * @override
   */
  WebChannelBaseTransport.prototype.createWebChannel = (url, opt_options) =>
    new WebChannelBaseTransport.Channel(url, opt_options);
  ...
  WebChannelBaseTransport.Channel = function (url, opt_options) {
     ...
     */
    this.channel_ = new WebChannelBase(opt_options, goog.net.WebChannelTransport.CLIENT_VERSION);
    ...
    const httpHeadersOverwriteParam = opt_options && opt_options.httpHeadersOverwriteParam;
    if (httpHeadersOverwriteParam && !goog.string.isEmptyOrWhitespace(httpHeadersOverwriteParam)) {
     ...
     */
    this.supportsCrossDomainXhr_ = (opt_options && opt_options.supportsCrossDomainXhr) || false;
    ...
    if (httpSessionIdParam && !goog.string.isEmptyOrWhitespace(httpSessionIdParam)) {
      ...
      if (goog.object.containsKey(this.messageUrlParams_, httpSessionIdParam)) {
        ...
        goog.log.warning(
          this.logger_,
          'Ignore httpSessionIdParam also specified with messageUrlParams: ' + httpSessionIdParam
        );
      }
    }
   ...
   * @override
   */
  WebChannelBaseTransport.Channel.prototype.halfClose = () => {
    // to be implemented
   ...
   */
  WebChannelBaseTransport.Channel.prototype.messageToMapObject_ = function (message) {
    goog.asserts.assert(
  ...
  WebChannelBaseTransport.Channel.prototype.mapObjectToMessage_ = function (map) {
    if (Wire.RAW_DATA_KEY in map) {
      ...
      if (this.sendRawJson_) {
        return /** @type {!goog.net.WebChannel.MessageData} */ (goog.json.parse(rawMessage));
      } else {
        // string message
        return rawMessage;
      }
  ...
  goog.inherits(WebChannelBaseTransport.Channel.MessageEvent, goog.net.WebChannel.MessageEvent);
  ...
  goog.inherits(WebChannelBaseTransport.Channel.ErrorEvent, goog.net.WebChannel.ErrorEvent);
   ...
   * @override
   */
  WebChannelBaseTransport.Channel.Handler_.prototype.channelOpened = function (channel) {
   ...
   * @override
   */
  WebChannelBaseTransport.Channel.Handler_.prototype.channelHandleArray = function (
    channel,
    array
  ) {
    goog.asserts.assert(array, 'array expected to be defined');
    this.channel_.dispatchEvent(new WebChannelBaseTransport.Channel.MessageEvent(array));
  };

  /**
   * @override
   */
  WebChannelBaseTransport.Channel.Handler_.prototype.channelError = function (channel, error) {
    goog.log.info(
      this.channel_.logger_,
      'WebChannel aborted on ' + this.channel_.url_ + ' due to channel error: ' + error
    );
    this.channel_.dispatchEvent(new WebChannelBaseTransport.Channel.ErrorEvent(error));
  };
   ...
   * @final
   */
  WebChannelBaseTransport.ChannelProperties = function (transportChannel, channel) {
    /**
   ...
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.getConcurrentRequestLimit = function () {
   ...
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.getPendingRequestCount = function () {
   ...
   */
  WebChannelBaseTransport.ChannelProperties.prototype.getNonAckedMessages = function () {
    return this.channel_
      .getNonAckedMaps()
      .map((queued_map) => this.transportChannel_.mapObjectToMessage_(queued_map.map));
  };

  /**
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.getHttpSessionId = function () {
   ...
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.commit = function (callback) {
   ...
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.onCommit = goog.abstractMethod;

  /**
   * @override
   */
  WebChannelBaseTransport.ChannelProperties.prototype.ackCommit = goog.abstractMethod;

  /**
   * @override
   * @return {!Object<string, string>|undefined}
   */
  WebChannelBaseTransport.ChannelProperties.prototype.getLastResponseHeaders = function () {
    return this.channel_.getLastResponseHeaders();
  };

  /** @override */
  WebChannelBaseTransport.ChannelProperties.prototype.getLastStatusCode = function () {
