...
goog.scope(() => {
   ...
   */
  WebChannelDebug.prototype.xmlHttpChannelRequest = function (verb, uri, id, attempt, postData) {
    this.info(
      () =>
        'XMLHTTP REQ (' +
        id +
   ...
   */
  WebChannelDebug.prototype.xmlHttpChannelResponseText = function (id, responseText, opt_desc) {
    this.info(
      () =>
        'XMLHTTP TEXT (' +
        id +
   ...
   * @private
   */
  WebChannelDebug.prototype.maybeRedactArray_ = (array) => {
    if (array.length < 2) {
      return;
    }
