...
goog.scope(() => {
   ...
   * @private
   */
  requestStats.getStatEventTarget_ = () => {
    requestStats.eventTarget_ = requestStats.eventTarget_ || new goog.events.EventTarget();
 ...
 */
  requestStats.ServerReachabilityEvent = function (target, reachabilityType) {
    goog.events.Event.call(this, requestStats.Event.SERVER_REACHABILITY_EVENT, target);
   ...
   *     The reachability event type.
   */
  requestStats.notifyServerReachabilityEvent = (reachabilityType) => {
    const target = requestStats.getStatEventTarget_();
    target.dispatchEvent(new requestStats.ServerReachabilityEvent(target, reachabilityType));
  };
 ...
 */
   ...
   */
  requestStats.getStatEventTarget = () => requestStats.getStatEventTarget_();
 ...
 */
  ...
  requestStats.notifyTimingEvent = (size, rtt, retries) => {
    const target = requestStats.getStatEventTarget_();
    target.dispatchEvent(new requestStats.TimingEvent(target, size, rtt, retries));
  };
   ...
   */
  requestStats.setStartThreadExecutionHook = (startHook) => {
   ...
   */
  requestStats.setEndThreadExecutionHook = (endHook) => {
