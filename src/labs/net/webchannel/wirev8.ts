...
goog.scope(() => {
   ...
   */
  WireV8.prototype.encodeMessage = (message, buffer, opt_prefix) => {
    const prefix = opt_prefix || '';
    try {
      goog.structs.forEach(message, (value, key) => {
   ...
   */
  WireV8.prototype.encodeMessageQueue = function (messageQueue, count, badMapHandler) {
    let offset = -1;
    while (true) {
