...
goog.scope(() => {
  ...
  const ForwardChannelRequestPool = goog.labs.net.webChannel.ForwardChannelRequestPool;
   ...
   */
  const ALLOW_ORIGIN_TRIAL_FEATURES = goog.define(
    'goog.net.webChannel.ALLOW_ORIGIN_TRIAL_FEATURES',
    false
  );
   ...
   */
  const ENABLE_GOOGLE_COM_PING = goog.define('goog.net.webChannel.ENABLE_GOOGLE_COM_PING', true);
  ...
  function getInternalChannelParam(paramName, defaultValue, options) {
    ...
    return /** @type {T} */ (options.internalChannelParams[paramName]) || defaultValue;
  }
   ...
   */
  goog.labs.net.webChannel.WebChannelBase = function (opt_options, opt_clientVersion, opt_conn) {
    /**
     ...
     * @private {number}
     */
    this.baseRetryDelayMs_ = getInternalChannelParam('baseRetryDelayMs', 5 * 1000, opt_options);
     ...
     * @private {number}
     */
    this.retryDelaySeedMs_ = getInternalChannelParam('retryDelaySeedMs', 10 * 1000, opt_options);
     ...
     * @private {number}
     */
    this.forwardChannelMaxRetries_ = getInternalChannelParam(
     ...
     */
    this.xmlHttpFactory_ = (opt_options && opt_options.xmlHttpFactory) || undefined;

    /**
     * Whether or not this channel uses WHATWG Fetch/streams.
     * @private {boolean}
     */
    this.usesFetchStreams_ = (opt_options && opt_options.useFetchStreams) || false;
     ...
     * @private {boolean}
     */
    this.supportsCrossDomainXhrs_ = (opt_options && opt_options.supportsCrossDomainXhr) || false;
     ...
     * @private {boolean}
     */
    this.encodeInitMessageHeaders_ = (opt_options && opt_options.encodeInitMessageHeaders) || false;

    if (this.fastHandshake_ && this.encodeInitMessageHeaders_) {
      this.channelDebug_.warning('Ignore encodeInitMessageHeaders because fastHandshake is set.');
     ...
     * @private {boolean}
     */
    this.blockingHandshake_ = (opt_options && opt_options.blockingHandshake) || false;
    ...
    if (opt_options && opt_options.longPollingTimeout && opt_options.longPollingTimeout > 0) {
    ...
    this.enableOriginTrials_ =
      ALLOW_ORIGIN_TRIAL_FEATURES && (!opt_options || opt_options.enableOriginTrials !== false);
  ...
  WebChannelBase.prototype.disconnect = function () {
    ...
    if (this.state_ == WebChannelBase.State.OPENED) {
      ...
      const request = ChannelRequest.createChannelRequest(this, this.channelDebug_, this.sid_, rid);
  ...
  WebChannelBase.prototype.connectChannel_ = function () {
    ...
    this.forwardChannelUri_ = this.getForwardChannelUri(/** @type {string} */ (this.path_));
    ...
    try {
      ...
      environment.startOriginTrials(channelPath, (e) => {
        this.channelDebug_.dumpException(
          /** @type {?Error} */ (e),
          'Error in running origin trials'
        );
      });
   ...
   */
  WebChannelBase.prototype.setHttpHeadersOverwriteParam = function (httpHeadersOverwriteParam) {
   ...
   */
  WebChannelBase.prototype.setSupportsCrossDomainXhrs = function (supportCrossDomain) {
 ...
 */
  ...
  WebChannelBase.prototype.sendMap = function (map, opt_context) {
    ...
    this.outgoingMaps_.push(new Wire.QueuedMap(this.nextMapId_++, map, opt_context));
  ...
  WebChannelBase.prototype.setFailFast = function (failFast) {
    ...
    if (
      (this.forwardChannelRequestPool_.hasPendingRequest() || this.forwardChannelTimerId_) &&
      ...
      if (!this.forwardChannelRequestPool_.forceComplete(goog.bind(this.onRequestComplete, this))) {
   ...
   */
  WebChannelBase.prototype.getBackChannelMaxRetries = () => {
  ...
  WebChannelBase.prototype.maybeRetryForwardChannel_ = function (request) {
    if (
      ...
      this.forwardChannelRequestPool_.getMaxSize() - (this.forwardChannelTimerId_ ? 1 : 0)
    ) {
    ...
    if (this.forwardChannelTimerId_) {
      this.channelDebug_.debug('Use the retry request that is already scheduled.');
      this.outgoingMaps_ = request.getPendingMessages().concat(this.outgoingMaps_);
      return true;
    }
   ...
   * @private
   */
  WebChannelBase.prototype.onStartForwardChannelTimer_ = function (opt_retryRequest) {
  ...
  WebChannelBase.prototype.open_ = function () {
    ...
    const rid = this.nextRid_++;
    const request = ChannelRequest.createChannelRequest(this, this.channelDebug_, '', rid);
    ...
    if (this.httpHeadersOverwriteParam_ === null && !this.encodeInitMessageHeaders_) {
    ...
    if (this.getHttpSessionIdParam()) {
      uri.setParameterValue(WebChannel.X_HTTP_SESSION_ID, this.getHttpSessionIdParam());
    }
    ...
    if (extraHeaders) {
      if (this.encodeInitMessageHeaders_) {
        const encodedHeaders = httpCors.generateEncodedHttpHeadersOverwriteParam(extraHeaders);
  ...
  WebChannelBase.prototype.getMaxNumMessagesForFastHandshake_ = function () {
    ...
    for (let i = 0; i < this.outgoingMaps_.length; i++) {
      ...
      if (total === WebChannelBase.MAX_CHARS_PER_GET_ || i === this.outgoingMaps_.length - 1) {
        return i + 1;
      }
    }
   ...
   * @private
   */
  WebChannelBase.prototype.makeForwardChannelRequest_ = function (opt_retryRequest) {
    let rid;
    if (opt_retryRequest) {
  ...
  WebChannelBase.prototype.addAdditionalParams_ = function (uri) {
    if (this.extraParams_) {
      goog.object.forEach(this.extraParams_, (value, key) => {
    ...
    if (this.handler_) {
      ...
      if (params) {
        goog.structs.forEach(params, (value, key, coll) => {
          uri.setParameterValue(key, value);
        });
      }
    }
  ...
  WebChannelBase.prototype.dequeueOutgoingMaps_ = function (request, maxNum) {
    ...
    const badMapHandler = this.handler_
      ...
      : null;
    const result = this.wireCodec_.encodeMessageQueue(this.outgoingMaps_, count, badMapHandler);
  ...
  WebChannelBase.prototype.onStartBackChannelTimer_ = function () {
    ...
    if (this.backChannelRequest_ == null || this.handshakeRttMs_ <= 0) {
      this.channelDebug_.warning(
        'Skip bpDetectionTimerId_ ' + this.backChannelRequest_ + ' ' + this.handshakeRttMs_
      );
      return;
    }
    ...
    this.bpDetectionTimerId_ = requestStats.setTimeout(
      goog.bind(this.onBpDetectionTimer_, this),
      bpDetectionTimeout
    );
  };
  ...
  WebChannelBase.prototype.onBpDetectionTimer_ = function () {
    ...
    if (this.backChannelRequest_.getXhr() != null) {
      ...
      if (responseData) {
        this.channelDebug_.warning('Timer should have been cancelled : ' + responseData);
      }
    }
  ...
  WebChannelBase.prototype.startBackChannel_ = function () {
    ...
    this.backChannelRequest_.xmlHttpGet(uri, true /* decodeChunks */, this.hostPrefix_);
  ...
  WebChannelBase.prototype.onFirstByteReceived = function (request, responseText) {
    if (this.backChannelRequest_ == request && this.detectBufferingProxy_) {
      if (!this.bpDetectionDone_) {
        ...
        goog.asserts.assert(this.bpDetectionTimerId_, 'Timer should not have been cancelled.');
        this.clearBpDetectionTimer_();
        this.bpDetectionDone_ = true;
  ...
  WebChannelBase.prototype.onRequestData = function (request, responseText) {
    if (
      this.state_ == WebChannelBase.State.CLOSED ||
      (this.backChannelRequest_ != request && !this.forwardChannelRequestPool_.hasRequest(request))
    ) {
    ...
    if (
      ...
      if (request.isInitialResponseDecoded() || this.backChannelRequest_ == request) {
        this.clearDeadBackchannelTimer_();
      }

      if (!goog.string.isEmptyOrWhitespace(responseText)) {
        const response = this.wireCodec_.decodeMessage(responseText);
   ...
   * @private
   */
  WebChannelBase.prototype.handlePostResponse_ = function (responseValues, forwardReq) {
  ...
  WebChannelBase.prototype.handleBackchannelMissing_ = function (forwardReq) {
    ...
    if (this.backChannelTimerId_) {
      ...
      this.backChannelRequest_.getRequestStartTime() + WebChannelBase.RTT_ESTIMATE <
  ...
  WebChannelBase.prototype.shouldRetryBackChannel_ = function (outstandingBytes) {
    ...
    return (
      outstandingBytes < WebChannelBase.OUTSTANDING_DATA_BACKCHANNEL_RETRY_CUTOFF &&
      !this.isBuffered() &&
      this.backChannelRetryCount_ == 0
    );
  };
   ...
   * @private
   */
  WebChannelBase.isFatalError_ = (error, statusCode) =>
    ...
    if (request.getSuccess()) {
      if (type == WebChannelBase.ChannelType_.FORWARD_CHANNEL) {
        ...
        requestStats.notifyTimingEvent(
          size,
          Date.now() - request.getRequestStartTime(),
          this.forwardChannelRetryCount_
        );
        this.ensureForwardChannel_();
    ...
    if (!WebChannelBase.isFatalError_(lastError, lastStatusCode)) {
      this.channelDebug_.debug(
        () =>
          ...
          ChannelRequest.errorStringFromCode(lastError, this.errorResponseStatusCode_)
      );
      if (type == WebChannelBase.ChannelType_.FORWARD_CHANNEL) {
  ...
  WebChannelBase.prototype.getRetryTime_ = function (retryCount) {
    let retryTime = this.baseRetryDelayMs_ + Math.floor(Math.random() * this.retryDelaySeedMs_);
    if (!this.isActive()) {
  ...
  WebChannelBase.prototype.applyControlHeaders_ = function (request) {
    ...
    if (xhr) {
      const clientProtocol = xhr.getStreamingResponseHeader(WebChannel.X_CLIENT_WIRE_PROTOCOL);
      if (clientProtocol) {
        this.forwardChannelRequestPool_.applyClientProtocol(clientProtocol);
      }

      if (this.getHttpSessionIdParam()) {
        const httpSessionIdHeader = xhr.getStreamingResponseHeader(WebChannel.X_HTTP_SESSION_ID);
  ...
  WebChannelBase.prototype.onInput_ = function (respArray, request) {
    const batch = this.handler_ && this.handler_.channelHandleMultipleArrays ? [] : null;
    for (let i = 0; i < respArray.length; i++) {
      ...
      if (this.state_ == WebChannelBase.State.OPENING) {
        if (nextArray[0] == 'c') {
          ...
          if (
            serverKeepaliveMs != null &&
            typeof serverKeepaliveMs === 'number' &&
            serverKeepaliveMs > 0
          ) {
          ...
          if (this.detectBufferingProxy_) {
            ...
            this.channelDebug_.info('Handshake RTT: ' + this.handshakeRttMs_ + 'ms');
          }
  ...
  WebChannelBase.prototype.onClose_ = function () {
    ...
    if (this.handler_) {
      const pendingMessages = this.forwardChannelRequestPool_.getPendingMessages();
  ...
  WebChannelBase.prototype.getNonAckedMaps = function () {
    ...
    // callers.
    const unAckedMaps = [];
    goog.array.extend(unAckedMaps, this.forwardChannelRequestPool_.getPendingMessages());
  ...
  WebChannelBase.prototype.getBackChannelUri = function (hostPrefix, path) {
    const uri = this.createDataUri(this.shouldUseSecondaryDomains() ? hostPrefix : null, path);
   ...
   * @override
   */
  WebChannelBase.prototype.createDataUri = function (hostPrefix, path, opt_overridePort) {
   ...
   */
  WebChannelBase.Handler.prototype.okToMakeRequest = (channel) => WebChannelBase.Error.OK;
   ...
   * @param {WebChannelBase} channel The channel.
   */
  WebChannelBase.Handler.prototype.channelOpened = (channel) => {};
   ...
   */
  WebChannelBase.Handler.prototype.channelHandleArray = (channel, array) => {};
   ...
   */
  WebChannelBase.Handler.prototype.channelSuccess = (channel, request) => {};
   ...
   */
  WebChannelBase.Handler.prototype.channelError = (channel, error) => {};
   ...
   */
  WebChannelBase.Handler.prototype.getAdditionalParams = (channel) => ({});
   ...
   */
  WebChannelBase.Handler.prototype.getNetworkTestImageUri = (channel) => null;
   ...
   */
  WebChannelBase.Handler.prototype.isActive = (channel) => true;
   ...
   */
  WebChannelBase.Handler.prototype.badMapError = (channel, map) => {};
   ...
   */
  WebChannelBase.Handler.prototype.correctHostPrefix = (serverHostPrefix) => serverHostPrefix;
}); // goog.scope
