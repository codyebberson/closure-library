...
goog.labs.net.webChannel.ChannelRequest = function (
   ...
   * @private {goog.Timer}
   */
  this.pollingTimer_ = new goog.Timer(goog.labs.net.webChannel.environment.getPollingInterval());
...
goog.scope(() => {
   ...
   */
  ChannelRequest.errorStringFromCode = (errorCode, statusCode) => {
    switch (errorCode) {
  ...
  ChannelRequest.prototype.sendXmlHttp_ = function (hostPrefix) {
    ...
    const headers = this.extraHeaders_ ? goog.object.clone(this.extraHeaders_) : {};
    if (this.postData_) {
      if (!this.verb_) {
        this.verb_ = 'POST';
      }
    ...
    requestStats.notifyServerReachabilityEvent(requestStats.ServerReachability.REQUEST_MADE);
  ...
  ChannelRequest.prototype.readyStateChangeHandler_ = function (evt) {
    ...
    if (throttle && xhr.getReadyState() == goog.net.XmlHttp.ReadyState.INTERACTIVE) {
  ...
  ChannelRequest.prototype.onXmlHttpReadyStateChanged_ = function () {
    ...
    if (
      ...
      if (errorCode == goog.net.ErrorCode.TIMEOUT || statusCode <= 0) {
        requestStats.notifyServerReachabilityEvent(requestStats.ServerReachability.REQUEST_FAILED);
      } else {
    ...
    if (!this.successful_) {
      ...
      if (status == 400 && responseText.indexOf('Unknown SID') > 0) {
        ...
        requestStats.notifyStatEvent(requestStats.Stat.REQUEST_UNKNOWN_SESSION_ID);
        ...
        this.channelDebug_.warning('XMLHTTP Bad status ' + status + ' (' + this.rid_ + ')');
      }
      this.cleanup_();
  ...
  ChannelRequest.prototype.getInitialResponse_ = function () {
    if (this.xmlHttp_) {
      const value = this.xmlHttp_.getStreamingResponseHeader(WebChannel.X_HTTP_INITIAL_RESPONSE);
  ...
  ChannelRequest.prototype.decodeXmlHttpResponse_ = function () {
    ...
    const responseChunks = /** @type {!Array<!Uint8Array>} */ (this.xmlHttp_.getResponse());
    let responseText = '';
    const responseLength = responseChunks.length;
    const requestCompleted = this.xmlHttp_.getReadyState() == goog.net.XmlHttp.ReadyState.COMPLETE;
    if (!this.fetchResponseState_.textDecoder) {
      if (typeof TextDecoder === 'undefined') {
        this.channelDebug_.severe('TextDecoder is not supported by this browser.');
        this.cleanup_();
        this.dispatchFailure_();
        return '';
      }
    ...
    for (let i = 0; i < responseLength; i++) {
      ...
      responseText += this.fetchResponseState_.textDecoder.decode(responseChunks[i], {
        stream: isLastChunk,
      });
    }
   ...
   * @private
   */
  ChannelRequest.prototype.decodeNextChunks_ = function (readyState, responseText) {
    ...
    if (this.useFetchStreamsForResponse_() && this.xmlHttpChunkStart_ != 0) {
      ...
      this.fetchResponseState_.responseBuffer = this.fetchResponseState_.responseBuffer.slice(
        this.xmlHttpChunkStart_
      );
      this.xmlHttpChunkStart_ = 0;
    }
  ...
  ChannelRequest.prototype.startWatchDogTimer_ = function (time) {
    ...
    this.watchDogTimerId_ = requestStats.setTimeout(goog.bind(this.onWatchDogTimeout_, this), time);
  };
