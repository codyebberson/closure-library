...
goog.scope(() => {
   ...
   */
  xhr.get = (url, opt_options) =>
    xhr.send('GET', url, null, opt_options).then((request) => request.responseText);
   ...
   *     response text once the request completes.
   */
  xhr.post = (url, data, opt_options) =>
    xhr.send('POST', url, data, opt_options).then((request) => request.responseText);
   ...
   */
  xhr.getJson = (url, opt_options) =>
    xhr
      .send('GET', url, null, opt_options)
      .then((request) => xhr.parseJson_(request.responseText, opt_options));
   ...
   */
  xhr.getBlob = (url, opt_options) => {
    goog.asserts.assert('Blob' in goog.global, 'getBlob is not supported in this browser.');

    const options = /** @type {!xhr.Options} */ (opt_options ? goog.object.clone(opt_options) : {});
    options.responseType = xhr.ResponseType.BLOB;

    return xhr.send('GET', url, null, options).then((request) => request.response);
  };
  ...
  xhr.getBytes = (url, opt_options) => {
    ...
    return xhr.send('GET', url, null, options).then((request) => {
   ...
   *     response JSON once the request completes.
   */
  xhr.postJson = (url, data, opt_options) =>
    xhr
      .send('POST', url, data, opt_options)
      .then((request) => xhr.parseJson_(request.responseText, opt_options));
      ...
      (resolve, reject) => {
        ...
        request.onreadystatechange = () => {
          if (request.readyState == goog.net.XmlHttp.ReadyState.COMPLETE) {
            ...
            // as a success too.
            if (
              HttpStatus.isSuccess(request.status) ||
        ...
        const dataIsFormData = goog.global['FormData'] && data instanceof goog.global['FormData'];
   ...
   */
  xhr.parseJson = (responseText, opt_xssiPrefix) =>
    xhr.parseJson_(responseText, { xssiPrefix: opt_xssiPrefix });
   ...
   * @private
   */
  xhr.parseJson_ = (responseText, options) => {
   ...
   * @private
   */
  xhr.stripXssiPrefix_ = (prefix, string) => {
  ...
  xhr.HttpError = function (status, url, request) {
    xhr.HttpError.base(this, 'constructor', 'Request Failed, status=' + status, url, request);
