...
testSuite({
  testIntersection() {
    testTwoSetLikeImplInterop((setACtor, setBCtor) => {
      ...
      assertSameElements([1], sets.intersection(new setACtor([1, 1, 1]), [1, 1]));

      // generators
      assertSameElements([2], sets.intersection(new setACtor([1, 2]), yieldArguments(2, 3)));
      assertSameElements([], sets.intersection(new setACtor([]), emptyGenerator()));
      assertSameElements([1], sets.intersection(new setACtor([1, 1, 1]), yieldArguments(1, 1)));

      // sets
      assertSameElements([2], sets.intersection(new setACtor([1, 2]), new setBCtor([2, 3])));
      assertSameElements([], sets.intersection(new setACtor([]), new setBCtor()));
      assertSameElements([1], sets.intersection(new setACtor([1, 1, 1]), new setBCtor([1, 1])));
    });
  },
  ...
  testUnion() {
    testTwoSetLikeImplInterop((setACtor, setBCtor) => {
      ...
      // generators
      assertSameElements([1, 2, 3], sets.union(new setACtor([1, 2]), yieldArguments(2, 3)));
      assertSameElements([], sets.union(new setACtor([]), emptyGenerator()));
      assertSameElements([1], sets.union(new setACtor([1, 1, 1]), yieldArguments(1, 1)));

      // sets
      assertSameElements([1, 2, 3], sets.union(new setACtor([1, 2]), new setBCtor([2, 3])));
      assertSameElements([], sets.union(new setACtor([]), new setBCtor()));
      assertSameElements([1], sets.union(new setACtor([1, 1, 1]), new setBCtor([1, 1])));
    });
  },
  ...
  testDifference() {
    testTwoSetLikeImplInterop((setACtor, setBCtor) => {
      ...
      // generators
      assertSameElements([1], sets.difference(new setACtor([1, 2]), yieldArguments(2, 3)));
      assertSameElements([], sets.difference(new setACtor([]), emptyGenerator()));
      assertSameElements([], sets.difference(new setACtor([1, 1, 1]), yieldArguments(1, 1)));

      // sets
      assertSameElements([1], sets.difference(new setACtor([1, 2]), new setBCtor([2, 3])));
      assertSameElements([], sets.difference(new setACtor([]), new setBCtor()));
      assertSameElements([], sets.difference(new setACtor([1, 1, 1]), new setBCtor([1, 1])));
    });
  },

  testSymmetricDifference() {
    // sets (only sets are accepted for symmetricDifference)
    assertSameElements([1, 3], sets.symmetricDifference(new Set([1, 2]), new Set([2, 3])));
    assertSameElements([], sets.symmetricDifference(new Set([]), new Set()));
    assertSameElements([], sets.symmetricDifference(new Set([1, 1, 1]), new Set([1, 1])));
  },
  ...
  testRemoveAll() {
    const testRemoveAllImpl = (msg, elements1, elements2, expectedResult) => {
      testTwoSetLikeImplInterop((setACtor, setBCtor) => {
        const set1 = new setACtor(elements1);
        const set2 = new setBCtor(elements2);
        sets.removeAll(set1, set2);

        assertTrue(`${msg}: set1 count increased after removeAll`, elements1.length >= set1.size);
    ...
    testRemoveAllImpl('removeAll of [a,b,c,d] from [a,d]', ['a', 'd'], ['a', 'b', 'c', 'd'], []);
    testRemoveAllImpl('removeAll of [a,b,c,d] from [b,c]', ['b', 'c'], ['a', 'b', 'c', 'd'], []);
    testRemoveAllImpl(
  ...
  testHasAll() {
    testTwoSetLikeImplInterop((setACtor, setBCtor) => {
      ...
      assertTrue('{1, 2, 3} contains {a: 1}', sets.hasAll(s, Object.values({ a: 1 })));
      assertFalse("{1, 2, 3} doesn't contain {a: 4}", sets.hasAll(s, Object.values({ a: 4 })));

      assertTrue('{1, 2, 3} contains {1}', sets.hasAll(s, new setBCtor([1])));
      assertFalse("{1, 2, 3} doesn't contain {4}", sets.hasAll(s, new setBCtor([4])));
    });
  },
  ...
  testEquals() {
    ...
    const testEqualsImpl = (a, b, c, d) => {
      testTwoSetLikeImplInterop((setACtor, setBCtor) => {
        ...
        assertTrue('set == array of same elements in different order', sets.equals(s, [c, b, a]));

        assertFalse('set != empty set', sets.equals(s, new setBCtor()));
        assertFalse('set != its subset', sets.equals(s, new setBCtor([a, c])));
        assertFalse('set != its superset', sets.equals(s, new setBCtor([a, b, c, d])));
        assertFalse('set != different set', sets.equals(s, new setBCtor([b, c, d])));
        assertFalse('set != its subset as array', sets.equals(s, [a, c]));
        assertFalse('set != its superset as array', sets.equals(s, [a, b, c, d]));
  ...
  testIsSubsetOf() {
     ...
     * @param {?} d Fourth element to use in the tests.
     */
    const testSubsetOfImpl = (a, b, c, d) => {
      testTwoSetLikeImplInterop((setACtor, setBCtor) => {
        const s = new setACtor([a, b, c]);

        assertTrue('set <= itself', sets.isSubsetOf(s, s));
        assertTrue('set <= same set', sets.isSubsetOf(s, new setBCtor([a, b, c])));
        assertTrue('set <= its clone', sets.isSubsetOf(s, new setBCtor(s)));
        assertTrue('set <= array of same elements', sets.isSubsetOf(s, [a, b, c]));
        assertTrue('set <= array of same elements in different order', sets.equals(s, [c, b, a]));

        assertTrue('set <= Set([a, b, c, d])', sets.isSubsetOf(s, new setBCtor([a, b, c, d])));
        assertTrue('set <= [a, b, c, d]', sets.isSubsetOf(s, [a, b, c, d]));
        assertTrue('set <= [a, b, c, c]', sets.isSubsetOf(s, [a, b, c, c]));

        assertFalse('set !<= Set([a, b])', sets.isSubsetOf(s, new setBCtor([a, b])));
        assertFalse('set !<= [a, b]', sets.isSubsetOf(s, [a, b]));
        assertFalse('set !<= Set([c, d])', sets.isSubsetOf(s, new setBCtor([c, d])));
        assertFalse('set !<= [c, d]', sets.isSubsetOf(s, [c, d]));
        assertFalse('set !<= Set([a, c, d])', sets.isSubsetOf(s, new setBCtor([a, c, d])));
