...
testSuite({
  ...
  testGetCompatModeQuirks() {
    ...
    expectedFailures.run(() => {
      assertFalse(
        ...
        editorNode.isStandardsMode(googDom.getFrameContentDocument(quirksIfr))
      );
    });
  ...
  testGetCompatModeStandards() {
    ...
    doc.close();
    assertTrue('Iframe with DOCTYPE written in is standards mode', editorNode.isStandardsMode(doc));
  ...
  testGetLeftMostLeaf() {
    ...
    assertEquals('Should skip ws node', gChildMixedNode1, editorNode.getLeftMostLeaf(parentNode));
    assertEquals('Should skip ws node', gChildMixedNode1, editorNode.getLeftMostLeaf(childNode1));
    assertEquals('Has no non ws leaves', childNode2, editorNode.getLeftMostLeaf(childNode2));
    assertEquals(
      'Should return first child',
      gChildTextNode3a,
      editorNode.getLeftMostLeaf(childNode3)
    );
    assertEquals('Has no children', gChildTextNode1, editorNode.getLeftMostLeaf(gChildTextNode1));
  ...
  testGetRightMostLeaf() {
    ...
    assertEquals(
      "Should return child3's rightmost child",
      gChildTextNode3b,
      editorNode.getRightMostLeaf(parentNode)
    );
    assertEquals('Should skip ws node', gChildTextNode1, editorNode.getRightMostLeaf(childNode1));
    assertEquals('Has no non ws leaves', childNode2, editorNode.getRightMostLeaf(childNode2));
    assertEquals(
      'Should return last child',
      gChildTextNode3b,
      editorNode.getRightMostLeaf(childNode3)
    );
    assertEquals('Has no children', gChildTextNode1, editorNode.getRightMostLeaf(gChildTextNode1));
  ...
  testGetFirstChild() {
    ...
    assertEquals(
      'Should skip first child, as it is ws',
      gChildMixedNode1,
      editorNode.getFirstChild(childNode1)
    );
    assertEquals(
      'Should just return first child',
      gChildTextNode3a,
      editorNode.getFirstChild(childNode3)
    );
    assertEquals('Should return first child', childNode1, editorNode.getFirstChild(parentNode));

    assertNull(
      'First child of a text node should return null',
      editorNode.getFirstChild(gChildTextNode1)
    );
    assertNull('First child of null should return null', editorNode.getFirstChild(null));
  ...
  testGetLastChild() {
    ...
    assertEquals(
      'Should skip last child, as it is ws',
      gChildTextNode1,
      editorNode.getLastChild(childNode1)
    );
    assertEquals(
      'Should just return last child',
      gChildTextNode3b,
      editorNode.getLastChild(childNode3)
    );
    assertEquals('Should return last child', childNode3, editorNode.getLastChild(parentNode));

    assertNull(
      'Last child of a text node should return null',
      editorNode.getLastChild(gChildTextNode1)
    );
    assertNull('Last child of null should return null', editorNode.getLastChild(gChildTextNode1));
  ...
  testIsImportant() {
    ...
    assertFalse('White space node is ignorable', editorNode.isImportant(wsNode));
  ...
  testIsAllNonNbspWhiteSpace() {
    const wsNode = document.createTextNode(' \t\r\n');
    assertTrue('String is all non nbsp', editorNode.isAllNonNbspWhiteSpace(wsNode));
    const textNode = document.createTextNode('Hello');
    assertFalse('String should not be whitespace', editorNode.isAllNonNbspWhiteSpace(textNode));
    const nbspNode = document.createTextNode('\u00a0');
  ...
  testGetPreviousSibling() {
    ...
    assertNull('No previous sibling', editorNode.getPreviousSibling(gChildTextNode3a));
    assertEquals(
    ...
    assertEquals(
      'Should skip over white space sibling',
      gChildTextNode3a,
      editorNode.getPreviousSibling(gChildTextNode3b)
    );
    assertNull('No previous sibling', editorNode.getPreviousSibling(gChildMixedNode1));
    assertEquals(
      'Should have mixed text sibling',
      gChildMixedNode1,
    ...
    assertNotEquals(
      'Should not move past ws and nbsp',
      gChildMixedNode1,
      editorNode.getPreviousSibling(gChildTextNode1)
    );
    assertEquals('Should go to child 2', childNode2, editorNode.getPreviousSibling(childNode3));
    assertEquals('Should go to child 1', childNode1, editorNode.getPreviousSibling(childNode2));
    assertNull('Only has white space siblings', editorNode.getPreviousSibling(gChildWsNode2b));
  ...
  testGetNextSibling() {
    ...
    assertEquals('Child 1 should have Child 2', childNode2, editorNode.getNextSibling(childNode1));
    assertEquals('Child 2 should have child 3', childNode3, editorNode.getNextSibling(childNode2));
    assertNull('Child 3 has no next sibling', editorNode.getNextSibling(childNode3));
    assertNotEquals(
      'Should not skip ws and nbsp nodes',
      gChildTextNode1,
      editorNode.getNextSibling(gChildMixedNode1)
    );
    assertNotEquals(
      'Should not skip nbsp node',
      gChildTextNode1,
    ...
    assertNull('Should have no next sibling', editorNode.getNextSibling(gChildTextNode1));
    assertNull('Only has ws sibling', editorNode.getNextSibling(gChildWsNode2a));
  ...
  testIsEmpty() {
    ...
    assertTrue('Text node with no content should be empty', editorNode.isEmpty(textNode));
    textNode.data = '\xa0';
    assertTrue('Text node with nbsp should be empty', editorNode.isEmpty(textNode));
    assertFalse(
    ...
    textNode.data = '     ';
    assertTrue('Text node with whitespace should be empty', editorNode.isEmpty(textNode));
    textNode.data = 'notEmpty';
    assertFalse('Text node with text should not be empty', editorNode.isEmpty(textNode));

    const div = googDom.createElement(TagName.DIV);
    assertTrue('Empty div should be empty', editorNode.isEmpty(div));
    div.innerHTML = '<iframe></iframe>';
    assertFalse('Div containing an iframe is not empty', editorNode.isEmpty(div));
    div.innerHTML = '<img></img>';
    assertFalse('Div containing an image is not empty', editorNode.isEmpty(div));
    div.innerHTML = '<embed></embed>';
    assertFalse('Div containing an embed is not empty', editorNode.isEmpty(div));
    div.innerHTML = '<div><span></span></div>';
    assertTrue('Div containing other empty tags is empty', editorNode.isEmpty(div));
    div.innerHTML = '<div><span>  </span></div>';
    assertTrue('Div containing other empty tags and whitespace is empty', editorNode.isEmpty(div));
    div.innerHTML = '<div><span>Not empty</span></div>';
    assertFalse('Div containing tags and text is not empty', editorNode.isEmpty(div));
  ...
  testGetLength() {
    ...
    assertEquals('Length 0 and no children', 0, editorNode.getLength(parentNode));
    ...
    assertEquals('Length 0 and 3 children', 3, editorNode.getLength(parentNode));
  ...
  testFindHighestMatchingAncestor() {
    ...
    let node = editorNode.findHighestMatchingAncestor(gChildTextNode3a, predicateFunc);
    assertNotNull('Should return an ancestor', node);
    assertEquals(
      'Should have found "parentNode" as the last ' + 'ancestor matching the predicate',
      parentNode,
      node
    );

    predicateFunc = (node) => node.childNodes.length == 1;
    node = editorNode.findHighestMatchingAncestor(gChildTextNode3a, predicateFunc);
  ...
  testIsBlock() {
    ...
    for (const tag in TagName) {
      ...
      const display =
        style.getCascadedStyle(el, 'display') || style.getComputedStyle(el, 'display');
  ...
  testSkipEmptyTextNodes() {
    ...
    assertEquals(
      'expected not to skip first child',
      dom1.firstChild,
      editorNode.skipEmptyTextNodes(dom1.firstChild)
    );
    assertEquals(
      'expected to skip second child',
      dom1.childNodes[2],
  ...
  testIsEditable() {
    ...
    const childElement = googDom.getElementsByTagName(TagName.SPAN, editableContainerElement)[0];

    assertFalse(
      'Container element should not be considered editable',
      editorNode.isEditable(editableContainerElement)
    );
    assertTrue('Child text node should be considered editable', editorNode.isEditable(childNode));
    assertTrue('Child element should be considered editable', editorNode.isEditable(childElement));
    assertTrue(
  ...
  testFindTopMostEditableAncestor() {
    ...
    assertEquals(
      'Should return self if self is matched.',
      textNode,
      editorNode.findTopMostEditableAncestor(textNode, (node) => node.nodeType == NodeType.TEXT)
    );
    assertEquals(
      'Should not walk out of editable node.',
      null,
    ...
    assertEquals(
      'Should not match editable container.',
      null,
      editorNode.findTopMostEditableAncestor(
        textNode /**
                         @suppress {strictMissingProperties} suppression added
    ...
    assertEquals(
      'Should find node in editable container.',
      span,
      editorNode.findTopMostEditableAncestor(
        textNode /**
                         @suppress {strictMissingProperties} suppression added
