...
function setUpTableTests() {
  ...
  div.innerHTML =
    '<table><tr> <th> head1</th><th id= "outerTh">' +
...
function runExpectingFailuresForUnimplementedBrowsers(func) {
  ...
  expectedFailures.expectFailureFor(userAgent.IE, 'Proper behavior not yet implemented for IE.');
...
testSuite({
  ...
  setUpPage() {
    ...
    if (WEBKIT_AFTER_CHROME_21) {
      ...
      if (product.isVersion(14)) {
        ...
        insertImageFontGarbage = '<fontsize="-1"><font class="p" size="-1"></font></fontsize="-1">';
      }
    }
  ...
  testTableTagsAreNotRemoved() {
    ...
    if (!userAgent.WEBKIT && !userAgent.EDGE) {
      ...
      assertEquals('TD should be clean', 'four', googString.trim(elem.innerHTML));
    }
  ...
  testTableDataIsNotRemoved() {
    ...
    if (userAgent.IE) {
      ...
      Range.createFromNodeContents(document.getElementById('outerTr2')).select();
    } else {
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        const span = document.getElementById('outerTr2');
        assertEquals(
  ...
  testLinksAreNotRemoved() {
    ...
    const div = document.getElementById('html');
    div.innerHTML =
      'Foo<span id="link">Pre<a href="http://www.google.com">' +
  ...
  testRelativeLinksAreNotAbsolutified() {
    ...
    const div = document.getElementById('html');
    div.innerHTML =
      'Foo<span id="link">Pre<a href="/hello.html">' +
      'Outside Span<span style="font-size:15pt">Inside Span' +
      '</span></a></span>';
  ...
  testAlternateRemoveFormattingFunction() {
    ...
    assertHTMLEquals(
      'regular cleaner should not have run',
  ...
  testRemoveFormattingAddsNoNbsps() {
    ...
    assertEquals(
      'Text should be the same, with no non-breaking spaces',
      '"Twin Cinema"',
      div.innerHTML
    );
  ...
  testRemoveFormattingNestedDivs() {
    /** @suppress {visibility} suppression added to enable type checking */
    const html = FORMATTER.removeFormattingWorker_('<div>1</div><div><div>2</div></div>');
  ...
  testTheJavascriptReplaceMetacharacters() {
    ...
    const expected = '123 $&lt; $&gt; $" $&amp; $$ $` $\' 456' + (product.SAFARI ? '<br>' : '');
    ...
    FORMATTER.removeFormatting_();
    assertHTMLEquals(
      'String.prototype.replace metacharacters should not trigger',
      expected,
      div.innerHTML
    );
  },
  ...
  testRemoveFormattingForTableFormatting() {
    ...
    let html =
      '<table><tr><td>cell00</td><td>cell01</td></tr>' +
  ...
  testRemoveFormattingDoesNotShrinkSelection() {
    ...
    if (userAgent.EDGE) {
      // TODO(sdh): I have no idea where this comes from
  ...
  testInsideListRemoveFormat() {
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        assertHTMLEquals('<ul><li>one</li></ul>two<ul><li>three</li></ul>', div.innerHTML);
        FIELDMOCK.$verify();
      }
    );
  },
  ...
  testFullListRemoveFormat() {
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        assertHTMLEquals('one<br>two<br>threeafter', div.innerHTML);
        FIELDMOCK.$verify();
      }
    );
  },
  ...
  testPartialListRemoveFormat() {
    ...
    Range.createFromNodes(div.firstChild.childNodes[1], 0, div.lastChild, 5).select();

    expectedFailures.expectFailureFor(userAgent.IE, 'IE leaves behind an empty LI.');
    expectedFailures.expectFailureFor(userAgent.WEBKIT, 'WebKit completely loses the "one".');
    if (userAgent.EDGE) {
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        assertHTMLEquals('<ul><li>one</li></ul>two<br>threeafter', div.innerHTML);
        FIELDMOCK.$verify();
      }
    );
  },
  ...
  testPartialBasicRemoveFormatting() {
    ...
    Range.createFromNodes(
      ...
      2,
      div.firstChild.lastChild.firstChild,
      3
    ).select();
  ...
  testRemoveFormattingLinkedImageBorderZero() {
    ...
    expectedFailures.expectFailureFor(
      userAgent.WEBKIT,
      'WebKit removes the image entirely, see ' + 'https://bugs.webkit.org/show_bug.cgi?id=13125 .'
    );

    expectedFailures.run(() => {
      assertHTMLEquals(
        "Image's border=0 should not be removed during remove formatting",
        testHtml + controlCleanHtml,
        div.innerHTML
      );
      FIELDMOCK.$verify();
    });
  ...
  testRemoveFormattingLinkedImageBorderNonzero() {
    ...
    expectedFailures.run(() => {
      assertHTMLEquals(
        "Image's border should be removed during remove formatting" + ' if non-zero',
  ...
  testRemoveFormattingUnlinkedImage() {
    const testHtml = '<img src="http://www.google.com/images/logo.gif" border="0">';
    const div = document.getElementById('html');
    div.innerHTML = testHtml + controlHtml;
    Range.createFromNodeContents(div).select();
    FORMATTER.removeFormatting_();
  ...
  testRemoveFormattingLinkedImageDeep() {
    const testHtml =
      '<a href="http://www.google.com/"><b>hello' +
    ...
    expectedFailures.run(() => {
      assertHTMLEquals(
        ...
        testHtml.replace(/<\/?b>/g, '') + controlCleanHtml + insertImageBoldGarbage,
        div.innerHTML
      );
      FIELDMOCK.$verify();
    });
  ...
  testPreservesLinkFormatting() {
    ...
    expectedFailures.run(() => {
      FORMATTER.removeFormatting_();
      assertHTMLEquals(
        'Anchor formatting should be preserved',
        testHtml + controlHtml,
        div.innerHTML
      );
      FIELDMOCK.$verify();
    });
  ...
  testInsideTableRemoveFormatting() {
    const div = document.getElementById('html');
    div.innerHTML = '<table><tr><td><b id="b">foo</b></td></tr><tr><td>ba</td></tr></table>';
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        assertHTMLEquals(
          `<table><tr><td>foo${insertImageBoldGarbage}` + '</td></tr><tr><td>ba</td></tr></table>',
          div.innerHTML
        );
        FIELDMOCK.$verify();
      }
  ...
  testPartialTableRemoveFormatting() {
    ...
    expectedFailures.run(
      ...
      () => {
        ...
        // Only remove styling from inside tables.
        assertHTMLEquals(
          'bar<table><tr><td><b id="b">f</b>oo</td></tr>' + '<tr><td>banana</td></tr></table>baz',
          div.innerHTML
        );
        FIELDMOCK.$verify();
      }
  ...
  testTwoTablesSelectedFullyRemoveFormatting() {
    runExpectingFailuresForUnimplementedBrowsers(
      ...
      () => {
        ...
        // completely.
        div.innerHTML =
          '<table><tr><td>foo</td></tr></table>' + '<table><tr><td>bar</td></tr></table>';
        Range.createFromNodes(div.firstChild, 0, div.lastChild, 1).select();
        FORMATTER.removeFormatting_();
  ...
  testTwoTablesSelectedFullyInsideRemoveFormatting() {
    ...
    runExpectingFailuresForUnimplementedBrowsers(
      ...
      () => {
        ...
        Range.createFromNodes(
          dom.getElement('td1').firstChild,
          0,
          dom.getElement('td2').firstChild,
          3
        ).select();
        FORMATTER.removeFormatting_();
        assertHTMLEquals('<br>foo<br><br>bar<br>', div.innerHTML);
    ...
    runExpectingFailuresForUnimplementedBrowsers(
      ...
      () => {
        ...
        Range.createFromNodes(
          dom.getElement('td1').firstChild,
          0,
          dom.getElement('td2').firstChild.firstChild,
          2
        ).select();
        FORMATTER.removeFormatting_();
        let expectedHtml = '<br>foo<br>' + '<table><tr><td id="td2">ba<b>r</b></td></tr></table>';
        if (userAgent.EDGE) {
          // TODO(sdh): Edge
          // inserts an extra
          // empty <b> tag but is
    ...
    runExpectingFailuresForUnimplementedBrowsers(
      ...
      () => {
        ...
        Range.createFromNodes(
          dom.getElement('td1').firstChild,
          1,
          dom.getElement('td2').childNodes[1],
          1
        ).select();
        FORMATTER.removeFormatting_();
        assertHTMLEquals(
  ...
  testGetTableAncestor() {
    ...
    assertTrue('Full table is in table', !!FORMATTER.getTableAncestor_(div.childNodes[1]));

    assertFalse('Outside of table', !!FORMATTER.getTableAncestor_(div.firstChild));

    assertTrue(
      'Table cell is in table',
      !!FORMATTER.getTableAncestor_(div.childNodes[1].firstChild.firstChild.firstChild)
    );

    dom.setTextContent(div, 'foo');
    assertNull('No table inside field.', FORMATTER.getTableAncestor_(div.childNodes[0]));
  },
