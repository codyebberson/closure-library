...
goog.editor.Table = function (node) {
  ...
  if (!this.element) {
    goog.log.error(
      this.logger_,
      "Can't create Table based on a node " + "that isn't a table, or descended from a table."
    );
  }
...
goog.editor.Table.prototype.refresh = function () {
  ...
  var tbody = goog.dom.getElementsByTagName(
    ...
    goog.asserts.assert(this.element)
  )[0];
  if (!tbody) {
    return;
  }
 ...
 */
goog.editor.Table.getChildCellElements = (tr) => {
  var cells = [];
...
goog.editor.Table.prototype.insertColumn = function (opt_colIndex) {
  ...
  var colIndex =
    opt_colIndex != null ? opt_colIndex : (this.rows[0] && this.rows[0].columns.length) || 0;
  var newTds = [];
...
goog.editor.Table.prototype.removeColumn = function (colIndex) {
  for (var i = 0, row; (row = this.rows[i]); i++) {
    ...
    if (!cell) {
      goog.log.error(
        this.logger_,
        "Can't remove cell at position " + i + ', ' + colIndex + ': no such cell.'
      );
    }
    if (cell.colSpan > 1) {
  ...
  for (var i = startRowIndex; i <= endRowIndex; i++) {
    for (var j = startColIndex; j <= endColIndex; j++) {
      ...
      if (
        ...
        goog.log.warning(
          this.logger_,
          "Can't merge cells: the cell in row " +
            i +
            ', column ' +
            j +
 ...
 */
goog.editor.Table.createDomTable = (doc, columns, rows, opt_tableStyle) => {
