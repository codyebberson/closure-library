...
function assertArrayContainsString(string, strings) {
  asserts.assertEquals(
    'Expected the string "' +
      string +
      '" to appear exactly once in the array <' +
      strings.join(', ') +
      '>.',
    1,
    array.count(strings, (str) => str == string)
  );
}
...
testSuite({
  ...
  testStartButtonStartsTests /**
    ...
    const mockStart = mocks.createMethodMock(MultiTestRunner.prototype, 'start');
  ...
  testStopButtonStopsTests: () => {
    ...
    stubs.replace(MultiTestRunner.TestFrame.prototype, 'runTest', () => {
      return;
    });
  ...
  testRunsTestsAndReportsResults: () => {
    const promise = createEventPromise(testRunner, 'testsFinished');
    ...
    return promise.then((results) => {
      ...
      assertArrayContainsString('testdata/fake_failing_test2:testFail', testNames);
      assertArrayContainsString('testdata/fake_failing_test:testFail', testNames);
      assertArrayContainsString('testdata/fake_passing_test:testPass', testNames);
      ...
      assertArrayContainsString('testdata/fake_failing_test2.html', failedTests);
    });
  },

  testMissingTestResultsIsAFailure: () => {
    const promise = createEventPromise(testRunner, 'testsFinished');
    ...
    return promise.then((results) => {
      ...
      assertArrayContainsString('testdata/fake_failing_test3.html', failedTests);
    });
  },

  testShouldRunTestsFalseIsSuccess: () => {
    const promise = createEventPromise(testRunner, 'testsFinished');
  ...
  testFilterFunctionFiltersTest: () => {
    const promise = createEventPromise(testRunner, 'testsFinished');

    testRunner.render(document.getElementById('runner'));
    testRunner.setFilterFunction((test) => test.indexOf('fake_failing_test2') != -1);
    testRunner.start();
  ...
  testTimeoutFailsAfterTimeout: () => {
    ...
    return promise.then((results) => {
      ...
      assertArrayContainsString('testdata/fake_long_running_failing_test', testNames);
      assertArrayContainsString('testdata/fake_long_running_passing_test', testNames);
      ...
      assertEquals(2, failureReports.length);
      const failedTests = testRunner.getTestsThatFailed();
      assertArrayContainsString('testdata/fake_long_running_failing_test', failedTests);
      assertArrayContainsString('testdata/fake_long_running_passing_test', failedTests);
      assertEquals(2, failedTests.length);
    });
  },

  testRunsAllTestsWhenPoolSizeSmallerThanTotalTests: () => {
    const promise = createEventPromise(testRunner, 'testsFinished');
  ...
  testFrameDisposeInternal /**
    ...
    frame.createDom();
    frame.render();
    stubs.replace(frame, 'checkForCompletion_', () => {
      return;
    });
    frame.runTest(ALL_TESTS[0]);
    assertEquals(1, frame.getDomHelper().getElementsByTagNameAndClass('iframe').length);
    frame.dispose();
    assertTrue(frame.eh_.isDisposed());
    assertEquals(0, frame.getDomHelper().getElementsByTagNameAndClass('iframe').length);
