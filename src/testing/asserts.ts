...
var DOUBLE_EQUALITY_PREDICATE = (var1, var2) => var1 == var2;
var JSUNIT_UNDEFINED_VALUE = void 0;
var TO_STRING_EQUALITY_PREDICATE = (var1, var2) => var1.toString() === var2.toString();
 ...
 * @const {!Array<string>}
 */
const PRIMITIVE_TRUE_TYPES = ['String', 'Boolean', 'Number', 'Array', 'RegExp', 'Date', 'Function'];
...
const EQUALITY_PREDICATES = {
  ...
  Date: (date1, date2) => date1.getTime() == date2.getTime(),
 ...
 * @private
 */
goog.testing.asserts.numberRoughEqualityPredicate_ = (var1, var2, tolerance) =>
  Math.abs(var1 - var2) <= tolerance;
...
var _displayStringForValue = (aVar) => {
  var result;
  try {
    result = '<' + String(aVar) + '>';
  } catch (ex) {
...
goog.testing.asserts.fail = (failureMessage) => {
...
var argumentsIncludeComments = (expectedNumberOfNonCommentArgs, args) =>
  args.length == expectedNumberOfNonCommentArgs + 1;

var commentArg = (expectedNumberOfNonCommentArgs, args) => {
...
var nonCommentArg = (desiredNonCommentArgIndex, expectedNumberOfNonCommentArgs, args) =>
  argumentsIncludeComments(expectedNumberOfNonCommentArgs, args)
    ? args[desiredNonCommentArgIndex]
    : args[desiredNonCommentArgIndex - 1];

var _validateArguments = (expectedNumberOfNonCommentArgs, args) => {
  var valid =
...
var _assert = (comment, booleanValue, failureMessage) => {
  // If another framework has installed an adapter, tell it about the assertion.
  var adapter = typeof window !== 'undefined' && window['Closure assert adapter'];
  if (adapter) {
...
goog.testing.asserts.getDefaultErrorMsg_ = (expected, actual) => {
  ...
  var msg = ['Expected', expectedDisplayString, 'but was', actualDisplayString].join(
    shouldUseNewLines ? '\n' : ' '
  );
  ...
  if (typeof expected == 'string' && typeof actual == 'string') {
    ...
    if (commonPrefix > 2 || commonSuffix > 2) {
      ...
      msg +=
        ...
        commonPrefix +
        '. ' +
        ['Expected', '[' + expectedPrinted + ']', 'vs. actual', '[' + expectedActual + ']'].join(
          shouldUseNewLinesInDiff ? '\n' : ' '
        );
    }
  }
  return msg;
 ...
 */
goog.testing.asserts.assert = (a, opt_b) => {
  ...
  _assert(
    ...
    'Bad argument to assert(boolean): ' + _displayStringForValue(booleanValue)
  );
 ...
 */
goog.testing.asserts.assertThrows = (a, opt_b) => {
  _validateArguments(1, arguments);
 ...
 * @throws {goog.testing.JsUnitException} If the assertion failed.
 */
goog.testing.asserts.assertNotThrows = (a, opt_b) => {
  _validateArguments(1, arguments);
  var comment = commentArg(1, arguments);
  var func = nonCommentArg(1, 1, arguments);
  _assert(
 ...
 *     JsUnitException.
 */
goog.testing.asserts.assertThrowsJsUnitException = (callback, opt_expectedMessage) => {
  try {
    callback();
  } catch (e) {
    var testCase = _getCurrentTestCase();
    ...
    if (!e.isJsUnitException) {
      goog.testing.asserts.fail("Expected a JsUnitException, got '" + e + "' instead");
    }

    if (typeof opt_expectedMessage != 'undefined' && e.message != opt_expectedMessage) {
      goog.testing.asserts.fail(
        'Expected message [' + opt_expectedMessage + '] but got [' + e.message + ']'
      );
    }
...
/** @const */
var assertThrowsJsUnitException = goog.testing.asserts.assertThrowsJsUnitException;
 ...
 */
goog.testing.asserts.assertRejects = (a, opt_b) => {
  _validateArguments(1, arguments);
  var thenable = /** @type {!IThenable<*>} */ nonCommentArg(1, 1, arguments);
  var comment = commentArg(1, arguments);
  _assert(
    comment,
    goog.isObject(thenable) && typeof thenable.then === 'function',
  ...
  return thenable.then(
    () => {
      goog.testing.asserts.raiseException(
        comment,
        'IThenable passed into assertRejects did not reject'
      );
    },
    (e) => {
      goog.testing.asserts.removeOperaStacktrace_(e);
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertTrue = (a, opt_b) => {
  _validateArguments(1, arguments);
  var comment = commentArg(1, arguments);
  var booleanValue = nonCommentArg(1, 1, arguments);
  ...
  _assert(
    comment,
    typeof booleanValue === 'boolean',
    'Bad argument to assertTrue(boolean): ' + _displayStringForValue(booleanValue)
  );
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertFalse = (a, opt_b) => {
  _validateArguments(1, arguments);
  var comment = commentArg(1, arguments);
  var booleanValue = nonCommentArg(1, 1, arguments);
  ...
  _assert(
    comment,
    typeof booleanValue === 'boolean',
    'Bad argument to assertFalse(boolean): ' + _displayStringForValue(booleanValue)
  );
 ...
 */
goog.testing.asserts.assertEquals = (a, b, opt_c) => {
 ...
 * @param {*=} opt_c The actual value (3 args only).
 */
goog.testing.asserts.assertNotEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var var1 = nonCommentArg(1, 2, arguments);
  var var2 = nonCommentArg(2, 2, arguments);
  _assert(
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNull = (a, opt_b) => {
  _validateArguments(1, arguments);
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNotNull = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertUndefined = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
    aVar === JSUNIT_UNDEFINED_VALUE,
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNotUndefined = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNullOrUndefined = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNotNullNorUndefined = (a, opt_b) => {
  _validateArguments(1, arguments);
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNonEmptyString = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
    aVar !== JSUNIT_UNDEFINED_VALUE && aVar !== null && typeof aVar == 'string' && aVar !== '',
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNaN = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(
    commentArg(1, arguments),
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertNotNaN = (a, opt_b) => {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
 ...
 * @private
 */
goog.testing.asserts.getCustomEquality = (prototype) => {
  for (
    ;
    prototype != null && typeof prototype === 'object' && prototype !== Object.prototype;
 ...
 * @private
 */
goog.testing.asserts.getMostSpecificCustomEquality = (obj1, obj2) => {
  for (
 ...
 * @private
 */
goog.testing.asserts.applyCustomEqualityFunction = (comparator, obj1, obj2, path) => {
  const /* !goog.testing.asserts.EqualityFunction */ callback = (left, right, unusedEq) => {
 ...
 */
goog.testing.asserts.registerComparator = (prototype, fn) => {
 ...
 */
goog.testing.asserts.clearCustomComparator = (prototype) =>
  goog.testing.asserts.CUSTOM_EQUALITY_MATCHERS.delete(prototype);
 ...
 */
goog.testing.asserts.findDifferences = (expected, actual, opt_equalityPredicate) => {
  var failures = [];
  ...
  const equalityPredicate = (type, var1, var2) => {
    ...
    if (customPredicateResult !== goog.testing.asserts.EQUALITY_PREDICATE_CANT_PROCESS) {
    ...
    if (typeOfVar1 === typeOfVar2) {
      ...
      if (var1 && typeof var1 === 'object') {
        try {
          ...
          const comparator = goog.testing.asserts.getMostSpecificCustomEquality(o1, o2);
          if (comparator) {
      ...
      const isArray = isArrayBuffer || goog.testing.asserts.ARRAY_TYPES[typeOfVar1];
      ...
      if (errorMessage != goog.testing.asserts.EQUALITY_PREDICATE_CANT_PROCESS) {
        ...
        if (
          (typeof Map != 'undefined' && var1 instanceof Map) ||
          (typeof Set != 'undefined' && var1 instanceof Set)
        ) {
          var1.forEach((value, key) => {
            if (var2.has(key)) {
              ...
              if (var2.get) {
                innerAssertWithCycleCheck(
                  ...
                  value,
                  var2.get(key),
                  childPath.replace('%s', String(key))
                );
              }
            } else {
          ...
          for (var prop in var2) {
            ...
            if (!(prop in var1)) {
              failures.push(
                'property ' + prop + ' not present in expected ' + (path || typeOfVar1)
              );
            }
          }
 ...
 */
goog.testing.asserts.assertObjectEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
 ...
 */
goog.testing.asserts.assertObjectRoughlyEquals = (a, b, c, opt_d) => {
  ...
  var equalityPredicate = (type, var1, var2) => {
 ...
 * @param {*=} opt_c Comparison object, if an assertion message was provided.
 */
goog.testing.asserts.assertObjectNotEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var v1 = nonCommentArg(1, 2, arguments);
  var v2 = nonCommentArg(2, 2, arguments);
  var failureMessage = commentArg(2, arguments) ? commentArg(2, arguments) : '';
 ...
 */
goog.testing.asserts.assertArrayEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var v1 = nonCommentArg(1, 2, arguments);
  var v2 = nonCommentArg(2, 2, arguments);
  var failureMessage = commentArg(2, arguments) ? commentArg(2, arguments) : '';
 ...
 */
goog.testing.asserts.assertElementsEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  ...
  if (!v1) {
    ...
    goog.testing.asserts.assertEquals('length mismatch: ' + failureMessage, v1.length, v2.length);
    for (var i = 0; i < v1.length; ++i) {
      goog.testing.asserts.assertEquals(
        'mismatch at index ' + i + ': ' + failureMessage,
        v1[i],
        v2[i]
      );
    }
 ...
 */
goog.testing.asserts.assertElementsRoughlyEqual = (a, b, c, opt_d) => {
  _validateArguments(3, arguments);
  ...
  if (!v1) {
    ...
    goog.testing.asserts.assertEquals('length mismatch: ' + failureMessage, v1.length, v2.length);
    for (var i = 0; i < v1.length; ++i) {
      goog.testing.asserts.assertRoughlyEquals(failureMessage, v1[i], v2[i], tolerance);
    }
  }
};
/** @const */
var assertElementsRoughlyEqual = goog.testing.asserts.assertElementsRoughlyEqual;
 ...
 */
goog.testing.asserts.assertSameElements = (a, b, opt_c) => {
  _validateArguments(2, arguments);
 ...
 * @private
 */
goog.testing.asserts.isArrayLikeOrIterable_ = (obj) =>
  goog.isArrayLike(obj) || goog.testing.asserts.isIterable_(obj);

/**
 * @param {*} a The value to assert (1 arg) or debug message (2 args).
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertEvaluatesToTrue = (a, opt_b) => {
  _validateArguments(1, arguments);
 ...
 * @param {*=} opt_b The value to assert (2 args only).
 */
goog.testing.asserts.assertEvaluatesToFalse = (a, opt_b) => {
  _validateArguments(1, arguments);
  var value = nonCommentArg(1, 1, arguments);
  if (value) {
 ...
 * @param {*=} opt_c The actual value (3 args only).
 */
goog.testing.asserts.assertHTMLEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var var1 = nonCommentArg(1, 2, arguments);
  var var2 = nonCommentArg(2, 2, arguments);
  ...
  _assert(
    ...
    goog.testing.asserts.getDefaultErrorMsg_(var1Standardized, var2Standardized)
  );
};
/** @const */
 ...
 */
goog.testing.asserts.assertCSSValueEquals = (a, b, c, opt_d) => {
  _validateArguments(3, arguments);
  ...
  _assert(
    ...
    goog.testing.asserts.getDefaultErrorMsg_(expectedValueStandardized, actualValueStandardized)
  );
};
/** @const */
 ...
 * @param {*=} opt_c The actual value (3 args only).
 */
goog.testing.asserts.assertHashEquals = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var var1 = nonCommentArg(1, 2, arguments);
  var var2 = nonCommentArg(2, 2, arguments);
  var message = commentArg(2, arguments);
 ...
 */
goog.testing.asserts.assertRoughlyEquals = (a, b, c, opt_d) => {
  _validateArguments(3, arguments);
  ...
  _assert(
    commentArg(3, arguments),
    goog.testing.asserts.numberRoughEqualityPredicate_(expected, actual, tolerance),
 ...
 */
goog.testing.asserts.assertContains = (a, b, opt_c) => {
  _validateArguments(2, arguments);
 ...
 * @param {*=} opt_c The container.
 */
goog.testing.asserts.assertNotContains = (a, b, opt_c) => {
  _validateArguments(2, arguments);
  var contained = nonCommentArg(1, 2, arguments);
  var container = nonCommentArg(2, 2, arguments);
  _assert(
 ...
 */
goog.testing.asserts.assertRegExp = (a, b, opt_c) => {
  _validateArguments(2, arguments);
 ...
 * @private
 */
goog.testing.asserts.toArray_ = (obj) => {
  var ret = [];
  if (goog.testing.asserts.isIterable_(obj)) {
    var iterator = goog.testing.asserts.getIterator_(/** @type {!Iterable} */ obj);
 ...
 * @private
 */
goog.testing.asserts.isIterable_ = (obj) =>
 ...
 * @private
 */
goog.testing.asserts.getIterator_ = (iterable) => {
 ...
 * @private
 */
goog.testing.asserts.indexOf_ = (container, contained) => {
  if (typeof container.indexOf == 'function') {
    return /** @type {{indexOf: function(*): number}} */ container.indexOf(contained);
  } else {
 ...
 */
var standardizeCSSValue = (propertyName, value) => {
 ...
 */
goog.testing.asserts.raiseException = (comment, opt_message) => {
 ...
 * @private
 */
goog.testing.asserts.isArrayIndexProp_ = (prop) => prop === '0' || /^[1-9][0-9]*$/.test(prop);
