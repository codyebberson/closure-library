...
goog.testing.fs.FileReader.prototype.getResult = function () {
  ...
  } else if (this.returnType_ == goog.testing.fs.FileReader.ReturnType.ARRAY_BUFFER) {
    return this.blob_.toArrayBuffer();
  } else if (this.returnType_ == goog.testing.fs.FileReader.ReturnType.BINARY_STRING) {
    return this.blob_.toString();
  } else if (this.returnType_ == goog.testing.fs.FileReader.ReturnType.DATA_URL) {
...
goog.testing.fs.FileReader.prototype.read_ = function (blob) {
  ...
  goog.Timer.callOnce(
    function () {
      ...
      this.progressEvent_(goog.fs.FileReader.EventType.LOAD, blob.size / 2, blob.size);
      this.progressEvent_(goog.fs.FileReader.EventType.LOAD, blob.size, blob.size);
      this.readyState_ = goog.fs.FileReader.ReadyState.DONE;
      this.progressEvent_(goog.fs.FileReader.EventType.LOAD, blob.size, blob.size);
      this.progressEvent_(goog.fs.FileReader.EventType.LOAD_END, blob.size, blob.size);
    },
    0,
    this
  );
 ...
 * @private
 */
goog.testing.fs.FileReader.prototype.progressEvent_ = function (type, loaded, total) {
