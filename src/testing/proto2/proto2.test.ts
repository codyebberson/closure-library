...
testSuite({
  testAssertEquals() {
    ...
    let ex = assertThrowsJsUnitException(
      goog.partial(assertProto2Equals, new TestAllTypes(), new TestAllTypes.NestedMessage())
    );
    assertEquals('Message type mismatch: TestAllTypes != TestAllTypes.NestedMessage', ex.message);

    const message = new TestAllTypes();
    message.setOptionalInt32(1);
    ex = assertThrowsJsUnitException(goog.partial(assertProto2Equals, new TestAllTypes(), message));
  ...
  testFindDifferences_EmptyMessages() {
    assertEquals('', proto2.findDifferences_(new TestAllTypes(), new TestAllTypes(), ''));
  },
  ...
  testFindDifferences_FieldNotPresent() {
    ...
    assertEquals('optional_int32 should be present', proto2.findDifferences_(message, empty, ''));
    assertEquals(
  ...
  testFromObject() {
    ...
    assertObjectEquals(
      message,
      proto2.fromObject(TestAllTypes, {
        optional_int32: 2,
