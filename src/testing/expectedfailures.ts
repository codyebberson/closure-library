 ...
 * @private
 */
goog.testing.ExpectedFailures.prototype.logger_ = goog.log.getLogger(
...
goog.testing.ExpectedFailures.setUpConsole_ = () => {
  if (!goog.testing.ExpectedFailures.console_) {
    ...
    var div = goog.dom.createDom(
      ...
      {
        style:
          'position: absolute; border: 1px solid #333; right: 10px;' +
          'top : 10px; width: 400px; display: none',
      },
      xButton
    );
    document.body.appendChild(div);
    goog.events.listen(xButton, goog.events.EventType.CLICK, () => {
 ...
 * @private
 */
goog.testing.ExpectedFailures.prototype.getExpectationMessage_ = () =>
  "Expected a test failure in '" + goog.testing.TestCase.currentTestName + "' but the test passed.";
