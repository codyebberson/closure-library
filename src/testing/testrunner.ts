 ...
 * @return {string}
 */
goog.testing.TestRunner.prototype.getSearchString = () => window.location.search;
...
goog.testing.TestRunner.prototype.initialize = function (testCase) {
  if (this.testCase && this.testCase.running) {
    throw new Error('The test runner is already waiting for a test to complete');
  }
...
goog.testing.TestRunner.prototype.isComplete = function () {
  return this.initialized && !!this.testCase && this.testCase.started && !this.testCase.running;
};
...
goog.testing.TestRunner.prototype.logTestFailure = function (ex) {
  ...
  if (this.testCase) {
    ...
    throw new Error(
      'Test runner not initialized with a test case. Original ' + 'exception: ' + ex.message
    );
  }
};
...
goog.testing.TestRunner.prototype.execute = function () {
  if (!this.testCase) {
    throw new Error(
      'The test runner must be initialized with a test case ' + 'before execute can be called.'
    );
  }
 ...
 * @private
 */
goog.testing.TestRunner.shouldUsePromises_ = (testCase) =>
  testCase.constructor === goog.testing.TestCase;
...
goog.testing.TestRunner.prototype.writeLog = function (log) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var isFailOrError = /FAILED/.test(line) || /ERROR/.test(line) || /NO TESTS RUN/.test(line);
    if (isPassed) {
      color = 'darkgreen';
    } else if (isSkipped) {
      color = 'slategray';
    ...
    if (testNameMatch) {
      ...
      // names.
      var newSearch = 'runTests=' + encodeURIComponent(testNameMatch[1].replace(/,/g, '%2C'));
      var search = window.location.search;
      if (search) {
        var oldTests = /runTests=([^&]*)/.exec(search);
        if (oldTests) {
          newSearch =
            search.slice(0, oldTests.index) +
            newSearch +
  ...
  if (this.testCase) {
    ...
    if (this.hasErrors()) {
      ...
      for (var i = 0; i < this.errors.length; i++) {
        globalErrors.push({ source: '', message: this.errors[i], stacktrace: '' });
      }
