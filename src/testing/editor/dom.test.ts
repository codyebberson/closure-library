...
function setUpNonEmptyTests() {
  ...
  parentNode = dom.createDom(TagName.DIV, null, childNode1, childNode2, childNode3);
...
function createFakeRange(startNode, startOffset, endNode = undefined, endOffset = undefined) {
...
testSuite({
  ...
  testGetNextNonEmptyTextNode() {
    ...
    assertEquals('Should have found the next non-empty text node', 'One', nodeOne.nodeValue);
    const nodeTwoA = editorDom.getNextNonEmptyTextNode(nodeOne);
    assertEquals('Should have found the next non-empty text node', 'TwoA', nodeTwoA.nodeValue);
    const nodeTwoB = editorDom.getNextNonEmptyTextNode(nodeTwoA);
    assertEquals('Should have found the next non-empty text node', 'TwoB', nodeTwoB.nodeValue);
    const nodeThree = editorDom.getNextNonEmptyTextNode(nodeTwoB);
    assertEquals('Should have found the next non-empty text node', 'Three', nodeThree.nodeValue);
    ...
    const nodeBeforeStop = editorDom.getNextNonEmptyTextNode(nodeTwoA, childNode2);
    assertEquals(
      'Should have found the next non-empty text node',
      'TwoB',
  ...
  testGetPreviousNonEmptyTextNode() {
    ...
    assertEquals(
      'Should have found the previous non-empty text node',
      'Three',
      nodeThree.nodeValue
    );
    const nodeTwoB = editorDom.getPreviousNonEmptyTextNode(nodeThree);
    assertEquals('Should have found the previous non-empty text node', 'TwoB', nodeTwoB.nodeValue);
    const nodeTwoA = editorDom.getPreviousNonEmptyTextNode(nodeTwoB);
    assertEquals('Should have found the previous non-empty text node', 'TwoA', nodeTwoA.nodeValue);
    const nodeOne = editorDom.getPreviousNonEmptyTextNode(nodeTwoA);
    assertEquals('Should have found the previous non-empty text node', 'One', nodeOne.nodeValue);
    const nodeNull = editorDom.getPreviousNonEmptyTextNode(nodeOne, parentNode);
    assertNull('Should not have found any non-empty text node', nodeNull);

    const nodeStop = editorDom.getPreviousNonEmptyTextNode(nodeThree, childNode3);
    assertNull('Should have stopped before finding a node', nodeStop);

    const nodeBeforeStop = editorDom.getPreviousNonEmptyTextNode(nodeTwoB, childNode2);
    assertEquals(
      'Should have found the previous non-empty text node',
      'TwoA',
      nodeBeforeStop.nodeValue
  ...
  testAssertRangeBetweenText0() {
    ...
    editorDom.assertRangeBetweenText('0', '1', createFakeRange(first.firstChild, 1));
  },

  testAssertRangeBetweenText1() {
    setUpAssertRangeBetweenText();
    editorDom.assertRangeBetweenText('1', '2', createFakeRange(first.firstChild, 2));
  },
  ...
  testAssertRangeBetweenText5() {
    setUpAssertRangeBetweenText();
    editorDom.assertRangeBetweenText('1', '2', createFakeRange(middle.firstChild, 0));
  },
  ...
  testAssertRangeBetweenText9() {
    setUpAssertRangeBetweenText();
    editorDom.assertRangeBetweenText('1', '2', createFakeRange(last.firstChild, 0));
  },

  testAssertRangeBetweenTextBefore() {
    setUpAssertRangeBetweenText();
    // Test that it works when the cursor is at the beginning of all text.
    editorDom.assertRangeBetweenText('', '0', createFakeRange(first.firstChild, 0), root); // Restrict to root div so it won't find /n's and script.
  },

  testAssertRangeBetweenTextAfter() {
    setUpAssertRangeBetweenText();
    // Test that it works when the cursor is at the end of all text.
    editorDom.assertRangeBetweenText('3', '', createFakeRange(last.firstChild, 2), root); // Restrict to root div so it won't find /n's and script.
  },

  testAssertRangeBetweenTextFail1() {
    setUpAssertRangeBetweenText();
    const e = assertThrowsJsUnitException(() => {
      editorDom.assertRangeBetweenText('1', '3', createFakeRange(first.firstChild, 2));
    });
    assertContains('Assert reason incorrect', 'Expected <3> after range but found <23>', e.message);
  },
  ...
  testAssertRangeBetweenTextFail2() {
    ...
    assertContains('Assert reason incorrect', 'Expected <2> after range but found <3>', e.message);
  },
  ...
  testAssertRangeBetweenTextBeforeFail() {
    ...
    const e = assertThrowsJsUnitException(() => {
      editorDom.assertRangeBetweenText('-1', '0', createFakeRange(first.firstChild, 0), root); // Restrict to root div so it won't find /n's and script.
    });
    assertContains(
  ...
  testAssertRangeBetweenTextAfterFail() {
    ...
    const e = assertThrowsJsUnitException(() => {
      editorDom.assertRangeBetweenText('3', '4', createFakeRange(last.firstChild, 2), root); // Restrict to root div so it won't find /n's and script.
    });
    assertContains(
      'Assert reason incorrect',
