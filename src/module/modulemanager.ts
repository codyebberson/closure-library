...
goog.scope(() => {
  ...
  goog.module.ModuleManager.getInstance = () => goog.loader.activeModuleManager.get();
    ...
    for (var i = 0; i < modules.length; i++) {
      ...
      if (parts[1]) {
        ...
        for (var j = 0; j < deps.length; j++) {
          var index = Number.parseInt(deps[j], 36);
   ...
   */
  goog.module.ModuleManager.SUBTRACTIVE_MODULE_LOADING = goog.define(
  ...
  goog.module.ModuleManager.prototype.dispatchActiveIdleChangeIfNeeded_ = function () {
    ...
    if (active != lastActive) {
      this.executeCallbacks_(
        active
          ? goog.loader.AbstractModuleManager.CallbackType.ACTIVE
   ...
   * @private
   */
  goog.module.ModuleManager.prototype.loadModulesOrEnqueueIfNotLoadedOrLoading_ = function (
    ids,
    opt_userInitiated
  ) {
    var uniqueIds = [];
  ...
  goog.module.ModuleManager.prototype.registerModuleLoadCallbacks_ = function (
    ...
    moduleInfo.registerErrback((err) => {
  ...
  goog.module.ModuleManager.prototype.loadModules_ = function (ids, opt_isRetry, opt_forceReload) {
    ...
    var loadFn = goog.bind(
      this.getLoader().loadModules,
      goog.asserts.assert(this.getLoader()),
  ...
  goog.module.ModuleManager.prototype.processModulesForLoad_ = function (ids) {
    ids = ids.filter((id) => {
      const moduleInfo = this.moduleInfoMap[id];
      if (moduleInfo.isLoaded()) {
    ...
    if (!this.batchModeEnabled_ && idsWithDeps.length > 1) {
      ...
      var queuedModules = idsWithDeps.map((id) => [id]);
    ...
    if (this.currentlyLoadingModule_ == this.baseModuleInfo_) {
      ...
      if (error) {
        this.dispatchModuleLoadFailed_(
          new goog.module.ModuleLoadFailure(goog.module.ModuleLoadFailure.Type.INIT_ERROR)
        );
      }
  ...
  goog.module.ModuleManager.prototype.beforeLoadModuleCode = function (id) {
    ...
    if (
      ...
      goog.log.error(
        this.logger_,
        'beforeLoadModuleCode called with module "' +
          id +
          '" while module "' +
  ...
  goog.module.ModuleManager.prototype.registerInitializationCallback = function (fn, opt_handler) {
    if (!this.currentlyLoadingModule_) {
      this.moduleInfoMap[SYNTHETIC_MODULE_OVERHEAD_ID] = new goog.module.ModuleInfo(
        [],
        SYNTHETIC_MODULE_OVERHEAD_ID
      );
  ...
  /** @override */
  goog.module.ModuleManager.prototype.registerLateInitializationCallback = function (
    fn,
    opt_handler
  ) {
    if (!this.currentlyLoadingModule_) {
  ...
  goog.module.ModuleManager.prototype.handleLoadError_ = function (
    ...
    requestedModuleIdsWithDeps.forEach(
      goog.partial(goog.array.remove, this.requestedModuleIds_),
      this
    );
    ...
    if (status == 401) {
      ...
      this.dispatchModuleLoadFailed_(
        new goog.module.ModuleLoadFailure(goog.module.ModuleLoadFailure.Type.UNAUTHORIZED, status)
      );
      ...
      this.requeueBatchOrDispatchFailure_(
        new goog.module.ModuleLoadFailure(goog.module.ModuleLoadFailure.Type.OLD_CODE_GONE, status)
      );
      this.loadNextModules_();
    } else if (this.consecutiveFailures_ >= 3) {
      goog.log.info(this.logger_, 'Aborting after failure to load: ' + this.loadingModuleIds_);
      this.requeueBatchOrDispatchFailure_(
        new goog.module.ModuleLoadFailure(
          goog.module.ModuleLoadFailure.Type.CONSECUTIVE_FAILURES,
          status
        )
      );
      this.loadNextModules_();
  ...
  goog.module.ModuleManager.prototype.handleLoadTimeout_ = function () {
    ...
    this.requeueBatchOrDispatchFailure_(
      new goog.module.ModuleLoadFailure(goog.module.ModuleLoadFailure.Type.TIMEOUT)
    );
    this.loadNextModules_();
  };
  ...
  goog.module.ModuleManager.prototype.requeueBatchOrDispatchFailure_ = function (cause) {
    ...
    if (this.requestedLoadingModuleIds_.length > 1) {
      var queuedModules = this.requestedLoadingModuleIds_.map((id) => [id]);
      this.requestedModuleIdsQueue_ = queuedModules.concat(this.requestedModuleIdsQueue_);
    } else {
    ...
    for (var i = 0; i < this.requestedModuleIdsQueue_.length; i++) {
      var dependentModules = this.requestedModuleIdsQueue_[i].filter(
        ...
        function (requestedId) {
          ...
          return goog.array.some(failedIds, (id) => goog.array.contains(requestedDeps, id));
        },
        this
      );
  ...
  goog.loader.activeModuleManager.setDefault(() => new goog.module.ModuleManager());
}); // goog.scope
