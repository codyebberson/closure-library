 ...
 */
goog.graphics.paths.createRegularNGon = (center, vertex, n) => {
 ...
 * @return {!goog.graphics.Path} The path.
 */
goog.graphics.paths.createArrow = (a, b, aHead, bHead) => {
  var path = new goog.graphics.Path();
  path.moveTo(a.x, a.y);
  path.lineTo(b.x, b.y);
  ...
  if (aHead) {
    path.appendPath(
      goog.graphics.paths.createRegularNGon(
        new goog.math.Coordinate(a.x + aHead * Math.cos(angle), a.y + aHead * Math.sin(angle)),
        a,
        3
      )
    );
