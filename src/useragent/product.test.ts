...
const DETECTED_BROWSER_KEYS = ['FIREFOX', 'IPHONE', 'IPAD', 'ANDROID', 'CHROME', 'SAFARI'];

// browserKey should be the constant name, as a string
// 'FIREFOX', 'CHROME', 'ANDROID', etc.
function assertIsBrowser(currentBrowser) {
  assertTrue(
    'Current browser key into goog.userAgent.product ' + 'should be true',
...
function assertBrowserAndVersion(userAgent, browser, version) {
  ...
  assertEquals('User agent should have this version', version, googUserAgent.VERSION);
}
...
function checkEachUserAgentDetected(userAgents, browser) {
  googArray.forEach(userAgents, (ua) => {
    ...
    ua.versions.forEach((v) => {
      ...
      updateUserAgentUtils();
      assertEquals(
        'Expected version ' + v.num + ' from ' + ua.ua + ' but got ' + product.VERSION,
        v.truth,
        isVersion(v.num)
      );
    });
...
testSuite({
  ...
  testSafari() {
    const userAgents = [
      {
        ua:
          'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_7; de-de) ' +
      ...
      {
        ua:
          'Mozilla/5.0 (Windows; U; Windows NT 6.0; pl-PL) ' +
  ...
  testIpad() {
    const userAgents = [
      ...
      {
        ua:
          'Mozilla/5.0 (iPad; CPU OS 8_1 like Mac OS X) AppleWebKit/600.1.4' +
